...
class WebsocketsFullDuplexHandlerAlt final
    : public server::websocket::WebsocketHandlerBase {
 public:
  static constexpr std::string_view kName = "websocket-duplex-handler-alt";
  ...
  void Handle(server::websocket::WebSocketConnection& chat,
    ...
    server::websocket::Message message;
    while (!engine::current_task::ShouldCancel()) {
      const bool msgIsReceived = chat.Recv2(message);
      if (msgIsReceived) {
        if (message.close_status) break;
        chat.Send(message);
      }
         ...
         // e.g. chat.Send(server::websocket::Message{ "blah", {}, true });
      }
    }
    if (message.close_status) chat.Close(*message.close_status);
  }
...
int main(int argc, char* argv[]) {
  const auto component_list = components::MinimalServerComponentList()
                                  ...
                                  .Append<WebsocketsFullDuplexHandlerAlt>()
