...
struct CacheControl::Impl final {
    ...
    using List = boost::intrusive::list<CacheInfoNode, MemberHook, boost::intrusive::constant_time_size<true>>;
...
bool CacheControl::IsPeriodicUpdateEnabled(const cache::Config& cache_config, const std::string& cache_name) const {
    ...
    if (is_periodic_update_forced) {
        ...
        reason = "config";
    } else if (impl_->periodic_updates_mode == impl::PeriodicUpdatesMode::kDefault) {
        enabled = true;
        reason = "default";
    } else {
        enabled = (impl_->periodic_updates_mode == impl::PeriodicUpdatesMode::kEnabled);
        reason = "global config";
    }

    LOG_DEBUG() << cache_name << " periodic update is " << (enabled ? "enabled" : "disabled") << " by " << reason;
    return enabled;
}
...
void CacheControl::DoResetSingleCache(
    ...
    update_type = force_incremental_names.count(info.name) ? cache::UpdateType::kIncremental : update_type;

    TESTPOINT(std::string("reset-cache-") + info.name, formats::json::MakeObject("update_type", ToString(update_type)));
...
void CacheControl::ResetAllCaches(
    ...
    const auto sp = tracing::ScopeTime::CreateOptionalScopeTime("reset_all_caches");
...
void CacheControl::ResetCaches(
    ...
    DoResetCaches(update_type, &reset_only_names, force_incremental_names, nullptr);
}

CacheResetRegistration CacheControl::RegisterPeriodicCache(cache::CacheUpdateTrait& cache) {
    CacheInfo info;
    info.name = std::string{cache.Name()};
    info.reset = [&cache](cache::UpdateType update_type) { cache.UpdateSyncDebug(update_type); };
