...
void PeriodicTaskControl::RegisterPeriodicTask(const std::string& name, utils::PeriodicTask& task) {
    UINVARIANT(
        !name.empty(),
...
void PeriodicTaskControl::UnregisterPeriodicTask(const std::string& name, utils::PeriodicTask& task) {
    auto periodic_tasks = periodic_tasks_.Lock();
    const auto it = periodic_tasks->find(name);

    UINVARIANT(it != periodic_tasks->end(), "Periodic task name " + name + " is not known");
    UINVARIANT(&it->second == &task, "Periodic task name " + name + " does not belong to the caller");
...
void PeriodicTaskControl::SuspendPeriodicTasks(const std::unordered_set<std::string>& names) {
...
utils::PeriodicTask& PeriodicTaskControl::FindPeriodicTask(const std::string& name) {
    const auto periodic_tasks = periodic_tasks_.Lock();
    const auto it = periodic_tasks->find(name);
    UINVARIANT(it != periodic_tasks->end(), "Periodic task name " + name + " is not known");
    return it->second;
...
PeriodicTaskRegistrationHolder::PeriodicTaskRegistrationHolder(
    PeriodicTaskControl& periodic_task_control,
    std::string name,
