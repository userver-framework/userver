...
[[maybe_unused]] NonMovable Read(dump::Reader&, dump::To<NonMovable>) { return {}; }
...
bool operator==(const MyPair& lhs, const MyPair& rhs) { return lhs.x == rhs.x && lhs.y == rhs.y; }
...
bool operator==(const NonAggregate& lhs, const NonAggregate& rhs) { return lhs.GetFoo() == rhs.GetFoo(); }

void Write(dump::Writer& writer, const NonAggregate& na) { writer.Write(na.GetFoo()); }

NonAggregate Read(dump::Reader& reader, dump::To<NonAggregate>) { return NonAggregate{reader.Read<int>()}; }
