...
bool operator==(const NonCopyable& lhs, const NonCopyable& rhs) { return lhs.value == rhs.value; }

void Write(dump::Writer& writer, const NonCopyable& value) { writer.Write(value.value); }

NonCopyable Read(dump::Reader& reader, dump::To<NonCopyable>) { return NonCopyable{reader.Read<int>()}; }

struct NonMovable {
    std::atomic<int> atomic{0};
};

void Write(dump::Writer& writer, const NonMovable& value) { writer.Write(value.atomic.load()); }

NonMovable Read(dump::Reader& reader, dump::To<NonMovable>) { return NonMovable{std::atomic<int>{reader.Read<int>()}}; }
...
bool operator==(const Dummy& d1, const Dummy& d2) { return d1.id == d2.id && d1.name == d2.name; }
...
TEST(DumpCommonContainers, Variant) {
    ...
    UEXPECT_THROW_MSG(
        (dump::FromBinary<std::variant<int, std::string>>("\x02")),
        std::runtime_error,
        "Invalid std::variant index in dump"
    );
    UEXPECT_THROW_MSG(
        (dump::FromBinary<std::variant<int, std::string>>("\x42")),
        std::runtime_error,
        "Invalid std::variant index in dump"
    );
}
...
TEST(DumpCommonContainers, UniquePtr) {
    ...
    const auto after1 = FromBinary<std::unique_ptr<NonMovable>>(ToBinary(before1));
    EXPECT_FALSE(after1);

    const auto before2 = std::make_unique<NonMovable>();
    before2->atomic = 42;
    const auto after2 = FromBinary<std::unique_ptr<NonMovable>>(ToBinary(before2));
...
TEST(DumpCommonContainers, SharedPtr) {
    ...
    const auto after1 = FromBinary<std::shared_ptr<NonMovable>>(ToBinary(before1));
    EXPECT_FALSE(after1);

    const auto before2 = std::make_shared<NonMovable>();
    before2->atomic = 42;
    const auto after2 = FromBinary<std::shared_ptr<NonMovable>>(ToBinary(before2));
    EXPECT_EQ(before2->atomic.load(), after2->atomic.load());
}
...
TEST(DumpCommonContainers, MultiIndexWithReserve) {
    using Index = boost::multi_index::indexed_by<
        boost::multi_index::hashed_unique<boost::multi_index::member<Dummy, int, &Dummy::id>>>;
...
TEST(DumpCommonContainers, MultiIndexWithoutReserve) {
    using Index = boost::multi_index::indexed_by<
        boost::multi_index::ordered_unique<boost::multi_index::member<Dummy, int, &Dummy::id>>>;
