...
using NumericTestTypes = ::testing::Types<
    int,
    std::uint8_t,
    std::int8_t,
    std::uint16_t,
...
TYPED_TEST(DumpCommonNumeric, Numeric) {
    ...
    if constexpr (std::is_floating_point_v<Num>) {
        ...
        TestWriteReadCycle(-std::numeric_limits<Num>::infinity());
        EXPECT_TRUE(std::isnan(dump::FromBinary<Num>(dump::ToBinary<Num>(std::numeric_limits<Num>::quiet_NaN()))));
    } else {
...
struct TwoStrings {
    ...
    bool operator==(const TwoStrings& other) const { return a == other.a && b == other.b; }
};
...
TEST(DumpCommon, ReadStringViewUnsafe) { TestWriteReadCycle(TwoStrings{"", "abc"}); }
...
void Write(dump::Writer& writer, const MyBigInt& value) { writer.Write(value.AsStringView()); }

MyBigInt Read(dump::Reader& reader, dump::To<MyBigInt>) { return MyBigInt(dump::ReadStringViewUnsafe(reader)); }
...
TEST(DumpCommon, TimePoint) {
    TestWriteReadCycle(std::chrono::system_clock::time_point{} + std::chrono::minutes{5});

    TestWriteReadCycle(
        std::chrono::time_point<std::chrono::system_clock, std::chrono::duration<int8_t, std::ratio<2, 7>>>{} +
...
TEST(DumpCommon, Decimal) {
    ...
    TestWriteReadCycle(decimal64::Decimal<4, decimal64::FloorRoundPolicy>{"0.4246"});
    TestWriteReadCycle(decimal64::Decimal<4, decimal64::CeilingRoundPolicy>{"0.4246"});
}
...
TEST(DumpCommon, JsonValue) { TestWriteReadCycle(formats::json::MakeObject("foo", 42, "bar", "baz")); }
