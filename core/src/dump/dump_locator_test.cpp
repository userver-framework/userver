...
dump::TimePoint BaseTime() {
    return std::chrono::time_point_cast<dump::TimePoint::duration>(
        utils::datetime::Stringtime("2015-03-22T090000.000000Z", "UTC", dump::kFilenameDateFormat)
    );
}

std::vector<std::string> InitialFileNames() {
    return {
        "2015-03-22T090000.000000Z-v5",
        "2015-03-22T090000.000000Z-v0",
        "2015-03-22T090000.000000Z-v42",
        "2015-03-22T090001.000000Z-v5",
        "2015-03-22T090002.000000Z-v5",
...
std::vector<std::string> UnrelatedFileNames() {
    return {
        "blah-2015-03-22T090000.000000Z-v5",
...
std::string Filename(const std::string& full_path) { return boost::filesystem::path{full_path}.filename().string(); }
...
max-age: null
    ...
    if (dump_info) {
        ...
        EXPECT_EQ(fs::blocking::ReadFileContents(dump_info->full_path), "2015-03-22T090003.000000Z-v5");
    }
...
max-age: 30m
...

    {
        ...
        locator.Cleanup();
        EXPECT_EQ(dump::FilenamesInDirectory(dir, kDumperName), (std::set<std::string>{file3, file4}));
    }

    {
        // Update time bumps from legacy filenames is not supported
        const auto dump_stats = locator.BumpDumpTime(BaseTime() + 50min, BaseTime() + 60min);
