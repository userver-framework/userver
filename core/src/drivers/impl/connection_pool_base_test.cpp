...
private:
    ...
    static ConnectionUniquePtr DoCreateConnection(engine::Deadline) { throw std::runtime_error{"oops"}; }
};
...
public:
    ...
    std::size_t AliveConnectionsCount() const { return AliveConnectionsCountApprox(); }
...
class BrokenConnectionsPool final : public DummyBasePool<BrokenConnectionsPool, BrokenConnection> {
public:
    ...
    std::size_t GetDestroyedMetric() const { return destroyed_connections_metric_; }
    ...
    void Release(ConnectionUniquePtr connection_ptr) { ReleaseConnection(std::move(connection_ptr)); }
...
private:
    ...
    void AccountConnectionDestroyed() noexcept override { ++destroyed_connections_metric_; }
...
std::shared_ptr<DerivedPool> CreatePool(
    ...
    std::size_t max_pool_size = 5,
    std::size_t max_simultaneously_connecting_clients = 2
) {
    auto pool = std::make_shared<DerivedPool>(max_pool_size, max_simultaneously_connecting_clients);
...
UTEST(ConnectionPoolBase, RethrowsFromInit) { EXPECT_ANY_THROW(CreatePool<ThrowingPool>()); }
...
UTEST(ConnectionPoolBase, HonorsMaxPoolSize) {
    ...
    const auto acquire = [&pool] {
        return pool->Acquire(engine::Deadline::FromDuration(std::chrono::milliseconds{2}));
    };
...
UTEST(ConnectionPoolBase, HonorsMaxSimultaneouslyConnectingClients) {
    ...
    UASSERT_THROW(engine::WaitAllChecked(acquire_tasks), PoolWaitLimitExceededError);
