...
#define IMPLEMENT_CURL_OPTION(FUNCTION_NAME, OPTION_NAME, OPTION_TYPE)                                               \
    ...
    inline void FUNCTION_NAME(OPTION_TYPE arg, std::error_code& ec) {                                                \
        ec = std::error_code(static_cast<errc::EasyErrorCode>(native::curl_easy_setopt(handle_, OPTION_NAME, arg))); \
    }
...
#define IMPLEMENT_CURL_OPTION_ENUM(FUNCTION_NAME, OPTION_NAME, ENUM_TYPE, OPTION_TYPE)    \
    ...
    inline void FUNCTION_NAME(ENUM_TYPE arg, std::error_code& ec) {                       \
        ec = std::error_code(static_cast<errc::EasyErrorCode>(                            \
            native::curl_easy_setopt(handle_, OPTION_NAME, static_cast<OPTION_TYPE>(arg)) \
        ));                                                                               \
    }
...
private:                                                                                                               \
    inline void FUNCTION_NAME##_impl(std::string_view sv, unsigned int flags, std::error_code& ec) {                   \
        native::curl_blob blob{};                                                                                      \
        /* NOLINTNEXTLINE(cppcoreguidelines-pro-type-const-cast) */                                                    \
        blob.data = const_cast<void*>(static_cast<const void*>(sv.data()));                                            \
        blob.len = sv.size();                                                                                          \
        blob.flags = flags;                                                                                            \
        ec = std::error_code(static_cast<errc::EasyErrorCode>(native::curl_easy_setopt(handle_, OPTION_NAME, &blob))); \
    }                                                                                                                  \
                                                                                                                       \
public:                                                                                                                \
    ...
    inline void FUNCTION_NAME##_no_copy(std::string_view sv, std::error_code& ec) {                                    \
...
#define DELETE_CURL_OPTION_BLOB(FUNCTION_NAME)                                     \
    ...
    inline void FUNCTION_NAME##_copy(std::string_view, std::error_code&) = delete; \
...
public:
    ...
    using progress_callback_t = std::function<bool(
    ...
    using debug_callback_t = int (*)(native::CURL*, native::curl_infotype, char*, size_t, void*);
    ...
    IMPLEMENT_CURL_OPTION_STRING(set_unix_socket_path, native::CURLOPT_UNIX_SOCKET_PATH);
    IMPLEMENT_CURL_OPTION(set_connect_to, native::CURLOPT_CONNECT_TO, native::curl_slist*);
    ...
    void add_header(
        ...
        DuplicateHeaderAction duplicate_header_action = DuplicateHeaderAction::kAdd
    );
    void add_header(
        std::string_view name,
        std::string_view value,
    ...
    void add_header(std::string_view name, std::string_view value, DuplicateHeaderAction duplicate_header_action);
    void add_header(
        std::string_view name,
        std::string_view value,
        std::error_code& ec,
    ...
    void add_resolve(const std::string& host, const std::string& port, const std::string& addr, std::error_code& ec);
    ...
    inline void set_ssl_verify_host(bool verify_host, std::error_code& ec) {
        ec = std::error_code{static_cast<errc::EasyErrorCode>(
            native::curl_easy_setopt(handle_, native::CURLOPT_SSL_VERIFYHOST, verify_host ? 2L : 0L)
        )};
    }
    ...
    IMPLEMENT_CURL_OPTION_GET_STRING_VIEW(get_effective_url, native::CURLINFO_EFFECTIVE_URL);
    ...
    IMPLEMENT_CURL_OPTION_GET_CURL_OFF_T(get_size_upload, native::CURLINFO_SIZE_UPLOAD_T);
    // CURLINFO_SIZE_DOWNLOAD
    IMPLEMENT_CURL_OPTION_GET_CURL_OFF_T(get_size_download, native::CURLINFO_SIZE_DOWNLOAD_T);
    // CURLINFO_SPEED_DOWNLOAD
    IMPLEMENT_CURL_OPTION_GET_CURL_OFF_T(get_speed_download_bps, native::CURLINFO_SPEED_DOWNLOAD_T);
    // CURLINFO_SPEED_UPLOAD
    IMPLEMENT_CURL_OPTION_GET_CURL_OFF_T(get_speed_upload_bps, native::CURLINFO_SPEED_UPLOAD_T);
    ...
    // CURLINFO_FILETIME
    IMPLEMENT_CURL_OPTION_GET_CURL_OFF_T(get_filetime_sec, native::CURLINFO_FILETIME_T);
    // CURLINFO_CONTENT_LENGTH_DOWNLOAD
    IMPLEMENT_CURL_OPTION_GET_CURL_OFF_T(get_content_length_download, native::CURLINFO_CONTENT_LENGTH_DOWNLOAD_T);
    // CURLINFO_CONTENT_LENGTH_UPLOAD
    IMPLEMENT_CURL_OPTION_GET_CURL_OFF_T(get_content_length_upload, native::CURLINFO_CONTENT_LENGTH_UPLOAD_T);
    // CURLINFO_STARTTRANSFER_TIME
    IMPLEMENT_CURL_OPTION_GET_STRING_VIEW(get_content_type, native::CURLINFO_CONTENT_TYPE);
    ...
    IMPLEMENT_CURL_OPTION_GET_STRING_VIEW(get_rtsp_session_id, native::CURLINFO_RTSP_SESSION_ID);
    ...
    IMPLEMENT_CURL_OPTION_GET_STRING_VIEW(get_local_ip, native::CURLINFO_LOCAL_IP);
    IMPLEMENT_CURL_OPTION_GET_LONG(get_local_port, native::CURLINFO_LOCAL_PORT);
    // CURLINFO_TLS_SESSION
    // CURLINFO_ACTIVESOCKET
    // CURLINFO_TLS_SSL_PTR
    IMPLEMENT_CURL_OPTION_GET_LONG(get_http_version, native::CURLINFO_HTTP_VERSION);
    IMPLEMENT_CURL_OPTION_GET_LONG(get_proxy_ssl_verifyresult, native::CURLINFO_PROXY_SSL_VERIFYRESULT);
    ...
    IMPLEMENT_CURL_OPTION_GET_CURL_OFF_T(get_total_time_usec, native::CURLINFO_TOTAL_TIME_T);
    IMPLEMENT_CURL_OPTION_GET_CURL_OFF_T(get_namelookup_time_usec, native::CURLINFO_NAMELOOKUP_TIME_T);
    IMPLEMENT_CURL_OPTION_GET_CURL_OFF_T(get_connect_time_usec, native::CURLINFO_CONNECT_TIME_T);
    IMPLEMENT_CURL_OPTION_GET_CURL_OFF_T(get_pretransfer_time_usec, native::CURLINFO_PRETRANSFER_TIME_T);
    IMPLEMENT_CURL_OPTION_GET_CURL_OFF_T(get_starttransfer_time_usec, native::CURLINFO_STARTTRANSFER_TIME_T);
    IMPLEMENT_CURL_OPTION_GET_CURL_OFF_T(get_redirect_time_usec, native::CURLINFO_REDIRECT_TIME_T);
    IMPLEMENT_CURL_OPTION_GET_CURL_OFF_T(get_appconnect_time_usec, native::CURLINFO_APPCONNECT_TIME_T);
    IMPLEMENT_CURL_OPTION_GET_CURL_OFF_T(get_retry_after_sec, native::CURLINFO_RETRY_AFTER);
...
private:
    ...
    static size_t read_function(void* ptr, size_t size, size_t nmemb, void* userdata) noexcept;
    static int seek_function(void* instream, native::curl_off_t offset, int origin) noexcept;
    static int xferinfo_function(
        void* clientp,
        native::curl_off_t dltotal,
        native::curl_off_t dlnow,
        native::curl_off_t ultotal,
