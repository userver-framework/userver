...
constexpr auto kGlobalSocketLimit = curl::errc::RateLimitErrorCode::kGlobalSocketLimit;
constexpr auto kPerHostSocketLimit = curl::errc::RateLimitErrorCode::kPerHostSocketLimit;

std::error_code GetCheckError(curl::ConnectRateLimiter& limiter, const char* url) {
    std::error_code ec;
...
TEST(CurlConnectRateLimiter, GlobalLimits) {
    ...
    for (size_t i = 0; i < kRepetitions; ++i) {
        ...
        EXPECT_FALSE(GetCheckError(limiter, "https://secure_init"));
        EXPECT_FALSE(GetCheckError(limiter, "https://secure_init"));
        EXPECT_EQ(kGlobalSocketLimit, GetCheckError(limiter, "https://secure_init"));
        EXPECT_EQ(kGlobalSocketLimit, GetCheckError(limiter, "https://secure_test"));
...
TEST(CurlConnectRateLimiter, AllLimits) {
    ...
    for (size_t i = 0; i < kRepetitions; ++i) {
        ...
        EXPECT_EQ(kPerHostSocketLimit, GetCheckError(limiter, "https://secure_first"));
        EXPECT_FALSE(GetCheckError(limiter, "https://secure_second"));
        EXPECT_FALSE(GetCheckError(limiter, "https://secure_third"));
        EXPECT_EQ(kPerHostSocketLimit, GetCheckError(limiter, "https://secure_second"));
        EXPECT_EQ(kPerHostSocketLimit, GetCheckError(limiter, "https://secure_third"));
        EXPECT_EQ(kPerHostSocketLimit, GetCheckError(limiter, "http://secure_first"));
        EXPECT_EQ(kPerHostSocketLimit, GetCheckError(limiter, "http://secure_second"));
        EXPECT_EQ(kPerHostSocketLimit, GetCheckError(limiter, "http://secure_third"));
...
TEST(CurlConnectRateLimiter, Multithread) {
    ...
    curl::ConnectRateLimiter limiter;
    limiter.SetGlobalHttpLimits(kPerHostLimit * kThreadsCount, std::chrono::seconds{1});
