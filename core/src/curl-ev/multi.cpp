...
void multi::CheckRateLimit(const char* url_str, std::error_code& ec) { connect_rate_limiter_->Check(url_str, ec); }

void multi::SetMultiplexingEnabled(bool value) { SetOptionAsync(native::CURLMOPT_PIPELINING, value ? 1 : 0); }

void multi::SetMaxHostConnections(long value) { SetOptionAsync(native::CURLMOPT_MAX_HOST_CONNECTIONS, value); }
...
void multi::start_read_op(socket_info* si) {
    ...
    si->watcher.ReadAsync([this, si](std::error_code err) { handle_socket_read(err, si); });
}
...
void multi::start_write_op(socket_info* si) {
    ...
    si->watcher.WriteAsync([this, si](std::error_code err) { handle_socket_write(err, si); });
}
...
int multi::socket(native::CURL*, native::curl_socket_t s, int what, void* userp, void* socketp) noexcept {
    auto* self = static_cast<multi*>(userp);
    LOG_TRACE() << "socket multi=" << self << " what=" << what << " si=" << socketp;
...
int multi::timer(native::CURLM*, [[maybe_unused]] long timeout_ms, void* userp) noexcept {
    ...
    if (timeout_ms > 0) {
        pimpl->timer_.SingleshotAsync(std::chrono::milliseconds(timeout_ms), [self](std::error_code err) {
