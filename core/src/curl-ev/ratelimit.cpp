...
void ConnectRateLimiter::SetGlobalHttpLimits(size_t limit, utils::TokenBucket::Duration rate) {
    global_http_.SetMaxSize(limit);
    global_http_.SetRefillPolicy({1, rate});
}

void ConnectRateLimiter::SetGlobalHttpsLimits(size_t limit, utils::TokenBucket::Duration rate) {
    global_https_.SetMaxSize(limit);
    global_https_.SetRefillPolicy({1, rate});
}

void ConnectRateLimiter::SetPerHostLimits(size_t limit, utils::TokenBucket::Duration rate) {
...
void ConnectRateLimiter::Check(const char* url_str, std::error_code& ec) {
    ...
    if (factors.host_ptr) {
        const auto current_per_host_limit = per_host_limit_.load(std::memory_order_relaxed);
        const auto current_per_host_rate = per_host_rate_.load(std::memory_order_relaxed);
