...
void Controller::Start() {
    if (config_.enabled) {
        LOG_INFO() << fmt::format("Congestion controller {} has started", name_) << LogFakeMode();
        periodic_.Start("congestion_control", {kStepPeriod}, [this] { Step(); });
    } else {
        LOG_INFO() << fmt::format("Congestion controller {} is disabled via static config, not starting", name_);
    }
}
