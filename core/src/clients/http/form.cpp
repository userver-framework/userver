...
void Form::AddContent(std::string_view key, std::string_view content) { impl_->add_content(key, content); }

void Form::AddContent(std::string_view key, std::string_view content, utils::zstring_view content_type) {
    impl_->add_content(key, content, content_type);
}

void Form::AddBuffer(std::string_view key, utils::zstring_view file_name, const std::shared_ptr<std::string>& buffer) {
