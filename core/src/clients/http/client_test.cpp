...
public:
    ...
    using TwoArgsFunction = std::function<Request&(Request&, const std::string& url, std::string data)>;
    ...
    auto PrepareRequest(const std::string& url, const Callback& callback, clients::http::Request request) const {
        ...
        *callback.data = data_;
        ...
        return request.verify(true).http_version(USERVER_NAMESPACE::http::HttpVersion::k11).timeout(kTimeout);
    }
...
struct ValidatingSharedCallback {
    const std::shared_ptr<std::string> method_name = std::make_shared<std::string>();
    ...
    HttpResponse operator()(const HttpRequest& request) const {
        ...
        EXPECT_LT(request.find(*method_name), first_line_end)
            << "No '" << *method_name << "' in first line of a request: " << request;

        const auto header_end = request.find("\r\n\r\n", first_line_end) + 4;
        EXPECT_EQ(request.substr(header_end), *data)
            << "Request body differ from '" << *data << "'. Whole request: " << request;
...
std::string TryGetHeader(const HttpRequest& request, std::string_view header) {
    ...
    std::string header_value(request.data() + values_begin_pos, values_end_pos - values_begin_pos);
...
struct Response301WithHeader {
    ...
    HttpResponse operator()(const HttpRequest&) const {
        return {
            fmt::format(kResponse301WithHeaderPattern, "Location: " + location + "\r\n" + header),
            HttpResponse::kWriteAndClose,
        };
    }
};
...
struct CheckCookie {
    ...
    HttpResponse operator()(const HttpRequest& request) {
        ...
        for (auto cookie : received_cookies) {
            boost::trim(cookie);
            EXPECT_TRUE(unseen_cookies.erase(cookie)) << "Unexpected cookie '" << cookie << "' in request: " << request;
        }
...
auto LogLevelScope(logging::Level level) { return logging::DefaultLoggerLevelScope{level}; }
...
UTEST(HttpClient, CancelRetries) {
    ...
    auto callback = [&server_requests, &enough_retries_event](const HttpRequest& request) {
        ++server_requests;
    ...
    const auto cancellation_duration = cancellation_end_time - cancellation_start_time;
    ...
    const auto future_destruction_duration = future_destruction_time - cancellation_end_time;
    EXPECT_LT(future_destruction_duration, kMaxNonIoReactionTime)
        << "Looks like cancel did not cancelled the request, because after the "
           "cancel future has been destructing for "
        << duration_cast<milliseconds>(future_destruction_duration).count() << "ms";
...
UTEST(HttpClient, MethodsMix) {
    ...
    const RequestMethodTestData tests[] = {
        ...
        {"DELETE",
         "",
         [](Request& request, const std::string& url, std::string data) -> Request& {
             return request.delete_method(url, data);
         }},
        {"PATCH",
         kTestData,
         [](Request& request, const std::string& url, std::string data) -> Request& {
...
UTEST(HttpClient, MethodsMixReuseRequest) {
    ...
    const RequestMethodTestData tests[] = {
        {"PUT", "", [](Request& request, const std::string& url) -> Request& { return request.put(url); }},
        {"POST", "", [](Request& request, const std::string& url) -> Request& { return request.post(url); }},
        {"GET", "", [](Request& request, const std::string& url) -> Request& { return request.get(url); }},
        {"HEAD", "", [](Request& request, const std::string& url) -> Request& { return request.head(url); }},
        {"DELETE", "", [](Request& request, const std::string& url) -> Request& { return request.delete_method(url); }},
        {"DELETE",
        ...
        {"PATCH", "", [](Request& request, const std::string& url) -> Request& { return request.patch(url); }},
    };

    for (const auto& method1 : tests) {
        for (const auto& method2 : tests) {
            auto request = method1.PrepareRequest(http_server.GetBaseUrl(), callback, http_client->CreateRequest());
    ...
    for (const auto& [method1, func1] : tests) {
        for (const auto& [method2, func2] : tests) {
            ...
            *callback.method_name = method2;
            ...
            EXPECT_TRUE(response2->IsOk()) << "Failed to perform " << method2 << " after " << method1;
        }
    }
}
...
UTEST(HttpClient, Cookies) {
    const auto test = [](const clients::http::Request::Cookies& cookies, std::set<std::string> expected) {
...
UTEST(HttpClient, DISABLED_IN_MAC_OS_TEST_NAME(HttpsWithCert)) {
    ...
    const auto ssl_url = http_server.GetBaseUrl(utest::SimpleServer::Schema::kHttps);
    ...
    for (unsigned i = 0; i < 2; ++i) {
        ...
        auto response =
            http_client_ptr->CreateRequest().post(url).timeout(kTimeout).client_key_cert(pkey, cert).perform();

        EXPECT_TRUE(response->IsOk());

        response = http_client_ptr->CreateRequest().post(url).timeout(kTimeout).perform();
...
UTEST(HttpClient, BadUrl) {
    ...
    UEXPECT_THROW(check("http:\\\\localhost/"), clients::http::BadArgumentException);
    ...
    UEXPECT_THROW(check("http://localhost:99999/"), clients::http::BadArgumentException);
    UEXPECT_THROW(check("http://localhost:abcd/"), clients::http::BadArgumentException);
}
...
UTEST(HttpClient, Retry) {
    ...
    auto response =
        http_client_ptr->CreateRequest().get(unavail_server.GetBaseUrl()).timeout(kTimeout).retry(3).perform();
...
UTEST(HttpClient, RequestReuseBasic) {
    ...
    for (unsigned i = 0; i < kFewRepetitions; ++i) {
        UEXPECT_NO_THROW(res = request.perform()) << "Probably your version of cURL is outdated";
...
UTEST(HttpClient, RequestReuseSample) {
    ...
    auto resp = sample::DifferentUrlsRetry(
        data,
        *http_client_ptr,
        kSmallTimeout,
        {
    ...
    resp = sample2::DifferentUrlsRetry(
        data,
        *http_client_ptr,
        kSmallTimeout,
        {
...
UTEST(HttpClient, CheckSchema) {
    ...
    UEXPECT_NO_THROW(http_client_ptr->CreateRequest().url("httpS://localhost"));
    UEXPECT_NO_THROW(http_client_ptr->CreateRequest().url("HTTP://LOCALHOST"));
    UEXPECT_THROW(http_client_ptr->CreateRequest().url("dict://localhost"), clients::http::BadArgumentException);
