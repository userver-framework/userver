...
std::exception_ptr PrepareEasyException(std::error_code ec, std::string_view url, const LocalStats& stats) {
    ...
    switch (static_cast<ErrorCode>(ec.value())) {
        case ErrorCode::kCouldNotResolveHost:
            return std::make_exception_ptr(DNSProblemException(ec, "DNS problem", url, stats));

        case ErrorCode::kOperationTimedout:
            return std::make_exception_ptr(TimeoutException(fmt::format("Timeout happened, url: {}", url), stats));
        ...
        case ErrorCode::kSslCrlBadfile:
            return std::make_exception_ptr(SSLException(ec, "SSL problem", url, stats));

        case ErrorCode::kBadFunctionArgument:
            return std::make_exception_ptr(BadArgumentException(ec, "Incorrect argument", url, stats));

        case ErrorCode::kTooManyRedirects:
            return std::make_exception_ptr(TooManyRedirectsException(ec, "Too many redirects", url, stats));

        case ErrorCode::kSendError:
        case ErrorCode::kRecvError:
        case ErrorCode::kCouldNotConnect:
            return std::make_exception_ptr(NetworkProblemException(ec, "Network problem", url, stats));

        case ErrorCode::kLoginDenied:
            return std::make_exception_ptr(AuthFailedException(ec, "Failed to login", url, stats));

        default:
            return std::make_exception_ptr(TechnicalError(ec, "Technical error", url, stats));
    }
}

std::exception_ptr PrepareRateLimitException(std::error_code ec, std::string_view url, const LocalStats& stats) {
    UASSERT(ec.category() == curl::errc::GetRateLimitCategory());

    return std::make_exception_ptr(NetworkProblemException(ec, "Rate limited", url, stats));
}
...
std::exception_ptr PrepareException(std::error_code ec, std::string_view url, const LocalStats& stats) {
    ...
    return std::make_exception_ptr(BaseCodeException(ec, "Unknown exception", url, stats));
}
