...
RequestStats::RequestStats(Statistics& stats)
    : stats_(&stats)
{
...
void RequestStats::FinishOk(int code, unsigned int attempts) noexcept {
    ...
    if (attempts > 1) {
        stats_->retries_ += utils::statistics::Rate{attempts - 1};
    }
    StoreTiming();
}

void RequestStats::FinishEc(std::error_code ec, unsigned int attempts) noexcept {
    UASSERT(stats_);
...
void RequestStats::StoreTimeToStart(std::chrono::microseconds micro_seconds) noexcept {
    UASSERT(stats_);
...
void Statistics::AccountError(ErrorGroup error) { ++error_count_[static_cast<int>(error)]; }
...
void DumpMetric(utils::statistics::Writer& writer, const DestinationStatisticsView& view) {
    ...
    for (std::size_t i = 0; i < Statistics::kErrorGroupCount; i++) {
        ...
        writer["errors"].ValueWithLabels(stats.error_count[i], {"http_error", Statistics::ToString(error_group)});
    }
...
void DumpMetric(utils::statistics::Writer& writer, const InstanceStatistics& stats) {
    ...
    writer["last-time-to-start-us"] = SumToMean(stats.last_time_to_start_us, stats.instances_aggregated);
    writer["event-loop-load"][utils::statistics::DurationToString(utils::statistics::kDefaultMaxPeriod
...
InstanceStatistics& InstanceStatistics::operator+=(const InstanceStatistics& stat) {
