...
UTEST(Rcu, NoCopy) {
    struct X {
        ...
        bool operator==(const X& other) const { return std::tie(x, y) == std::tie(other.x, other.y); }
...
UTEST(Rcu, AsyncGc) {
    ...
    rcu::Variable<utils::ScopeGuard> var([&] { EXPECT_FALSE(mutation_task.IsCurrent()); });
...
constexpr std::size_t kTotalTasks = kReadablePtrPingPongTasks + kReadingTasks + kWritingTasks + kSleeperTask;
