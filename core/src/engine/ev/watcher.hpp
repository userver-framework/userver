...
class Watcher final : public MultiShotAsyncPayload<Watcher<EvType>> {
 public:
  ...
  void Init(void (*cb)(struct ev_loop*, ev_io*, int) noexcept, int fd,
            int events) noexcept;  // TODO: use utils::Flags for events
  void Init(void (*cb)(struct ev_loop*, ev_timer*, int) noexcept,
            LibEvDuration after, LibEvDuration repeat) noexcept;
  ...
  template <typename T = EvType>
  std::enable_if_t<std::is_same_v<T, ev_timer>> Set(
      LibEvDuration after, LibEvDuration repeat) noexcept;
...
template <typename EvType>
template <typename Obj>
Watcher<EvType>::Watcher(const ThreadControl& thread_control,
                         Obj* data) noexcept
...
void Watcher<EvType>::Stop() noexcept {
  ...
  static_assert(noexcept(StopImpl()),
                "Stop() is called from destructor and it should be noexcept");
}
...
template <typename EvType>
template <typename Function>
void Watcher<EvType>::RunInBoundEvLoopSync(Function&& func) noexcept(
    noexcept(func())) {
