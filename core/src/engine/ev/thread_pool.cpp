...
ThreadPool::ThreadPool(ThreadPoolConfig config, bool use_ev_default_loop)
  ...
  default_controls_.controls = utils::GenerateFixedArray(
      threads_.size(),
      [this](std::size_t index) { return ThreadControl(threads_[index]); });

  timer_controls_.controls =
      utils::GenerateFixedArray(threads_.size(), [this](std::size_t index) {
...
TimerThreadControl& ThreadPool::NextTimerThread() {
  return timer_controls_.Next();
}

ThreadControl& ThreadPool::GetEvDefaultLoopThread() {
  UINVARIANT(!default_controls_.Empty() && use_ev_default_loop_,
