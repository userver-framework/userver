...
namespace engine::ev {
namespace {
[[maybe_unused]] bool IsFdValid(int fd) noexcept {
...
template <>
void Watcher<ev_io>::Init(void (*cb)(struct ev_loop*, ev_io*,
                                     int) noexcept) noexcept {
  Init(cb, -1, 0);
}
...
template <>
template <>
void Watcher<ev_timer>::Set(LibEvDuration after,
                            LibEvDuration repeat) noexcept {
  UASSERT_MSG(!is_running_, "Values in active watcher should not be changed.");
  // NOLINTNEXTLINE(cppcoreguidelines-pro-type-cstyle-cast)
