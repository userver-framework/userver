...
class StackConsumingScope final {
 public:
  ...
  std::uint32_t Get() const noexcept {
    return values_[utils::RandRange(kValuesCount)];
  }
...
UTEST_F(StackUsageMonitorTest, BacktraceLogging) {
  ...
  EXPECT_THAT(logged_string,
              testing::HasSubstr("Coroutine is using approximately"));
  ...
  EXPECT_THAT(logged_string,
              testing::HasSubstr("SelfLimitingRecursiveFunction"));
  // Name of the test
  EXPECT_THAT(
      logged_string,
      testing::HasSubstr("StackUsageMonitorTest_BacktraceLogging_Utest"));
