...
UTEST(Inotify, File) {
    ...
    {
        auto fd =
            fs::blocking::FileDescriptor::Open(path, {fs::blocking::OpenFlag::kRead, fs::blocking::OpenFlag::kWrite});
        fd.Write("1");
        fd.Seek(0);
        char buffer[1];
...
UTEST(Inotify, Directory) {
    ...
    auto event = inotify.Poll({});
    ASSERT_TRUE(event);
    EXPECT_EQ(event, (sys_linux::Event{path + "/2", sys_linux::EventType::kOpen}));

    event = inotify.Poll({});
    ASSERT_TRUE(event);
    EXPECT_EQ(event, (sys_linux::Event{path + "/2", sys_linux::EventType::kModify}));

    event = inotify.Poll({});
    ASSERT_TRUE(event);
    EXPECT_EQ(event, (sys_linux::Event{path + "/2", sys_linux::EventType::kCloseWrite}));
    ...
    event = inotify.Poll({});
    ASSERT_TRUE(event);
    EXPECT_EQ(event, (sys_linux::Event{path + "/2", sys_linux::EventType::kAccess}));

    event = inotify.Poll({});
    ASSERT_TRUE(event);
    EXPECT_EQ(event, (sys_linux::Event{path + "/2", sys_linux::EventType::kCloseNoWrite}));

    fs::blocking::Rename(path + "/2", path + "/3");

    event = inotify.Poll({});
    ASSERT_TRUE(event);
    EXPECT_EQ(event, (sys_linux::Event{path + "/2", sys_linux::EventType::kMovedFrom}));

    event = inotify.Poll({});
    ASSERT_TRUE(event);
    EXPECT_EQ(event, (sys_linux::Event{path + "/3", sys_linux::EventType::kMovedTo}));
}
