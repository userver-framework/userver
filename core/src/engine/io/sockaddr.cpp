...
Sockaddr Sockaddr::MakeUnixSocketAddress(std::string_view path) {
    ...
    if (path.size() > kMaxPathLength) {
        throw std::runtime_error(fmt::format("unix socket path is too long ({}), max-length={}", path, kMaxPathLength));
    }
    if (path.empty() || path[0] == '\0') {
        throw std::runtime_error("abstract sockets are not supported");
    }
    if (path[0] != '/') {
        throw std::runtime_error(fmt::format("unix socket path must be absolute ({})", path));
    }
...
std::uint16_t Sockaddr::Port() const {
    switch (Data()->sa_family) {
        ...
        default:
            throw AddrException(fmt::format("Cannot determine port for address family {}", Data()->sa_family));
    }
}
...
void Sockaddr::SetPort(std::uint16_t port) {
    ...
    switch (Data()->sa_family) {
        ...
        default:
            throw AddrException(
                fmt::format("Unable to set port {} for an unknown address family {}", port, Data()->sa_family)
            );
    }
}
    ...
    switch (Domain()) {
        case AddrDomain::kInet: {
            ...
            utils::CheckSyscallNotEqualsCustomException<IoSystemError>(
                ...
                nullptr,
                "converting IPv4 address to string"
            );
            return buf.data();
        } break;
        ...
        case AddrDomain::kInet6: {
            ...
            utils::CheckSyscallNotEqualsCustomException<IoSystemError>(
                ...
                nullptr,
                "converting IPv6 address to string"
            );
            return buf.data();
        } break;
...
logging::LogHelper& operator<<(logging::LogHelper& lh, const Sockaddr& sa) { return lh << fmt::to_string(sa); }
