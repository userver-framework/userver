...
bool GenericWaitList::GetSignalOrAppend(
    boost::intrusive_ptr<TaskContext>&& context) noexcept {
  return utils::Visit(
      waiters_,  //
      [&context](WaitListLight& ws) {
        return ws.GetSignalOrAppend(std::move(context));
      },
...
bool GenericWaitList::IsShared() const noexcept {
  return std::holds_alternative<WaitListAndSignal>(waiters_);
}
