...
UTEST(ConditionVariable, CancelledOnTaskDeadline) {
    const auto deadline = engine::Deadline::FromDuration(std::chrono::milliseconds{20});
    engine::Mutex mutex;
...
UTEST(ConditionVariable, AlreadyCancelled) {
    ...
    UEXPECT_NO_THROW(EXPECT_FALSE(cv.WaitFor(lock, utest::kMaxTestWaitTime, [] { return false; })));
}
