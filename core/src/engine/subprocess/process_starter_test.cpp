...
std::optional<std::string> GetEnvironmentInsideExec(
  ...
  auto stdout_file =
      fs::TempFile::Create(engine::current_task::GetTaskProcessor());

  engine::subprocess::EnvironmentVariablesScope scope{};
  SetEnvironmentVariable("PATH", kPath,
  ...
  auto status =
      starter.Exec("printenv", {variable_name}, std::move(options)).Get();
...
UTEST(Subprocess, EnvironmentVariablesScope) {
  ...
  {
    engine::subprocess::EnvironmentVariablesScope scope{};
    SetEnvironmentVariable(kEnvVariableName, kEnvVariableValue,
                           engine::subprocess::Overwrite::kAllowed);

    EXPECT_EQ(kEnvVariableValue,
              engine::subprocess::GetCurrentEnvironmentVariables().GetValue(
                  kEnvVariableName));

    ASSERT_EQ(kEnvVariableValue,
              GetEnvironmentInsideExec(starter, kEnvVariableName));
  }
