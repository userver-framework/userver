...
constexpr std::uint64_t kConsumersSleepingShift = static_cast<std::uint64_t>(1) << 32;
...
bool ConsumersState::TryIncrementStealersCount(const ConsumersState& old) noexcept {
    std::uint64_t old_state = old.state_.load();
    return state_.compare_exchange_strong(old_state, old_state + 1);
}

ConsumersState::State ConsumersState::Get() noexcept { return CreateState(state_.load()); }
...
void ConsumersState::IncrementSleepingCount() noexcept { state_.fetch_add(kConsumersSleepingShift); }

void ConsumersState::DecrementSleepingCount() noexcept { state_.fetch_sub(kConsumersSleepingShift); }
