...
SharedTask::SharedTask(const SharedTask& other) noexcept : TaskBase(static_cast<const TaskBase&>(other)) {
...
SharedTask::SharedTask(SharedTask&& other) noexcept : TaskBase(static_cast<TaskBase&&>(other)) {
...
void SharedTask::DecrementSharedUsages() noexcept {
    ...
    UASSERT_MSG(usages != std::numeric_limits<decltype(usages)>::max(), "Underflow of reference counter in SharedTask");
    if (usages == 0) {
...
void SharedTask::IncrementSharedUsages() noexcept {
    ...
    [[maybe_unused]] const auto usages = GetContext().IncrementFetchSharedTaskUsages();
    UASSERT_MSG(usages >= 2, "Copied SharedTask from a task that has no alive references to it");
}
