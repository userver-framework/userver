...
void TagScope::AddTag(std::string&& key, logging::LogExtra::ProtectedValue&& value) {
    ...
    if (it) {
        ...
        parent_.pimpl_->log_extra_inheritable_.extra_->emplace_back(std::move(key), std::move(value));
    }
}

TagScope::TagScope(std::string key, logging::LogExtra::Value value, logging::LogExtra::ExtendType extend_type)
    : tracing::TagScope(Span::CurrentSpan(), std::move(key), std::move(value), extend_type) {}
...
TagScope::TagScope(logging::LogExtra&& extra) : tracing::TagScope(Span::CurrentSpan(), std::move(extra)) {}
