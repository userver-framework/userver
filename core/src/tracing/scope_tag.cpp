...
tracing::impl::ScopeTagsImpl::GetPreviousValue(Span& parent, std::string_view key) {
  auto* node = parent.pimpl_->log_extra_inheritable_.Find(key);
  std::optional ret = (node == nullptr) ? std::nullopt : std::make_optional(node->second);
  return ret;
}
...
tracing::impl::ScopeTagsImpl::~ScopeTagsImpl() {
  ...
  if (my_node == nullptr || my_node->second.GetValue() != my_value_){
    // value was overwritten by span::AddTag(key, new_value), or another (Frozen)ScopeTag, or initially we were trying to overwrite frozen value.
    // It is logical to not overwrite new_value here
    return;
  }
  if (IsRoot()) {
    const std::ptrdiff_t idx = my_node - parent_.pimpl_->log_extra_inheritable_.extra_->data();
...
bool tracing::impl::ScopeTagsImpl::IsRoot() const noexcept { return !previous_value_.has_value(); }
void tracing::impl::ScopeTagsImpl::AddTag() {
  parent_.AddTag(key_, my_value_);
}
...
tracing::FrozenScopeTag::FrozenScopeTag(
    std::string key, userver::logging::LogExtra::Value value)
    : impl_(Span::CurrentSpan(), std::move(key), std::move(value))
{
  impl_.AddTagFrozen();
}
