...
StorageData::DoUpdateAndListen(concurrent::FunctionId id, std::string_view name, SnapshotChannel::Function&& func) {
    ...
    return snapshot_channel_.DoUpdateAndListen(id, name, std::move(func), std::move(updater));
}

concurrent::AsyncEventSubscriberScope
StorageData::DoUpdateAndListen(concurrent::FunctionId id, std::string_view name, DiffChannel::Function&& func) {
