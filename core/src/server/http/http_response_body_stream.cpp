...
ResponseBodyStream::~ResponseBodyStream() {
  if (http_response_.GetStreamId().has_value()) {
    ...
    std::get<impl::Http2StreamEventProducer>(queue_producer_)
        .CloseStream(*http_response_.GetStreamId());
  }
}
...
void ResponseBodyStream::PushBodyChunk(std::string&& chunk,
  ...
  std::visit(
      utils::Overloaded{
          [&chunk,
           &deadline](HttpResponse::Queue::Producer& queue_producer) mutable {
            const bool success =
                queue_producer.Push(std::move(chunk), deadline);
            UASSERT(success);
          },
          [this, &chunk,
           &deadline](impl::Http2StreamEventProducer& queue_producer) mutable {
            UASSERT(http_response_.GetStreamId().has_value());
            queue_producer.PushEvent(
                {*http_response_.GetStreamId(), std::move(chunk)}, deadline);
          },
