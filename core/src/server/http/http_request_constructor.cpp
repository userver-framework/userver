...
struct HttpRequestConstructor::LegacyHttpParserUrlImpl final {
...
void HttpRequestConstructor::ParseUrl() {
  ...
  if (http_parser_parse_url(request_->url_.data(), request_->url_.size(),
                            ...
                            &legacy_parsed_url_->parsed_url)) {
    SetStatus(Status::kParseUrlError);
    throw std::runtime_error("error in http_parser_parse_url() for url '" +
                             request_->url_ + '\'');
  }

  if (legacy_parsed_url_->parsed_url.field_set & (1 << http_parser_url_fields::UF_PATH)) {
    const auto& str_info =
        legacy_parsed_url_->parsed_url.field_data[http_parser_url_fields::UF_PATH];
...
void HttpRequestConstructor::ParseArgs(const LegacyHttpParserUrlImpl& url) {
