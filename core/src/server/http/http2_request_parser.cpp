...
void Http2RequestParser::Sumbit200Response() {
  ...
  int rv = nghttp2_submit_response(session_.get(), 1, hdrs.data(), hdrs.size(),
                                   nullptr);
  UASSERT(!rv);
}
...
int Http2RequestParser::OnFrameRecv(nghttp2_session* session,
  ...
  if (frame->hd.type == NGHTTP2_WINDOW_UPDATE) {
    LOG_ERROR() << std::to_string(frame->window_update.window_size_increment);
    LOG_ERROR() << std::to_string(frame->window_update.reserved);
    LOG_ERROR() << std::to_string(frame->hd.stream_id);
    LOG_ERROR() << "NGHTTP2_WINDOW_UPDATE";
  }
  if (frame->hd.type == NGHTTP2_SETTINGS) {
    LOG_ERROR() << std::to_string(frame->settings.niv);
...
bool Http2RequestParser::DoUpgrade(std::string_view data) {
  if (auto pos = data.find(kHttp2SettingsHeader);
    ...
    StreamData s{request_constructor_config_, handler_info_index_,
                 data_accounter_, 1};
    s.constructor.SetUpgradeHttpResponse(
        std::string{kSwitchingProtocolResponse} + WantWrite());
