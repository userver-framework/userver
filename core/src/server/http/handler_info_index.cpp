...
class HandlerInfoIndex::HandlerInfoIndexImpl final {
    using FallbackHandlersStorage = std::array<std::optional<HandlerInfo>, handlers::kFallbackHandlerMax + 1>;
...
public:
    ...
    void SetFallbackHandler(const handlers::HttpHandlerBase& handler, engine::TaskProcessor& task_processor);
...
void HandlerInfoIndex::HandlerInfoIndexImpl::AddHandler(
    ...
    if (!impl::HasWildcardSpecificSymbols(path) && (path.empty() || path.back() != '*')) {
...
void HandlerInfoIndex::AddHandler(const handlers::HttpHandlerBase& handler, engine::TaskProcessor& task_processor) {
    std::visit(
        utils::Overloaded{
...
MatchRequestResult HandlerInfoIndex::MatchRequest(HttpMethod method, const std::string& path) const {
    return impl_->MatchRequest(method, path);
}

const HandlerInfo* HandlerInfoIndex::GetFallbackHandler(handlers::FallbackHandler fallback) const {
