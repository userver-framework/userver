      ...
      streaming_consumer_(streaming_queue_->GetConsumer())
{
    ...
    // Not auto-resetting, so that the connection loop can await it directly.
...
void Http2Session::HandleStreamingEvents() {
    // Reset before draining, so that an event pushed while we drain still wakes us up
    // again instead of being silently swallowed.
    streaming_event_.Reset();
    ...
    while (streaming_consumer_.PopNoblock(event)) {
        ...
        if (stream == nullptr) {
            // The peer closed the stream while its producer was still writing.
            LOG_LIMITED_DEBUG() << fmt::format("Dropping a streamed chunk of the closed stream {}", event.stream_id);
            event = {};
            continue;
        }
        if (stream->IsDeferred()) {
