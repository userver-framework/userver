...
[[nodiscard]] std::string_view LtrimOws(std::string_view str) {
    ...
    str.remove_prefix(first_pchar_pos == std::string_view::npos ? str.size() : first_pchar_pos);
...
[[nodiscard]] std::string_view RtrimOws(std::string_view str) {
    ...
    str.remove_suffix(str.size() - (last_pchar_pos == std::string_view::npos ? 0 : last_pchar_pos + 1));

    return str;
}

[[nodiscard]] std::string_view TrimOws(std::string_view str) { return RtrimOws(LtrimOws(str)); }

[[nodiscard]] bool SkipSymbol(std::string_view& str, char ch) {
    if (str.empty() || str.front() != ch) {
        LOG_WARNING() << "Symbol #" << int{ch} << " (" << ch << ") expected, but not found in request body";
        return false;
    }
    str.remove_prefix(1);
    return true;
...
bool ParseContentDisposition(std::string_view content_disposition, FormDataArgInfo& arg_info) {
    ...
    if (!IEquals(value, kFormData)) {
        LOG_WARNING() << "Bad Content-Disposition: '" << kFormData << "' expected '" << value << "' found";
        return false;
    }
    SkipOptionalSpaces(str);
    while (!str.empty()) {
...
size_t FindBoundaryEnd(std::string_view body, std::string_view boundary, std::string_view crlf) {
    size_t pos = 0;
    while (pos < body.size()) {
        if (body[pos] == crlf.front()) {
            if (pos + crlf.size() < body.size() && body.substr(pos, crlf.size()) == crlf) {
                pos += crlf.size();
...
bool ParseMultipartFormDataValue(
    ...
    if (arg_info.name == kCharset) {
        ...
        LOG_TRACE() << "add arg_info.name=" << arg_info.name << ", arg_info.arg=" << arg_info.arg.ToDebugString();
...
bool ParseMultipartFormDataBody(
    std::string_view body,
    std::string_view boundary,
        ...
        if (body.front() == '-') {
            ...
            for (auto& [name, args] : form_data_args) {
                ...
                for (auto& arg : args) {
                    ...
                    LOG_TRACE() << "form_data arg, name: " << name << ", arg: " << arg.ToDebugString();
                }
            }
            return true;
        }
...
bool ParseMultipartFormData(
    ...
    return ParseMultipartFormDataBody(body, boundary, std::move(charset), form_data_args, strict_cr_lf);
}
