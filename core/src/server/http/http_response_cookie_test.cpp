...
TEST(HttpCookie, FromString) {
    ...
    const std::string expected =
        "name1=value1; Domain=domain.com; Path=/; Expires=Wed, 12 Jun 2019 "
        "16:51:45 GMT; Max-Age=3600; Secure; SameSite=None; HttpOnly";
    EXPECT_EQ(server::http::Cookie::FromString(obsolete_time_format_cookie_as_str).value().ToString(), expected);
...
TEST(HttpCookie, AppendToString) {
    ...
    utils::SmallString<http::headers::kTypicalHeadersSize> str{"*Previous string content *"};
