...
void OutputHeader(std::string& header, std::string_view key, std::string_view val) {
    ...
    header.resize(old_size + key.size() + kKeyValueHeaderSeparator.size() + val.size() + kCrlf.size());
...
void http_headers_serialization_inplace(benchmark::State& state) {
    for ([[maybe_unused]] auto _ : state) {
        ...
        os.resize_and_overwrite(USERVER_NAMESPACE::http::headers::kTypicalHeadersSize, [&](char* data, std::size_t) {
...
void http_headers_serialization_no_ostreams(benchmark::State& state) {
    for ([[maybe_unused]] auto _ : state) {
        ...
        if (kHeaders.find(USERVER_NAMESPACE::http::headers::kContentLength) == kHeaders.end()) {
            OutputHeader(os, USERVER_NAMESPACE::http::headers::kContentLength, fmt::format(FMT_COMPILE("{}"), 1024));
        }
...
void http_headers_serialization_ostreams(benchmark::State& state) {
    for ([[maybe_unused]] auto _ : state) {
        ...
        os << "HTTP/" << 1 << "." << 1 << " " << 200 << StatusCodeString(server::http::HttpStatus::kOk) << "\r\n";
        ...
        if (kHeaders.find(USERVER_NAMESPACE::http::headers::kContentLength) == kHeaders.end()) {
            os << std::string_view{USERVER_NAMESPACE::http::headers::kContentLength} << ": " << 1024 << "\r\n";
        }
...
void HttpResponseSetHeaderBenchmark(benchmark::State& state) {
    ...
    for ([[maybe_unused]] auto _ : state) {
        ...
        response.SetHeader(Headers::kServer, "userver/1.0.0 (20201109134848; rv:e20945c83fd)");
        response.SetHeader(Headers::kXYaRequestId, "a3dd1efa7bf04e62902ba3283d24124f");
        response.SetHeader(Headers::kXYaTraceId, "66971057871746f6808c8c62a68b28c4");
