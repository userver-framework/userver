...
void Http2Connection::Shutdown() noexcept {
    LOG_TRACE()
...
void Http2Connection::ListenForRequests() noexcept {
...
bool Http2Connection::WaitOnSocket(engine::Deadline deadline) {
    if (!peer_socket_) {
        return false;
    }
...
void Http2Connection::ProcessRequest(std::shared_ptr<http::HttpRequest>&& request_ptr) {
...
engine::TaskWithResult<void> Http2Connection::HandleQueueItem(const std::shared_ptr<http::HttpRequest>& request
) noexcept {
...
void Http2Connection::SendResponse(http::HttpRequest& request) {
    auto& response = request.GetHttpResponse();
...
std::string Http2Connection::Getpeername() const { return peer_name_; }

std::unique_ptr<request::RequestParser> Http2Connection::MakeParser(USERVER_NAMESPACE::http::HttpVersion ver) {
...
bool Http2Connection::TryDetectHttpVersion(std::string& buffer, std::string_view req) {
    using HttpVersion = USERVER_NAMESPACE::http::HttpVersion;
