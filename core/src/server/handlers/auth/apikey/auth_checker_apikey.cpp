...
const ApiKeysSet& GetApiKeysByType(const AuthCheckerSettings& settings, const std::string& apikey_type) {
    ...
    if (it == apikeys_map->end()) {
        throw std::runtime_error(
            "apikey_type '" + apikey_type + "' not found in apikeys settings (possible missing value in secdist)"
        );
    }
    return it->second;
}
...
AuthCheckerApiKey::AuthCheckerApiKey(const HandlerAuthConfig& auth_config, const AuthCheckerSettings& settings) {
...
[[nodiscard]] AuthCheckResult AuthCheckerApiKey::CheckAuth(const http::HttpRequest& request, request::RequestContext&)
    const {
    ...
    if (IsApiKeyAllowed(request_apikey, *allowed_keys)) {
        return AuthCheckResult{AuthCheckResult::Status::kOk, std::string{"IsApiKeyAllowed: OK"}};
    }
    LOG_WARNING() << "access is not allowed with apikey from " << USERVER_NAMESPACE::http::headers::kApiKey;
...
Parse(const yaml_config::YamlConfig& value, formats::parse::To<AuthCheckerApiKey::ApiKeyTypeByMethodSettings>) {
    AuthCheckerApiKey::ApiKeyTypeByMethodSettings settings;
    for (auto method : http::kHandlerMethods) {
        settings.apikey_type[static_cast<int>(method)] = value[http::ToString(method)].As<std::optional<std::string>>();
    }
    return settings;
}
