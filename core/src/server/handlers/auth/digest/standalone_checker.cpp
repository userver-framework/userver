...
std::optional<UserData> AuthStandaloneCheckerBase::FetchUserData(const std::string& username) const {
    if (username.empty()) {
        return std::nullopt;
    }
...
std::optional<TimePoint> AuthStandaloneCheckerBase::GetUnnamedNonceCreationTime(const std::string& nonce) const {
