...
std::string LogLevel::ProcessPut(const http::HttpRequest& request, request::RequestContext&) const {
    const std::string& logger_name = request.GetArg(kLogger);
    const logging::LoggerPtr logger = logger_name.empty() ? nullptr : logging_component_.GetLogger(logger_name);
    auto data = data_.Lock();
