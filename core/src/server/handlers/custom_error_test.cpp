...
TEST(CustomHandlerException, MappedCodes) {
    EXPECT_EQ(http::HttpStatus::kBadRequest, http::GetHttpStatus(HandlerErrorCode::kClientError));
    EXPECT_EQ(http::HttpStatus::kBadRequest, http::GetHttpStatus(HandlerErrorCode::kRequestParseError));
    EXPECT_EQ(http::HttpStatus::kUnauthorized, http::GetHttpStatus(HandlerErrorCode::kUnauthorized));
    EXPECT_EQ(http::HttpStatus::kMethodNotAllowed, http::GetHttpStatus(HandlerErrorCode::kInvalidUsage));
    EXPECT_EQ(http::HttpStatus::kNotFound, http::GetHttpStatus(HandlerErrorCode::kResourceNotFound));
    EXPECT_EQ(http::HttpStatus::kConflict, http::GetHttpStatus(HandlerErrorCode::kConflictState));
    EXPECT_EQ(http::HttpStatus::kPayloadTooLarge, http::GetHttpStatus(HandlerErrorCode::kPayloadTooLarge));
    EXPECT_EQ(http::HttpStatus::kTooManyRequests, http::GetHttpStatus(HandlerErrorCode::kTooManyRequests));

    EXPECT_EQ(http::HttpStatus::kInternalServerError, http::GetHttpStatus(HandlerErrorCode::kServerSideError));
    EXPECT_EQ(http::HttpStatus::kBadGateway, http::GetHttpStatus(HandlerErrorCode::kBadGateway));
}
...
using ErrorTypes = ::testing::Types<ClientError, Unauthorized, ResourceNotFound, ConflictError, InternalServerError>;
...
TYPED_TEST(CustomHandlerExceptionTest, OverrideMessage) {
    ...
    constexpr auto kDefaultCode = TestFixture::kDefaultCode;
    try {
        throw ErrorType(InternalMessage{"Something went wrong"}, ExternalBody{"Tell everyone about it"});
    } catch (const ErrorType& e) {
        EXPECT_EQ(kDefaultCode, e.GetCode());
...
TYPED_TEST(CustomHandlerExceptionTest, ServiceCode) {
    ...
    try {
        throw ErrorType(CustomMessageWithServiceCode{"custom_code", "custom_message"});
    } catch (const ErrorType& e) {
        EXPECT_EQ(kDefaultCode, e.GetCode());
