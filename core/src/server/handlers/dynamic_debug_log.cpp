...
std::string ProcessPut(const http::HttpRequest& request, request::RequestContext&) {
...
std::string ProcessDelete(const http::HttpRequest& request, request::RequestContext&) {
    const auto& location = request.GetArg("location");
    auto [path, line] = logging::SplitLocation(location);
