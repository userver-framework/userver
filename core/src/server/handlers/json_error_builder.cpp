...
JsonErrorBuilder::JsonErrorBuilder(const CustomHandlerException& ex)
    : JsonErrorBuilder(
          ex.GetServiceCode().empty() ? GetFallbackServiceCode(ex.GetCode()) : ex.GetServiceCode(),
          ex.what(),
          ex.GetExternalErrorBody().empty() ? GetCodeDescription(ex.GetCode()) : ex.GetExternalErrorBody(),
          ex.GetDetails()
      )
{}
...
{
    UASSERT_MSG(!error_code.empty(), "Service-specific error code must be provided");
