...
constexpr std::memory_order ToInternalMemoryOrder(std::memory_order order) noexcept { return order; }
...
constexpr boost::memory_order ToInternalMemoryOrder(std::memory_order order) noexcept {
    switch (order) {
...
public:
    ...
    USERVER_IMPL_PROTECT_DWCAS_ATTR T load() const noexcept {
        return static_cast<AtomicRef>(impl_).load(impl::ToInternalMemoryOrder(Order));
    }

    template <std::memory_order Order>
    USERVER_IMPL_PROTECT_DWCAS_ATTR T exchange(T desired) noexcept {
        return static_cast<AtomicRef>(impl_).exchange(desired, impl::ToInternalMemoryOrder(Order));
    }
