...
TEST(ConflatedEventChannel, Publish) {
  RunInCoro([]() {
...
TEST(ConflatedEventChannel, PublishMany) {
  RunInCoro([]() {
    concurrent::AsyncEventChannel channel("channel");
    concurrent::ConflatedEventChannel conflated_channel("conflated_channel");
    conflated_channel.AddChannel(channel);
...
TEST(ConflatedEventChannel, PublishFromManyChannels) {
  RunInCoro([]() {
...
TEST(ConflatedEventChannel, PublishForManyListeners) {
  RunInCoro([]() {
    concurrent::AsyncEventChannel channel("channel");
    concurrent::ConflatedEventChannel conflated_channel("conflated_channel");
    conflated_channel.AddChannel(channel);
    ...
    auto subscription1 = conflated_channel.AddListener(&subscriber1, "sub1",
                                                       &Subscriber::OnEvent);
    auto subscription2 = conflated_channel.AddListener(&subscriber2, "sub2",
                                                       &Subscriber::OnEvent);
