...
struct ValueHelper<std::unique_ptr<int>> {
    ...
    static bool HasMemoryLeakCheck() { return false; }
    static bool CheckMemoryOk() { return true; }
    static bool CheckWasNotMovedOut(const std::unique_ptr<int>& ptr) { return !!ptr; }
};

template <>
struct ValueHelper<std::unique_ptr<RefCountData>> {
    static auto Wrap(int tag) { return std::make_unique<RefCountData>(tag); }
    static auto Unwrap(const std::unique_ptr<RefCountData>& data) { return data->val; }
    // Checks that no object was leaked
    static bool CheckMemoryOk() { return RefCountData::objects_count.load() == 0; }
    static bool HasMemoryLeakCheck() { return true; }
    static bool CheckWasNotMovedOut(const std::unique_ptr<RefCountData>& ptr) { return !!ptr; }
};
...
protected:
    ...
    int Unwrap(const ValueType& object) { return ValueHelper<ValueType>::Unwrap(object); }
    // Some types may implement tracking to detect memory leaks. ValueType this
    // method queries whether everything is OK or not.
    static bool CheckMemoryOk() { return ValueHelper<ValueType>::CheckMemoryOk(); }
    static bool HasMemoryLeakCheck() { return ValueHelper<ValueType>::HasMemoryLeakCheck(); }
...
TYPED_UTEST_P(TypedQueueFixture, Block) {
    ...
    auto consumer_task = utils::Async("consumer", [consumer = queue->GetConsumer(), this]() {
        typename TypeParam::ValueType value{};
        EXPECT_TRUE(consumer.Pop(value));
...
TYPED_UTEST_P(QueueFixture, BlockConsumerWithProducer) {
    ...
    int value{0};
    auto consumer_task = utils::Async("consumer", [&] { ASSERT_TRUE(consumer.Pop(value)); });
...
TYPED_UTEST_P_MT(QueueFixture, ManyProducers, 4) {
    ...
    for (std::size_t i = 0; i < kProducersCount; ++i) {
        tasks.push_back(utils::Async("pusher", [&producer = producers[i], i] {
            for (std::size_t message = kMessageCount * i; message < (i + 1) * kMessageCount; ++message) {
