...
std::shared_ptr<SimpleCache> CreateSimpleCachePtr() { return std::make_shared<SimpleCache>(1, 1); }
...
UTEST(ExpirableLruCache, HitOptionalUnexpirableWithUpdate) {
    ...
    EXPECT_EQ(1, cache.GetOptionalUnexpirableWithUpdate(key, UpdateValue(counter, 2)));

    EngineYield();

    EXPECT_EQ(2, cache.GetOptionalUnexpirableWithUpdate(key, UpdateValue(counter, 2)));
}
