...
formats::json::Value DoLoadFromFile(const std::string& path, SecdistFormat format, bool missing_ok) {
    ...
    try {
        ...
        if (missing_ok) {
            LOG_WARNING() << "Failed to load secdist from file '" << path << "': " << e << ", booting without secdist";
        } else {
            throw SecdistError(
...
void UpdateFromEnv(formats::json::Value& doc, const std::optional<std::string>& environment_secrets_key) {
    ...
    if (value) {
        ...
        try {
            ...
            throw SecdistError("Can't parse '" + *environment_secrets_key + "' env variable: " + ex.what());
        }
