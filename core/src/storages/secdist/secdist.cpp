...
std::vector<std::function<std::any(const formats::json::Value&)>>& GetConfigFactories() {
    static std::vector<std::function<std::any(const formats::json::Value&)>> factories;
    return factories;
}
    ...
    for (const auto& config_factory : GetConfigFactories()) {
        try {
            ...
            const std::string_view missing_comment =
                doc.IsMissing() || doc.IsEmpty() ? " (which is missing! see WARNING logs above for details)" : "";
            throw std::runtime_error(
                fmt::format("Failed to parse secdist config from JSON{}: {}", missing_comment, ex.what())
            );
        }
    }
}

std::size_t SecdistConfig::Register(std::function<std::any(const formats::json::Value&)>&& factory) {
    auto& config_factories = GetConfigFactories();
    config_factories.emplace_back(std::move(factory));
    return config_factories.size() - 1;
}

const std::any& SecdistConfig::Get(const std::type_index& type, std::size_t index) const {
    try {
        return configs_.at(index);
    } catch (const std::out_of_range&) {
        throw std::out_of_range("Type " + compiler::GetTypeName(type) + " is not registered as config");
    }
}
