...
bool operator==(const DynamicDebugConfig& a, const DynamicDebugConfig& b) {
    return a.force_enabled == b.force_enabled && a.force_disabled == b.force_disabled;
}

DynamicDebugConfig Parse(const formats::json::Value& value, formats::parse::To<DynamicDebugConfig>) {
    DynamicDebugConfig result;

    result.force_enabled = value["force-enabled-level"].As<std::unordered_map<std::string, logging::Level>>({});
    result.force_disabled = value["force-disabled-level"].As<std::unordered_map<std::string, logging::Level>>({});
