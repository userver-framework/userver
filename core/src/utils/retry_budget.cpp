...
void DumpMetric(statistics::Writer& writer, const RetryBudget& budget) {
    ...
    writer["approx_token_count"] = budget.token_count_.load(std::memory_order_relaxed) / kMillis;
    writer["max_token_count"] = budget.max_tokens_.load(std::memory_order_relaxed) / kMillis;
}
