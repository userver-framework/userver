...
void CpuRelax::Relax() {
    ...
    if (++iterations_ == every_iterations_) {
        iterations_ = 0;
        pause_.Pause();
        LOG_TRACE() << fmt::format("CPU relax: yielding after {} iterations", every_iterations_);
...
std::uint64_t StreamingCpuRelax::GetBytesProcessed() const { return total_bytes_ + bytes_since_last_time_check_; }
