...
static_assert(kMaxBPlusBounds >= 50, "B+ tree should fit the largest recommended histogram size");
...
void Histogram::UpdateBounds() {
    ...
    const auto largest_bound = upper_bounds.empty() ? 0.0f : upper_bounds.begin()[upper_bounds.size() - 1];
...
void Histogram::Account(double value, std::uint64_t count) noexcept {
    ...
    for (std::size_t i = 0; i < kBlockLayers; ++i) {
        block_index = 1 + block_index * kBlockWays + LeastGreaterEqualIndex(bounds_[block_index], value);
    }
...
HistogramView Histogram::GetView() const& noexcept { return impl::histogram::MakeView(buckets_.get()); }

void DumpMetric(Writer& writer, const Histogram& histogram) { writer = histogram.GetView(); }
