...
void ParseProcStat(std::string_view data, SystemStats& stats) {
    ...
    while (pos < data.size()) {
        ...
        const auto get_current_value = [=] {
            return utils::FromString<int64_t>(std::string{data.substr(pos, next_delim_pos - pos)});
        };
...
void ParseProcStatIo(std::string_view data, SystemStats& stats) {
    ...
    while (pos < data.size()) {
        ...
        const auto parse_if_matches = [=](std::optional<std::int64_t>& field, std::string_view header) {
            ...
            field = utils::FromString<int64_t>(std::string{data.substr(value_pos, value_len)});
        };
...
SystemStats GetSystemStatisticsByProcPath(std::string_view path) {
    SystemStats stats;
    try {
        ParseProcStat(fs::blocking::ReadFileContents(fmt::format("{}/stat", path)), stats);
    ...
    try {
        ParseProcStatIo(fs::blocking::ReadFileContents(fmt::format("{}/io", path)), stats);
    } catch (const std::exception& ex) {
        LOG_LIMITED_DEBUG() << "Could not get I/O stats from " << path << ": " << ex;
    }
...
SystemStats GetSystemStatisticsByExeNameFromProc(std::string_view name) {
    ...
    if (ec) {
        LOG_LIMITED_DEBUG() << "Could not collect stats for " << name << ": " << ec.message();
        return {};
    }
    ...
    for (; !ec && it != boost::filesystem::directory_iterator{}; it.increment(ec)) {
        ...
        try {
            stat_data = fs::blocking::ReadFileContents((it->path() / "stat").native());
...
utils::statistics::impl::SystemStats GetSelfSystemStatisticsFromKernel() {
...

    {
        ...
        if (::getrusage(RUSAGE_SELF, &rusage) != -1) {
            ...
            stats.cpu_time_sec = rusage.ru_utime.tv_sec + rusage.ru_utime.tv_usec * 1e-6;
