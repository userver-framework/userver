...
HistogramAggregator::HistogramAggregator(HistogramAggregator&&) noexcept = default;

HistogramAggregator& HistogramAggregator::operator=(HistogramAggregator&&) noexcept = default;
...
void HistogramAggregator::AccountAt(std::size_t bucket_index, std::uint64_t count) noexcept {
    UASSERT(buckets_);
    UASSERT(bucket_index < GetView().GetBucketCount());
    const auto buckets = impl::histogram::Access::Buckets(impl::histogram::MutableView{buckets_.get()});
...
void DumpMetric(Writer& writer, const HistogramAggregator& histogram) { writer = histogram.GetView(); }
