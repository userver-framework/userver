...
class PortabilityInfoCollector final : public utils::statistics::BaseFormatBuilder {
public:
    void HandleMetric(std::string_view path_view, utils::statistics::LabelsSpan labels, const MetricValue& value)
        override {
        ...
        value.Visit(utils::Overloaded{
            ...
            [&, this](double x) {
                if (std::isinf(x)) {
                    ReportError(info_[WarningCode::kInf], path, labels, "Value is +/-INF");
                }

                if (std::isnan(x)) {
                    ReportError(info_[WarningCode::kNan], path, labels, "Value is +/-NAN");
                }
            },
            [](Rate) {},
            [&, this](HistogramView x) {
                if (x.GetBucketCount() > impl::solomon::kMaxHistogramBuckets) {
                    ReportError(
                        info_[WarningCode::kHistogramBucketsCount],
                        path,
                        labels,
                        fmt::format(
                            "Too many histogram buckets: {} while "
        ...
        static constexpr std::size_t kMaxLabelsPortableValue = impl::solomon::kMaxLabels;
        if (labels.size() > kMaxLabelsPortableValue) {
            ReportError(
                info_[WarningCode::kLabelsCount],
                path,
                labels,
                fmt::format(
                    "Too many labels: {} while recommended value is not more than {}",
                    labels.size(),
                    kMaxLabelsPortableValue
                )
            );
        }

        static constexpr std::size_t kMaxLabelNameLen = impl::solomon::kMaxLabelNameLen;
        static constexpr std::size_t kMaxLabelValueLen = impl::solomon::kMaxLabelValueLen;
        ...
        for (const auto& label : labels) {
            ...
            if (it != kReservedLabelNames.end()) {
                ReportError(info_[it->second], path, labels, fmt::format("Reserved label used: {}", it->first));
            }

            if (label.Name().size() > kMaxLabelNameLen) {
                ReportError(
                    info_[WarningCode::kLabelNameLength],
                    path,
                    labels,
                    fmt::format("Label '{}' exceeds recommended label name length {}", label.Name(), kMaxLabelNameLen)
                );
            }

            if (label.Value().size() > kMaxLabelValueLen) {
                ReportError(
                    info_[WarningCode::kLabelValueLength],
                    path,
                    labels,
                    fmt::format("Value of label '{}' exceeds recommended length {}", label.Name(), kMaxLabelValueLen)
                );
            }
        ...
        if (it == path_labels_.end()) {
            ...
            ReportError(
                info_[WarningCode::kLabelNameMismatch],
                path,
                labels,
                fmt::format(
                    "Same path '{}' has metrics with different labels set: "
                    "'{}' vs '{}'",
                    path,
                    fmt::join(it->second, ", "),
...
private:
    ...
    inline static const std::unordered_map<std::string_view, WarningCode> kReservedLabelNames = {
...
std::string_view ToString(WarningCode code) {
    ...
    UINVARIANT(false, "Unhandled WarningCode value " + std::to_string(static_cast<int>(code)));
}
