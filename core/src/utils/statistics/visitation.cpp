...
void ProcessInternalNode(
    ...
    if (metadata.IsObject()) {
        ...
        if (metadata.HasMember("solomon_children_labels")) {
            children_label_name = metadata["solomon_children_labels"].As<std::string>();
        }
    }
    ...
    dfs_stack.emplace(value, std::move(path_node).value_or(key), std::move(label), std::move(children_label_name));
}
...
void ProcessLeaf(
    ...
    if (value.IsInt64()) {
        ...
        if (!value.IsNull()) {
            UASSERT_MSG(false, fmt::format("Non-numeric metric at path '{}': {}", path.Get(), ToString(value)));
        }
    }
    ...
    if (metric_value && utils::text::StartsWith(path.Get(), request.prefix)) {
        if (LeftContainsRight(labels.Labels(), request.require_labels)) {
            if (request.prefix_match_type != Request::PrefixMatch::kExact || path.Get() == request.prefix) {
                ...
                labels_vector.reserve(labels.Labels().size() + request.add_labels.size());
...
void VisitMetrics(BaseFormatBuilder& out, const formats::json::Value& statistics_storage_json, const Request& request) {
    ...
    dfs_stack.emplace(statistics_storage_json, std::string{}, Label{}, std::string{});
