...
ThreadCpuUsage GetCurrentThreadCpuUsage() {
    ...
    const auto timeval_to_mcs = [](const timeval& tv) {
        return std::chrono::microseconds{static_cast<std::uint64_t>(tv.tv_sec) * 1'000'000 + tv.tv_usec};
    };
...
void ThreadCpuStatsStorage::Collect() {
    UASSERT_MSG(ValidateThreadSafety(), "ThreadCpuStatsStorage is not thread-safe, as it doesn't make sense.");
    ...
    std::vector<Percent> Collect() {
        ...
        for (auto& meta : threads_) {
            ...
            if (last_ts_ == Clock::time_point{}) {
                ...
                result.push_back(std::clamp(current_usage_pct, kMinAllowedUsage, kMaxAllowedUsage));
            }
        }
...
std::vector<Percent> ThreadPoolCpuStatsStorage::CollectCurrentLoadPct() { return impl_->Collect(); }
