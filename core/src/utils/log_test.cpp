...
std::string NonUtf8Msg(size_t bytes) { return "<Non utf-8, total " + std::to_string(bytes) + " bytes>"; }
...
const std::string kBrokenUtf8Data = std::string("qwe") + static_cast<char>(0b10111111) + std::string("rty");

const std::string kMultibyteUtf8Data = "qweрти";

}  // namespace

TEST(TestToLimitedHex, NonTruncated) { EXPECT_EQ(utils::log::ToLimitedHex(kBinaryData, 30), kBinaryHex); }
...
TEST(TestToLimitedHex, TruncatedNegative) { EXPECT_EQ(utils::log::ToLimitedHex(kBinaryData, -1), kBinaryHex); }
