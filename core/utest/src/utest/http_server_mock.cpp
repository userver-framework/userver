...
int HttpConnection::OnHeadersComplete(llhttp_t* p) { return GetConnection(p).DoOnHeadersComplete(); }

int HttpConnection::OnBody(llhttp_t* p, const char* data, size_t size) { return GetConnection(p).DoOnBody(data, size); }

int HttpConnection::OnMessageComplete(llhttp_t* p) { return GetConnection(p).DoOnMessageComplete(); }

HttpConnection& HttpConnection::GetConnection(llhttp_t* p) { return *static_cast<HttpConnection*>(p->data); }

int HttpConnection::DoOnUrl(llhttp_t* p, const char* data, size_t size) {
    http_request_.method = ConvertHttpMethod(static_cast<llhttp_method>(p->method));
...
SimpleServer::Response HttpConnection::operator()(const SimpleServer::Request& request) {
    ...
    if (http_response_) {
        auto data = "HTTP/1.1 " + std::to_string(http_response_->response_status) + " CODE\r\n";
