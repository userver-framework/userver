...
void LogFatalException(const std::exception& ex, const char* name) {
    ...
    const auto message = fmt::format(
        "C++ exception with description \"{}\" ({}) thrown in {}.{}{}",
        ex.what(),
...
bool IsCurrentTestCancelled() {
    const auto& current_test_status = *::testing::UnitTest::GetInstance()->current_test_info()->result();
...
void DoRunTest(
    ...
    DeathTestsEnabled death_tests_enabled,
    std::function<std::unique_ptr<EnrichedTestBase>()> factory
) {
