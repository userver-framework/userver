...
using ServerDigestAuthSecret = utils::NonLoggable<class DigestSecretKeyTag, std::string>;
...
public:
    ...
    [[nodiscard]] AuthCheckResult CheckAuth(const http::HttpRequest& request, request::RequestContext& request_context)
        const final;
    ...
    ValidateResult ValidateUserData(const ContextFromClient& client_context, const UserData& user_data) const;
    /// @endcond
private:
    ...
    std::string ConstructAuthInfoHeader(const ContextFromClient& client_context, std::string_view etag) const;
