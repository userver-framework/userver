...
struct fmt::formatter<USERVER_NAMESPACE::server::http::HttpMethod> {
    ...
    auto format(USERVER_NAMESPACE::server::http::HttpMethod method, FormatContext& ctx) USERVER_FMT_CONST {
        return fmt::format_to(ctx.out(), "{}", USERVER_NAMESPACE::server::http::ToString(method));
    }
};
