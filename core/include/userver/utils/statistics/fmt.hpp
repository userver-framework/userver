...
struct fmt::formatter<USERVER_NAMESPACE::utils::statistics::LabelsSpan> {
    ...
    template <typename FormatContext>
    auto format(USERVER_NAMESPACE::utils::statistics::LabelsSpan value, FormatContext& ctx) const {
...
public:
    constexpr auto parse(format_parse_context& ctx) { return rate_format_.parse(ctx); }
...
private:
    fmt::formatter<USERVER_NAMESPACE::utils::statistics::Rate::ValueType> rate_format_;
};
...
public:
    constexpr auto parse(format_parse_context& ctx) {
        ...
        return max(
            int_format_.parse(ctx),
            max(float_format_.parse(ctx), max(rate_format_.parse(ctx), histogram_format_.parse(ctx)))
        );
    }
...
private:
    ...
    fmt::formatter<USERVER_NAMESPACE::utils::statistics::HistogramView> histogram_format_;
};
