...
class MinMaxAvg final {
    static_assert(
        std::is_integral_v<ValueType> && !std::is_same_v<ValueType, bool>,
        "only integral value types are supported in MinMaxAvg"
    );
    static_assert(
        std::is_same_v<AverageType, ValueType> || std::is_floating_point_v<AverageType>,
        "MinMaxAvg average type must either be equal to value type or "
        "be a floating point type"
    );
    static_assert(
        std::atomic<ValueType>::is_always_lock_free && std::atomic<size_t>::is_always_lock_free,
...
public:
    ...
    constexpr MinMaxAvg() noexcept : minimum_(0), maximum_(0), sum_(0), count_(0) {}
    ...
    Current GetCurrent() const {
        ...
        current.average =
            count ? static_cast<AverageType>(sum_.load(std::memory_order_relaxed)) / static_cast<AverageType>(count)
                  : AverageType{0};
        return current;
    }
    ...
    void Account(ValueType value) {
        ...
        while (current_minimum > value || !count_.load(std::memory_order_relaxed)) {
            if (minimum_.compare_exchange_weak(current_minimum, value, std::memory_order_relaxed)) {
                break;
            }
        }
        ...
        while (current_maximum < value || !count_.load(std::memory_order_relaxed)) {
            if (maximum_.compare_exchange_weak(current_maximum, value, std::memory_order_relaxed)) {
                break;
            }
        }
    ...
    void Add(
        ...
        count_.fetch_add(other.count_.load(std::memory_order_acquire), std::memory_order_release);
    }
...
auto Serialize(const MinMaxAvg<ValueType, AverageType>& mma, formats::serialize::To<formats::json::Value>) {
    ...
    return formats::json::MakeObject("min", current.minimum, "max", current.maximum, "avg", current.average);
}
