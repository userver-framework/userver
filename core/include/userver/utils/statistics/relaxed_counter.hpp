...
public:
    ...
    void Store(T desired) noexcept { val_.store(desired, std::memory_order_relaxed); }
    ...
    T operator++(int) noexcept { return val_.fetch_add(1, std::memory_order_relaxed); }
    ...
    T operator--(int) noexcept { return val_.fetch_sub(1, std::memory_order_relaxed); }
