...
template <typename Result, typename Counter, typename Duration, typename = void_t<>>
...
struct CanReset<Counter, void_t<decltype(std::declval<Counter&>().Reset())>> : std::true_type {};
