...
template <
    ...
    std::size_t ExtraBuckets = 0,
    std::size_t ExtraBucketSize = 500>
    ...
    void Account(std::size_t value) noexcept {
        if (value < values_.size()) {
            ...
            if (!extra_values_.empty()) {
                std::size_t extra_bucket = (value - values_.size() + ExtraBucketSize / 2) / ExtraBucketSize;
...
private:
    size_t ExtraBucketToValue(std::size_t bucket) const noexcept { return values_.size() + bucket * ExtraBucketSize; }

    static_assert(
        std::atomic<Counter>::is_always_lock_free,
