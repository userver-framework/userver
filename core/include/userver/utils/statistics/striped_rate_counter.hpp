...
public:
    ...
    void Store(Rate desired) noexcept { offset_ = static_cast<std::uintptr_t>(desired.value) - val_.Read(); }

    Rate Load() const noexcept { return Rate{static_cast<std::uint64_t>(LoadImpl())}; }

    void Add(Rate arg) noexcept { val_.Add(static_cast<std::uintptr_t>(arg.value)); }
...
private:
    ...
    std::uintptr_t LoadImpl() const noexcept { return val_.Read() + offset_.load(std::memory_order_relaxed); }
