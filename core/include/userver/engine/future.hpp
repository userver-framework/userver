...
template <typename T>
template <typename Rep, typename Period>
FutureStatus Future<T>::wait_for(std::chrono::duration<Rep, Period> timeout) const {
    return wait_until(Deadline::FromDuration(timeout));
}

template <typename T>
template <typename Clock, typename Duration>
FutureStatus Future<T>::wait_until(std::chrono::time_point<Clock, Duration> until) const {
...
inline void Promise<void>::set_exception(std::exception_ptr ex) { state_->SetException(std::move(ex)); }
