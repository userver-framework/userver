...
template <typename Predicate>
bool ConditionVariable::Wait(std::unique_lock<Mutex>& lock, Predicate&& predicate) {
    return WaitUntil(lock, {}, std::forward<Predicate>(predicate));
}

template <typename Rep, typename Period>
CvStatus ConditionVariable::WaitFor(std::unique_lock<Mutex>& lock, std::chrono::duration<Rep, Period> timeout) {
...
bool ConditionVariable::WaitFor(
    ...
    return WaitUntil(lock, Deadline::FromDuration(timeout), std::forward<Predicate>(predicate));
}

template <typename Clock, typename Duration>
CvStatus ConditionVariable::WaitUntil(std::unique_lock<Mutex>& lock, std::chrono::time_point<Clock, Duration> until) {
...
bool ConditionVariable::WaitUntil(
    ...
    return WaitUntil(lock, Deadline::FromTimePoint(until), std::forward<Predicate>(predicate));
}

template <typename Predicate>
bool ConditionVariable::WaitUntil(std::unique_lock<Mutex>& lock, Deadline deadline, Predicate&& predicate) {
