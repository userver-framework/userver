...
class SQLiteHandler final : public server::handlers::HttpHandlerBase {
 public:
  ...
  SQLiteHandler(const components::ComponentConfig& config,
...
SQLiteHandler::SQLiteHandler(const components::ComponentConfig& config,
                             const components::ComponentContext& context)
...
std::string SQLiteHandler::HandleRequestThrow(
    const server::http::HttpRequest& request,
  ...
  if (request.GetMethod() == server::http::HttpMethod::kPost) {
    ...
    sqlite_client_->ExecuteDecompose(
        ...
        "INSERT INTO key_value_table VALUES (?, ?)", data);
  } else if (request.GetMethod() == server::http::HttpMethod::kDelete) {
    const storages::sqlite::Query query{
        "DELETE FROM key_value_table WHERE key = ?"};
    sqlite_client_->Execute(storages::sqlite::OperationType::kReadWrite, query,
                            key);
    return fmt::format("Deleted by key: {}", key);
  }
  const storages::sqlite::Query query{
      "SELECT key, value FROM key_value_table WHERE key = ?"};

  const auto result =
      sqlite_client_
          ->Execute(storages::sqlite::OperationType::kReadOnly, query, key)
