...
class SQLiteTypesStringViewTest
    : public SQLiteCompositeFixture<SQLiteInMemoryConnection> {
  void PreInitialize(const ClientPtr& client) final {
...
class SQLiteTypesMismatchTest
 ...
 protected:
  static constexpr std::string_view kMismatchTypesTable =
      "CREATE TABLE test(int INTEGER, str TEXT)";
};

class SQLiteTypesStrictMismatchTest : public SQLiteTypesMismatchTest {
  void PreInitialize(const ClientPtr& client) final {
    client->Execute(OperationType::kReadWrite,
                    std::string(kMismatchTypesTable) + " STRICT");
  }
};
...
struct TestRow {
  ...
  bool operator==(const TestRow& other) const {
    return std::tie(uint8, int8, uint16, int16, uint32, int32, uint64, int64,
                    fl, db, str, blob, boolean, datetime, uuid, decimal,
                    json) ==
           std::tie(other.uint8, other.int8, other.uint16, other.int16,
                    other.uint32, other.int32, other.uint64, other.int64,
                    other.fl, other.db, other.str, other.blob, other.boolean,
                    other.datetime, other.uuid, other.decimal, other.json);
  }
};
...
struct OptionalTestRow {
  ...
  bool operator==(const OptionalTestRow& other) const {
    return std::tie(uint8, int8, uint16, int16, uint32, int32, uint64, int64,
                    fl, db, str, blob, boolean, datetime, uuid, deciaml,
                    json) ==
           std::tie(other.uint8, other.int8, other.uint16, other.int16,
                    other.uint32, other.int32, other.uint64, other.int64,
                    other.fl, other.db, other.str, other.blob, other.boolean,
                    other.datetime, other.uuid, other.deciaml, other.json);
  }
};
...
UTEST_F(SQLiteTypesTest, BindExtract) {
  ...
  UEXPECT_NO_THROW(
      client->ExecuteDecompose(storages::sqlite::OperationType::kReadWrite,
  ...
  EXPECT_EQ(actual_test_row, expected_test_row)
      << "Input and output rows must coincide";
}
...
UTEST_F(SQLiteTypesStringViewTest, BindExtract) {
  ...
  EXPECT_EQ(actual_str, expected_str)
      << "Input and output std::string must coincide";
}
...
UTEST_F(SQLiteTypesMismatchTest, BindExtractMismatch) {
  ...
  UEXPECT_NO_THROW(client->ExecuteDecompose(
      ...
      "INSERT INTO test VALUES (?, ?)", expected_input_test_row))
      << "Try to bind values of diffrent types";

  {
    ...
    EXPECT_EQ(actual_test_row, expected_input_test_row)
        << "Input and output rows must coincide";
  }

  {
    ...
    EXPECT_EQ(actual_test_row, expected_correct_test_row)
        << "Input and output rows must coincide";
  }
}
...
UTEST_F(SQLiteTypesStrictMismatchTest, BindExtractMismatch) {
  ...
  UEXPECT_THROW(client->ExecuteDecompose(
                    storages::sqlite::OperationType::kReadWrite,
                    "INSERT INTO test VALUES (?, ?)", expected_test_row),
                SQLiteException)
