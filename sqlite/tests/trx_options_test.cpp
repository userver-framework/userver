...
std::string TestParamNameIsolationLevel(
    const ::testing::TestParamInfo< ::userver::storages::sqlite::settings::
...
class SQLiteTransactionsConcurentTest
    : public SQLiteParametrizedFixture<
          SQLiteCustomConnection,
          settings::TransactionOptions::IsolationLevel> {
...
UTEST_P_MT(SQLiteTransactionsConcurentTest, IsolationLevels, 3) {
  ...
  auto writer = engine::AsyncNoSpan([&]() {
    ...
    UEXPECT_NO_THROW(trx.Execute("INSERT INTO test VALUES (NULL, 'dirty_data')")
                         .AsExecutionResult());
  ...
  auto reader = engine::AsyncNoSpan([&]() {
    ...
    std::unique_lock<engine::Mutex> lock(mu);
    EXPECT_TRUE(
        reader_cv.Wait(lock, [&writer_inserted] { return writer_inserted; }));
    UEXPECT_NO_THROW(
        trx = client->Begin(OperationType::kReadOnly, reader_options));
    std::vector<std::string> result;
    UEXPECT_NO_THROW(
        result =
            trx.Execute("SELECT value FROM test WHERE value = 'dirty_data'")
  ...
  std::vector<std::string> result;
  UEXPECT_NO_THROW(
      result = trx.Execute("SELECT value FROM test WHERE value = 'dirty_data'")
                   .AsVector<std::string>(kFieldTag));
  EXPECT_TRUE(result.empty());
}

INSTANTIATE_UTEST_SUITE_P(
    SQLiteTrxIsolationLevels, SQLiteTransactionsConcurentTest,
    ::testing::Values(
        settings::TransactionOptions::IsolationLevel::kSerializable,
        settings::TransactionOptions::IsolationLevel::kReadUncommitted),
