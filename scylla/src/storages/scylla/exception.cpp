...
ServerException::ServerException(int code, std::string_view what) : QueryException{}, code_(code) {
...
TimeoutException::TimeoutException(int code, std::string_view what, std::chrono::milliseconds timeout)
    : ServerException{code, {}}, timeout_(timeout) {
    *this << "ScyllaDB timeout [" << code << ", " << timeout.count() << "ms]: " << what;
}

CancelledException::CancelledException(ByDeadlinePropagation) : ScyllaException{}, by_deadline_propagation_(true) {
