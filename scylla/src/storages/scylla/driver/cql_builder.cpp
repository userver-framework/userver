...
void AppendValueToCollection(CassCollection* col, const Value& v) {
    std::visit([col](const auto& alt) {
        ...
        if constexpr (std::is_same_v<T, std::monostate>) {
            ...
            CheckRc(cass_collection_append_int64(col, static_cast<cass_int64_t>(ms)), "collection.append_timestamp");
        } else if constexpr (std::is_same_v<T, Blob>) {
            CheckRc(
                cass_collection_append_bytes(
                    col, reinterpret_cast<const cass_byte_t*>(alt.data()), alt.size()),
...
void BindValue(CassStatement* stmt, std::size_t i, const Value& v) {
    std::visit([stmt, i](const auto& alt) {
        using T = std::decay_t<decltype(alt)>;
        if constexpr (std::is_same_v<T, std::monostate>) {
            ...
            CheckRc(
                cass_statement_bind_bytes(
                    stmt, i, reinterpret_cast<const cass_byte_t*>(alt.data()), alt.size()),
                "bind_bytes");
        } else if constexpr (std::is_same_v<T, Inet>) {
            CassInet inet;
            if (cass_inet_from_string(alt.text.c_str(), &inet) != CASS_OK) {
...
std::string BuildFullTableName(std::string_view keyspace, std::string_view table) {
    if (keyspace.empty()) return std::string{table};
    return fmt::format("{}.{}", keyspace, table);
}
...
CassStatementPtr BuildDropKeyspaceStatement(const RequestContext& ctx) {
    ...
    if (!IsValidCqlIdentifier(ctx.keyspace)) {
        throw InvalidConfigException(fmt::format(
            "Refusing to drop keyspace '{}': not a valid unquoted CQL identifier",
            ctx.keyspace));
    }
    return CassStatementPtr{
        cass_statement_new(fmt::format("DROP KEYSPACE IF EXISTS {}", ctx.keyspace).c_str(), 0)};
}
...
CassStatementPtr BuildTruncateStatement(const RequestContext& ctx) {
    ...
    if (!IsValidCqlIdentifier(ctx.table)) {
        throw InvalidConfigException(fmt::format(
            "Refusing to truncate '{}': not a valid unquoted CQL identifier",
            ctx.table));
    }
    if (!ctx.keyspace.empty() && !IsValidCqlIdentifier(ctx.keyspace)) {
        throw InvalidConfigException(fmt::format(
            "Refusing to truncate in keyspace '{}': not a valid unquoted CQL identifier",
            ctx.keyspace));
    }
    const auto full_table = BuildFullTableName(ctx.keyspace, ctx.table);
    return CassStatementPtr{
        cass_statement_new(fmt::format("TRUNCATE {}", full_table).c_str(), 0)};
}
