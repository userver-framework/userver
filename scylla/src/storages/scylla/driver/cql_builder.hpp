...
std::string BuildFullTableName(std::string_view keyspace, std::string_view table);
...
inline std::string BuildColumnList(const std::vector<std::string>& columns, bool select_all) {
    if (select_all || columns.empty()) return "*";
...
inline WhereFragment BuildWhereClause(const std::vector<operations::NamedValue>& conditions) {
    ...
    for (const auto& c : conditions) {
        clauses.emplace_back(fmt::format("{} = ?", c.column_name));
...
inline IfFragment BuildIfClause(
    ...
    if (if_conditions.empty()) return out;
...
inline auto Insert(
    ...
    return [&bindings, if_not_exists, &using_clause](std::string_view full_table) -> CqlQuery {
        if (bindings.empty()) {
        ...
        for (const auto& b : bindings) {
            names.emplace_back(b.column_name);
            values.emplace_back(b.value);
        }

        auto using_frag = BuildUsingClause(using_clause);
        for (auto& v : using_frag.values) values.emplace_back(std::move(v));

        return {
            fmt::format("INSERT INTO {} ({}) VALUES ({}){}{}",
                full_table, fmt::join(names, ", "), Placeholders(bindings.size()),
...
inline auto SelectOne(
    ...
    return [&columns, select_all, &conditions](std::string_view full_table) -> CqlQuery {
        auto where = BuildWhereClause(conditions);
        return {
            fmt::format("SELECT {} FROM {}{} LIMIT 1",
                BuildColumnList(columns, select_all), full_table, where.text),
...
inline auto SelectMany(
    ...
    return [&columns, select_all, &conditions, limit, allow_filtering](std::string_view full_table) -> CqlQuery {
        auto where = BuildWhereClause(conditions);
        ...
        return {
            fmt::format("SELECT {} FROM {}{}{}{}",
                BuildColumnList(columns, select_all), full_table, where.text, limit_text, af_text),
            std::move(where.values),
        };
    };
}
...
inline auto DeleteRows(const std::vector<operations::NamedValue>& conditions) {
    return [&conditions](std::string_view full_table) -> CqlQuery {
        if (conditions.empty()) {
...
inline auto DeleteRowsLwt(
    ...
    return [&conditions, &if_conditions, if_exists](std::string_view full_table) -> CqlQuery {
        if (conditions.empty()) {
            throw QueryException("DeleteOne: WHERE clause is required");
        }
        auto where = BuildWhereClause(conditions);
        ...
        for (auto& v : if_frag.values) values.emplace_back(std::move(v));
...
inline auto Update(
    ...
    return [&assignments, &conditions, &using_clause](std::string_view full_table) -> CqlQuery {
        ...
        for (const auto& a : assignments) {
            set_clauses.emplace_back(fmt::format("{} = ?", a.column_name));
            values.emplace_back(a.value);
        }

        auto where = BuildWhereClause(conditions);
        for (auto& v : where.values) values.emplace_back(std::move(v));

        return {
            fmt::format("UPDATE {}{} SET {}{}",
                full_table, using_frag.text, fmt::join(set_clauses, ", "), where.text),
            std::move(values),
        };
    };
}
...
inline auto UpdateLwt(
    ...
    return [&assignments, &conditions, &if_conditions, if_exists, &using_clause](std::string_view full_table) -> CqlQuery {
        if (assignments.empty()) {
            throw QueryException("UpdateOne: no assignments provided");
        }
        if (conditions.empty()) {
        ...
        std::vector<Value> values;
        values.reserve(
            using_frag.values.size() + assignments.size() + conditions.size() + if_conditions.size());
        ...
        return {
            fmt::format("UPDATE {}{} SET {}{}{}",
                full_table, using_frag.text, fmt::join(set_clauses, ", "), where.text, if_frag.text),
            std::move(values),
        };
    };
}
