...
public:
    explicit Transaction(infra::ConnectionPtr&& connection, engine::Deadline deadline);
    ~Transaction();
...
private:
    StatementResultSet DoExecute(const Query& query, impl::io::ParamsBinderBase& params) const;
...
template <typename T>
StatementResultSet Transaction::ExecuteDecompose(const Query& query, const T& row) const {
...
template <typename Container>
StatementResultSet Transaction::ExecuteBulk(const Query& query, const Container& params) const {
...
template <typename MapTo, typename Container>
StatementResultSet Transaction::ExecuteBulkMapped(const Query& query, const Container& params) const {
    UINVARIANT(!params.empty(), "Empty params in bulk execution");
