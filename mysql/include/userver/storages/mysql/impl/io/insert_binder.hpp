...
protected:
    void SetBindCallback(void* user_data, char (*param_cb)(void*, void*, std::size_t));
...
private:
    ...
    static constexpr bool kIsMapped = !std::is_same_v<typename Container::value_type, Row>;

    static char BindsRowCallback(void* user_data, void* binds_array, std::size_t row_number);

    void BindColumns() {
        UpdateCurrentRowPtr();
        boost::pfr::for_each_field(*current_row_ptr_, [&binds = GetBinds()](const auto& field, std::size_t i) {
    ...
    void UpdateCurrentRowPtr() {
        if constexpr (kIsMapped) {
            current_row_.value = storages::mysql::convert::DoConvert<Row>(*current_row_it_);
    ...
    bool CheckRowNumber(std::size_t row_number) {
        if (row_number < max_row_number_seen_ || row_number > max_row_number_seen_ + 1) {
            return false;
        }
...
char InsertBinder<Container, MapTo>::BindsRowCallback(void* user_data, void* binds_array, std::size_t row_number) {
