...
private:
    static constexpr bool kIsMapped = !std::is_same_v<typename Container::value_type, MapFrom>;
...
private:
    ...
    static constexpr bool kCanBindInPlace = std::is_same_v<Container, std::vector<MapFrom>>;
...
template <typename Container, typename MapFrom, typename ExtractionTag>
impl::bindings::OutputBindings& TypedExtractor<Container, MapFrom, ExtractionTag>::BindNextRow() {
...
template <typename Container, typename MapFrom, typename ExtractionTag>
std::size_t TypedExtractor<Container, MapFrom, ExtractionTag>::ColumnsCount() const {
...
template <typename Container, typename MapFrom>
template <typename ExtractionTag>
impl::bindings::OutputBindings& ProxyingStorage<Container, MapFrom>::BindNextRow() {
...
void ProxyingStorage<Container, MapFrom>::CommitLastRow() {
    if constexpr (kIsMapped) {
        *meta::Inserter(data_) = storages::mysql::convert::DoConvert<typename Container::value_type>(std::move(row_));
    } else {
