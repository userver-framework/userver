...
private:
    ...
    std::string GetValue(std::string_view key, const server::http::HttpRequest& request) const {
        auto result =
            mysql_
                ->Execute(cc_, storages::mysql::ClusterHostType::kPrimary, "SELECT value FROM kv WHERE `key` = ?", key)
    ...
    std::string DeleteValue(std::string_view key) const {
        mysql_->Execute(cc_, storages::mysql::ClusterHostType::kPrimary, "DELETE FROM kv WHERE `key` = ?", key);
