#include "mysql_cache_test_fwd.hpp"
...
struct MySqlExamplePolicy {
  ...
  using UpdatedFieldType = storages::mysql::DateTime;
  ...
  //
  // Default value is storages::MySql::ClusterHostType::kSlave, at least one
...
using USERVER_NAMESPACE::example::MySqlExamplePolicy;

struct MySqlExamplePolicy2 {
  using ValueType = MyStructure;
  static constexpr std::string_view kName = "my-pg-cache";
  static constexpr const char* kQuery =
      "select id, bar, updated from test.my_data";
...
static_assert(mysql_cache::detail::kHasName<MySqlExamplePolicy>);
static_assert(mysql_cache::detail::kHasName<MySqlExamplePolicy>);
static_assert(mysql_cache::detail::kHasName<MySqlExamplePolicy>);

static_assert((std::is_same<
               mysql_cache::detail::KeyMemberType<MySqlExamplePolicy>, int>{}));
static_assert(
    (std::is_same<mysql_cache::detail::KeyMemberType<MySqlExamplePolicy2>,
                  int>{}));

static_assert(mysql_cache::detail::ClusterHostType<MySqlExamplePolicy>() ==
              storages::mysql::ClusterHostType::kPrimary);
static_assert(mysql_cache::detail::ClusterHostType<MySqlExamplePolicy2>() ==
              storages::mysql::ClusterHostType::kPrimary);
...
static_assert(mysql_cache::detail::kHasCustomUpdated<MySqlExamplePolicy3>);

/*! [Pg Cache Policy GetQuery Example] */
struct MySqlExamplePolicy4 {
  static constexpr std::string_view kName = "my-pg-cache";
  ...
  using UpdatedFieldType =
      storages::mysql::DateTime;  // no time zone (should be avoided)
};
/*! [Pg Cache Policy GetQuery Example] */

static_assert(mysql_cache::detail::kHasGetQuery<MySqlExamplePolicy4>);
...
static_assert(mysql_cache::detail::kHasGetQuery<MySqlExamplePolicy5>);
...
using MyCache1 = MySqlCache<MySqlExamplePolicy>;
using MyCache2 = MySqlCache<MySqlExamplePolicy2>;
using MyCache3 = MySqlCache<MySqlExamplePolicy3>;
using MyCache4 = MySqlCache<MySqlExamplePolicy4>;
using MyTrivialCache = MySqlCache<MySqlTrivialPolicy>;
using MyCache5 = MySqlCache<MySqlExamplePolicy5>;
using MyCache6 = MySqlCache<MySqlExamplePolicy6>;
using MyCache7 = MySqlCache<MySqlExamplePolicy7>;
...
namespace mysql = storages::mysql;
static_assert(MyCache1::kClusterHostTypeFlags == mysql::ClusterHostType::kPrimary);
static_assert(MyCache2::kClusterHostTypeFlags == mysql::ClusterHostType::kPrimary);
static_assert(MyCache3::kClusterHostTypeFlags == mysql::ClusterHostType::kPrimary);
static_assert(MyCache4::kClusterHostTypeFlags == mysql::ClusterHostType::kPrimary);
static_assert(MyCache5::kClusterHostTypeFlags == mysql::ClusterHostType::kPrimary);
static_assert(MyCache6::kClusterHostTypeFlags == mysql::ClusterHostType::kPrimary);
static_assert(MyCache7::kClusterHostTypeFlags == mysql::ClusterHostType::kPrimary);
...
inline auto SampleOfComponentRegistration() {
  ...
  return components::MinimalServerComponentList()
      .Append<components::MySqlCache<example::MySqlTrivialPolicy>>();
  /*! [Pg Cache Trivial Usage] */
}
