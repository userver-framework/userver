...
void InputBindings::SetParamsCallback(ParamsCallback params_cb) { params_cb_ = params_cb; }

InputBindings::ParamsCallback InputBindings::GetParamsCallback() const { return params_cb_; }
...
void InputBindings::Bind(std::size_t pos, C<uint8_t>& val) { BindValue(pos, GetNativeType(val), val, 0, true); }
void InputBindings::Bind(std::size_t pos, C<int8_t>& val) { BindValue(pos, GetNativeType(val), val, 0); }

void InputBindings::Bind(std::size_t pos, C<uint16_t>& val) { BindValue(pos, GetNativeType(val), val, 0, true); }
void InputBindings::Bind(std::size_t pos, C<int16_t>& val) { BindValue(pos, GetNativeType(val), val, 0); }

void InputBindings::Bind(std::size_t pos, C<uint32_t>& val) { BindValue(pos, GetNativeType(val), val, 0, true); }
void InputBindings::Bind(std::size_t pos, C<int32_t>& val) { BindValue(pos, GetNativeType(val), val, 0); }

void InputBindings::Bind(std::size_t pos, C<uint64_t>& val) { BindValue(pos, GetNativeType(val), val, 0, true); }
void InputBindings::Bind(std::size_t pos, C<int64_t>& val) { BindValue(pos, GetNativeType(val), val, 0); }

void InputBindings::Bind(std::size_t pos, C<float>& val) { BindValue(pos, GetNativeType(val), val, 0); }
void InputBindings::Bind(std::size_t pos, C<double>& val) { BindValue(pos, GetNativeType(val), val, 0); }

void InputBindings::Bind(std::size_t pos, C<io::DecimalWrapper>& val) { BindDecimal(pos, val); }

void InputBindings::Bind(std::size_t pos, C<std::string>& val) { Bind(pos, std::string_view{val}); }
void InputBindings::Bind(std::size_t pos, C<std::string_view>& val) { BindStringView(pos, val); }

void InputBindings::Bind(std::size_t pos, C<formats::json::Value>& val) { BindJson(pos, val); }

void InputBindings::Bind(std::size_t pos, C<std::chrono::system_clock::time_point>& val) { BindTimePoint(pos, val); }

void InputBindings::Bind(std::size_t pos, C<Date>& val) { BindDate(pos, val); }
void InputBindings::Bind(std::size_t pos, C<DateTime>& val) { BindDateTime(pos, val); }
...
void InputBindings::BindTimePoint(std::size_t pos, C<std::chrono::system_clock::time_point>& val) {
    UASSERT(pos < Size());
