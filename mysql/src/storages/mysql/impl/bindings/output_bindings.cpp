...
bool IsNumericBindable(enum_field_types bind_type, enum_field_types field_type) {
    UASSERT(NativeBindsHelper::IsFieldNumeric(bind_type) && NativeBindsHelper::IsFieldNumeric(field_type));

    return NativeBindsHelper::NumericFieldWidth(bind_type) >= NativeBindsHelper::NumericFieldWidth(field_type);
}
...
bool IsBindable(enum_field_types bind_type, enum_field_types field_type) {
    if (bind_type == MYSQL_TYPE_STRING) {
        return field_type == MYSQL_TYPE_STRING || field_type == MYSQL_TYPE_VAR_STRING ||
    ...
    if (bind_type == MYSQL_TYPE_TIMESTAMP) {
        return field_type == MYSQL_TYPE_TIMESTAMP ||
               ...
               field_type == MYSQL_TYPE_DATE || field_type == MYSQL_TYPE_NEWDATE || field_type == MYSQL_TYPE_DATETIME;
    }

    if (NativeBindsHelper::IsFieldNumeric(bind_type) && NativeBindsHelper::IsFieldNumeric(field_type)) {
...
void OutputBindings::Bind(std::size_t pos, uint8_t& val) { BindValue(pos, GetNativeType(val), &val, true); }
void OutputBindings::Bind(std::size_t pos, O<uint8_t>& val) { BindPrimitiveOptional(pos, val, true); }
void OutputBindings::Bind(std::size_t pos, int8_t& val) { BindValue(pos, GetNativeType(val), &val); }
void OutputBindings::Bind(std::size_t pos, O<int8_t>& val) { BindPrimitiveOptional(pos, val); }

void OutputBindings::Bind(std::size_t pos, uint16_t& val) { BindValue(pos, GetNativeType(val), &val, true); }
void OutputBindings::Bind(std::size_t pos, O<uint16_t>& val) { BindPrimitiveOptional(pos, val, true); }
void OutputBindings::Bind(std::size_t pos, int16_t& val) { BindValue(pos, GetNativeType(val), &val); }
void OutputBindings::Bind(std::size_t pos, O<int16_t>& val) { BindPrimitiveOptional(pos, val); }

void OutputBindings::Bind(std::size_t pos, uint32_t& val) { BindValue(pos, GetNativeType(val), &val, true); }
void OutputBindings::Bind(std::size_t pos, O<uint32_t>& val) { BindPrimitiveOptional(pos, val, true); }
void OutputBindings::Bind(std::size_t pos, int32_t& val) { BindValue(pos, GetNativeType(val), &val); }
void OutputBindings::Bind(std::size_t pos, O<int32_t>& val) { BindPrimitiveOptional(pos, val); }

void OutputBindings::Bind(std::size_t pos, uint64_t& val) { BindValue(pos, GetNativeType(val), &val, true); }
void OutputBindings::Bind(std::size_t pos, O<uint64_t>& val) { BindPrimitiveOptional(pos, val, true); }
void OutputBindings::Bind(std::size_t pos, int64_t& val) { BindValue(pos, GetNativeType(val), &val); }
void OutputBindings::Bind(std::size_t pos, O<int64_t>& val) { BindPrimitiveOptional(pos, val); }

void OutputBindings::Bind(std::size_t pos, float& val) { BindValue(pos, GetNativeType(val), &val); }
void OutputBindings::Bind(std::size_t pos, O<float>& val) { BindPrimitiveOptional(pos, val); }
void OutputBindings::Bind(std::size_t pos, double& val) { BindValue(pos, GetNativeType(val), &val); }
void OutputBindings::Bind(std::size_t pos, O<double>& val) { BindPrimitiveOptional(pos, val); }

void OutputBindings::Bind(std::size_t pos, io::DecimalWrapper& val) { BindDecimal(pos, val); }
void OutputBindings::Bind(std::size_t pos, O<io::DecimalWrapper>& val) { BindOptionalDecimal(pos, val); }

void OutputBindings::Bind(std::size_t pos, std::string& val) { BindString(pos, val); }
void OutputBindings::Bind(std::size_t pos, O<std::string>& val) { BindOptionalString(pos, val); }

void OutputBindings::Bind(std::size_t pos, formats::json::Value& val) { BindJson(pos, val); }
void OutputBindings::Bind(std::size_t pos, O<formats::json::Value>& val) { BindOptionalJson(pos, val); }

void OutputBindings::Bind(std::size_t pos, std::chrono::system_clock::time_point& val) { BindTimePoint(pos, val); }
void OutputBindings::Bind(std::size_t pos, O<std::chrono::system_clock::time_point>& val) {
    BindOptionalTimePoint(pos, val);
}
void OutputBindings::Bind(std::size_t pos, Date& val) { BindDate(pos, val); }
void OutputBindings::Bind(std::size_t pos, O<Date>& val) { BindOptionalDate(pos, val); }
void OutputBindings::Bind(std::size_t pos, DateTime& val) { BindDateTime(pos, val); }
void OutputBindings::Bind(std::size_t pos, O<DateTime>& val) { BindOptionalDateTime(pos, val); }

void OutputBindings::BindValue(std::size_t pos, enum_field_types type, void* buffer, bool is_unsigned) {
    auto& bind = GetBind(pos);
...
void OutputBindings::BindTimePoint(std::size_t pos, std::chrono::system_clock::time_point& val) {
...
void OutputBindings::TimepointAfterFetch(void* value, MYSQL_BIND&, FieldIntermediateBuffer& buffer) {
...
void OutputBindings::BindOptionalTimePoint(std::size_t pos, std::optional<std::chrono::system_clock::time_point>& val) {
    UASSERT(!val.has_value());
...
void OutputBindings::OptionalTimePointAfterFetch(void* value, MYSQL_BIND& bind, FieldIntermediateBuffer& buffer) {
    auto* optional = static_cast<std::optional<std::chrono::system_clock::time_point>*>(value);
    UASSERT(optional);
...
void OutputBindings::DateAfterFetch(void* value, MYSQL_BIND&, FieldIntermediateBuffer& buffer) {
...
void OutputBindings::BindOptionalDate(std::size_t pos, std::optional<Date>& val) {
    UASSERT(!val.has_value());
...
void OutputBindings::OptionalDateAfterFetch(void* value, MYSQL_BIND& bind, FieldIntermediateBuffer& buffer) {
...
void OutputBindings::DateTimeAfterFetch(void* value, MYSQL_BIND&, FieldIntermediateBuffer& buffer) {
...
void OutputBindings::OptionalDateTimeAfterFetch(void* value, MYSQL_BIND& bind, FieldIntermediateBuffer& buffer) {
...
void OutputBindings::BindOptionalJson(std::size_t pos, std::optional<formats::json::Value>& val) {
    UASSERT(!val.has_value());
...
void OutputBindings::OptionalJsonAfterFetch(void* value, MYSQL_BIND& bind, FieldIntermediateBuffer& buffer) {
...
void OutputBindings::BindOptionalDecimal(std::size_t pos, O<io::DecimalWrapper>& val) {
...
void OutputBindings::OptionalDecimalAfterFetch(void* value, MYSQL_BIND& bind, FieldIntermediateBuffer& buffer) {
...
void OutputBindings::ValidateAgainstStatement(MYSQL_STMT& statement) {
    ...
    UINVARIANT(fields_count == Size(), fmt::format("Fields count mismatch: expected {}, got {}", fields_count, Size()));
    if (fields_count == 0) {
        return;
    }
...
void OutputBindings::ValidateBind(std::size_t pos, const MYSQL_BIND& bind, const MYSQL_FIELD& field) {
    const auto error_for_column = [&field, pos](std::string_view error, const auto&... args) {
        return fmt::format(
            "Field type mismatch for column '{}' ({}-th column): {}",
            field.name,
            pos,
    ...
    UINVARIANT(
        IsBindable(bind.buffer_type, field.type),
        error_for_column("expected {}, got {}", NativeTypeToString(bind.buffer_type), NativeTypeToString(field.type))
    );
     ...
     */
    UINVARIANT(
        !(is_field_nullable && !is_bind_nullable),
        error_for_column("field is nullable in DB but not-nullable in client code")
    );
    ...
    if (IsFieldNumeric(field.type)) {
        ...
        UINVARIANT(
            is_bind_signed == is_field_signed,
            error_for_column(
                "field is {} in DB but {} in client code",
