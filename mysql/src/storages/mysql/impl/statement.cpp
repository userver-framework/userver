...
void Statement::NativeStatementDeleter::operator()(MYSQL_STMT* statement) const noexcept {
...
StatementFetcher Statement::Execute(io::ParamsBinderBase& params, engine::Deadline deadline) {
    const USERVER_NAMESPACE::tracing::ScopeTime execute{impl::tracing::kExecuteScope};
    UpdateParamsBindings(params);

    const int err = NativeInterface{connection_->GetSocket(), deadline}.StatementExecute(native_statement_.get());
...
void Statement::StoreResult(engine::Deadline deadline) {
    const int err = NativeInterface{connection_->GetSocket(), deadline}.StatementStoreResult(native_statement_.get());
...
bool Statement::FetchResultRow(bindings::OutputBindings& binds, bool apply_binds, engine::Deadline deadline) {
    ...
    const int fetch_err = NativeInterface{connection_->GetSocket(), deadline}.StatementFetch(native_statement_.get());
    ...
    for (std::size_t i = 0; i < binds.Size(); ++i) {
        ...
        if (bind.error_value) {
            ...
            const int err = mysql_stmt_fetch_column(native_statement_.get(), &bind, i, 0);
            if (err != 0) {
                throw std::runtime_error{GetNativeError("Failed to fetch a column from statement result row")};
            }
        }
...
void Statement::Reset(engine::Deadline deadline) {
    ...
    if (batch_size_.has_value()) {
        ...
        if (connection_->GetServerInfo().server_type == metadata::ServerInfo::Type::kMySQL) {
...
void Statement::UpdateParamsBindings(io::ParamsBinderBase& params) {
    ...
    if (!binds.Empty()) {
        ...
        if (auto rows_count = params.GetRowsCount(); rows_count > 1) {
            ...
            mysql_stmt_attr_set(native_statement_.get(), STMT_ATTR_ARRAY_SIZE, &rows_count);
        }

        if (auto* user_data = binds.GetUserData()) {
            mysql_stmt_attr_set(native_statement_.get(), STMT_ATTR_CB_USER_DATA, user_data);
        }
        if (auto* param_cb = binds.GetParamsCallback()) {
            mysql_stmt_attr_set(native_statement_.get(), STMT_ATTR_CB_PARAM, reinterpret_cast<void*>(param_cb));
        }
    }
}

std::size_t Statement::RowsCount() const { return mysql_stmt_num_rows(native_statement_.get()); }

std::size_t Statement::ParamsCount() { return mysql_stmt_param_count(native_statement_.get()); }

std::size_t Statement::ResultColumnsCount() { return mysql_stmt_field_count(native_statement_.get()); }

void Statement::SetReadonlyCursor(std::size_t batch_size) {
    std::size_t cursor_type = CURSOR_TYPE_READ_ONLY;
    mysql_stmt_attr_set(native_statement_.get(), STMT_ATTR_CURSOR_TYPE, &cursor_type);
    mysql_stmt_attr_set(native_statement_.get(), STMT_ATTR_PREFETCH_ROWS, &batch_size);
...
std::optional<std::size_t> Statement::GetBatchSize() const { return batch_size_; }
...
bool StatementFetcher::FetchResult(io::ExtractorBase& extractor) {
    ...
    return guard.Execute([&] {
        ...
        for (size_t i = 0; i < rows_count; ++i) {
            ...
            // wide select.
            const auto parsed = statement_->FetchResultRow(binds, apply_binds, parent_statement_deadline_);
            if (!parsed) {
                extractor.RollbackLastRow();
                return false;
            }
...
std::uint64_t StatementFetcher::RowsAffected() const {
    const auto rows_affected = mysql_stmt_affected_rows(statement_->native_statement_.get());

    // libmariadb return -1 as ulonglong
    if (rows_affected == std::numeric_limits<std::uint64_t>::max()) {
        LOG_WARNING() << "RowsAffected called on a statement that doesn't affect any rows";
        return 0;
    }
