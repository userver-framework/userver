...
[[maybe_unused]] [[noreturn]] void AbortOnBuggyLibmariadb(metadata::SemVer client_version_id) {
...
void SetNativeOptions(MYSQL* mysql, const settings::ConnectionSettings& connection_settings) {
    UASSERT(mysql);
    ...
    // introduce ourselves
    mysql_optionsv(mysql, MYSQL_OPT_CONNECT_ATTR_ADD, "client_name", "userver-mysql");
...
void Connection::Ping(engine::Deadline deadline) {
    ...
    guard.Execute([this, deadline] {
        ...
        if (err != 0) {
            throw MySQLException{mysql_errno(&mysql_), GetNativeError("Failed to ping the server")};
        }
    });
}
...
void Connection::Commit(engine::Deadline deadline) {
    ...
    guard.Execute([this, deadline] {
        ...
        if (err != 0) {
            throw MySQLTransactionException{mysql_errno(&mysql_), GetNativeError("Failed to commit a transaction")};
        }
    });
}
...
void Connection::Rollback(engine::Deadline deadline) {
    ...
    guard.Execute([this, deadline] {
        ...
        if (err != 0) {
            throw MySQLTransactionException{mysql_errno(&mysql_), GetNativeError("Failed to rollback a transaction")};
        }
    });
}
...
const metadata::ServerInfo& Connection::GetServerInfo() const { return server_info_; }
...
void Connection::InitSocket(
    ...
    for (const auto& addr : addr_vec) {
        ...
        if (DoInitSocket(ip, endpoint_info.port, auth_settings, connection_settings, deadline)) {
            return;
    ...
    throw MySQLException{0, "Failed to connect to any of the the resolved addresses"};
}
...
bool Connection::DoInitSocket(
    ...
    auto mysql_events = mysql_real_connect_start(
        ...
        &mysql_,
        ip.c_str(),
        auth_settings.user.c_str(),
        auth_settings.password.GetUnderlying().c_str(),
        auth_settings.database.c_str(),
        port,
        nullptr /* unix_socket */,
        0 /* client_flags */
    );
    ...
    try {
        socket_.RunToCompletion(
            [mysql_events] { return mysql_events; },
            [this, &connect_res](int mysql_events) {
                return mysql_real_connect_cont(&connect_res, &mysql_, mysql_events);
            },
            deadline
        );
    ...
    if (!connect_res) {
        const auto client_version_id = metadata::NativeClientInfo::Get().client_version_id;
...
Statement& Connection::PrepareStatement(
    ...
    auto& mysql_statement = statements_cache_.PrepareStatement(statement, deadline);
