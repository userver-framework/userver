...
MySQLNativeInterface::MySQLNativeInterface(MySQLSocket& socket,
                                           engine::Deadline deadline)
    : socket_{socket}, deadline_{deadline} {}

int MySQLNativeInterface::StatementPrepare(MYSQL_STMT* stmt,
                                           const char* stmt_str,
                                           std::size_t length) && {
  int err = 0;
...
int MySQLNativeInterface::StatementExecute(MYSQL_STMT* stmt) && {
  int err = 0;
  socket_.RunToCompletion(
...
int MySQLNativeInterface::StatementStoreResult(MYSQL_STMT* stmt) && {
  int err = 0;
  socket_.RunToCompletion(
...
int MySQLNativeInterface::StatementFetch(MYSQL_STMT* stmt) && {
  int err = 0;
  socket_.RunToCompletion(
...
my_bool MySQLNativeInterface::StatementFreeResult(MYSQL_STMT* stmt) && {
  my_bool err{};
  socket_.RunToCompletion(
...
my_bool MySQLNativeInterface::StatementClose(MYSQL_STMT* stmt) && {
  my_bool err{};
  socket_.RunToCompletion(
...
int MySQLNativeInterface::QueryExecute(MYSQL* mysql, const char* stmt_str,
                                       std::size_t length) && {
  int err = 0;
  socket_.RunToCompletion(
...
void MySQLNativeInterface::QueryFreeResult(MYSQL_RES* result) && {
...
int MySQLNativeInterface::Ping(MYSQL* mysql) && {
  int err = 0;
  socket_.RunToCompletion(
...
my_bool MySQLNativeInterface::Commit(MYSQL* mysql) && {
  my_bool err = 0;
  socket_.RunToCompletion(
...
my_bool MySQLNativeInterface::Rollback(MYSQL* mysql) && {
  my_bool err = 0;
  socket_.RunToCompletion(
...
void MySQLNativeInterface::Close(MYSQL* mysql) && {
  try {
