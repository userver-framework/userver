...
std::vector<std::string> ParseHosts(const formats::json::Value& database) {
    ...
    UINVARIANT(unique_count == hosts.size(), "Hosts are not unique in mysql secdist");
...
AuthSettings::~AuthSettings() { std::memset(password.GetUnderlying().data(), 0, password.size()); }
...
ConnectionSettings Parse(const yaml_config::YamlConfig& doc, formats::parse::To<ConnectionSettings>) {
...
PoolSettings PoolSettings::Create(
    const components::ComponentConfig& config,
    const EndpointInfo& endpoint_info,
    const AuthSettings& auth_settings
) {
    PoolSettings settings{};
    settings.initial_pool_size = config["initial_pool_size"].As<std::size_t>(settings.initial_pool_size);
    settings.max_pool_size = config["max_pool_size"].As<std::size_t>(settings.max_pool_size);
...
MysqlSettings Parse(const formats::json::Value& value, formats::parse::To<MysqlSettings>) {
...
MysqlSettingsMulti::MysqlSettingsMulti(const formats::json::Value& secdist) {
    ...
    databases_ = mysql_settings.As<std::unordered_map<std::string, MysqlSettings>>();
}
