...
engine::Deadline GetDeadline(OptionalCommandControl optional_cc, CommandControl default_cc) {
    const auto timeout = optional_cc.has_value() ? optional_cc->execute : default_cc.execute;
...
std::unique_ptr<infra::topology::TopologyBase> CreateTopology(
    ...
    for (const auto& endpoint_info : settings.endpoints) {
        pools_settings.push_back(settings::PoolSettings::Create(config, endpoint_info, settings.auth));
    }
...
Transaction Cluster::Begin(ClusterHostType host_type) const { return Cluster::Begin(std::nullopt, host_type); }

Transaction Cluster::Begin(OptionalCommandControl command_control, ClusterHostType host_type) const {
    const auto deadline = GetDeadline(command_control, GetDefaultCommandControl());

    return Transaction{topology_->SelectPool(host_type).Acquire(deadline), deadline};
}

CommandResultSet Cluster::ExecuteCommand(ClusterHostType host_type, const Query& command) const {
...
CommandControl Cluster::GetDefaultCommandControl() { return CommandControl{kDefaultStatementTimeout}; }
