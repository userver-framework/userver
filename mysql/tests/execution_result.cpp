...
struct DbRowIdValue final {
    ...
    bool operator==(const DbRowIdValue& other) const { return id == other.id && value == other.value; }
};

DbRow Convert(std::int32_t id, storages::mysql::convert::To<DbRow>) { return {id}; }

}  // namespace

UTEST(ExecutionResult, SingleInsert) {
    TmpTable table{"Id INT NOT NULL"};
    const auto res = table.DefaultExecute("INSERT INTO {} VALUES(?)", 1).AsExecutionResult();
...
UTEST(ExecutionResult, SingleUpdate) {
    TmpTable table{"Id INT NOT NULL AUTO_INCREMENT, Value TEXT NOT NULL, PRIMARY KEY(Id)"};
    const auto insert_res = table.DefaultExecute("INSERT INTO {}(Value) VALUES(?)", "text").AsExecutionResult();
    ...
    const auto no_update_res_2 =
        table.DefaultExecute("UPDATE {} SET Value = ? WHERE ID = ?", "other text", 1).AsExecutionResult();
...
UTEST(ExecutionResult, BatchUpdate) {
    ClusterWrapper cluster{};
    TmpTable table{cluster, "Id INT NOT NULL AUTO_INCREMENT, Value TEXT NOT NULL, PRIMARY KEY(Id)"};
    ...
    const auto update_res = cluster->ExecuteBulk(ClusterHostType::kPrimary, upsert_query, db_rows).AsExecutionResult();
    EXPECT_EQ(update_res.rows_affected, kRowsCount * 2);
    EXPECT_EQ(update_res.last_insert_id, kRowsCount);

    EXPECT_EQ(db_rows, table.DefaultExecute("SELECT Id, Value FROM {}").AsVector<DbRowIdValue>());
}

UTEST(ExecutionResult, WithSelect) {
    TmpTable table{"Id INT, Value TEXT"};
    const auto res = table.DefaultExecute("SELECT Id, Value FROM {}").AsExecutionResult();
