...
UserRow Convert(DbRow&& db_row, To<UserRow>) { return {static_cast<std::int64_t>(db_row.a) * db_row.b}; }
...
UTEST(MappedResultSet, Works) {
    ...
    const auto db_row = table.DefaultExecute("SELECT a, b FROM {}").MapFrom<DbRow>().AsVector<UserRow>();
...
UTEST(MappedResultSet, FieldWorks) {
    ...
    const auto container = table.DefaultExecute("SELECT text FROM {}").AsVector<std::string>(kFieldTag);
...
std::string Convert(DbUser&& db_user, storages::mysql::convert::To<std::string>) {
...
UTEST(MappedResultSet, MappedFieldWorks) {
    ...
    const auto as_int = table.DefaultExecute("SELECT Value FROM {}").MapFrom<std::string>().AsVector<int>(kFieldTag);
...
UTEST(ResultSet, UnmappedFieldWorks) {
    ClusterWrapper cluster{};
    const auto res = cluster.DefaultExecute("SELECT 'field'").AsSingleField<std::string>();
    EXPECT_EQ(res, "field");
}
...
struct IdAndValue final {
    ...
    bool operator==(const IdAndValue& other) const noexcept { return id == other.id && value == other.value; }
};

std::pair<const std::int32_t, IdAndValue>
Convert(IdAndValue&& db_value, storages::mysql::convert::To<std::pair<const std::int32_t, IdAndValue>>) {
...
UTEST(MappedResultSet, WorksForUnorderedMap) {
    ...
    const auto container = table.DefaultExecute("SELECT Id, Value FROM {}").MapFrom<IdAndValue>().AsContainer<Map>();
