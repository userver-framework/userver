...
UTEST(Cluster, InsertMany) {
    ...
    const std::string long_string_to_avoid_sso{"hi i am some long string that doesn't fit in sso"};
    ...
    for (int i = 0; i < kRowsCount; ++i) {
        rows_to_insert.push_back({i, fmt::format("{}: {}", i, long_string_to_avoid_sso)});
    }
...
UTEST(ShowCase, Basic) {
    ...
    TmpTable table{cluster, "Id INT NOT NULL, Value TEXT NOT NULL, CreatedAt DATETIME(6) NOT NULL"};
    ...
    const std::vector<Row> rows =
        cluster
            ->Execute(
                ClusterHostType::kPrimary,
                table.FormatWithTableName("SELECT Id, Value, CreatedAt FROM {} WHERE CreatedAt > ?"),
                std::chrono::system_clock::now() - std::chrono::hours{3}
            )
            .AsVector<Row>();

    table.DefaultExecute("INSERT INTO {} VALUES(?, ?, ?)", 1, "we", std::chrono::system_clock::now());
...
UTEST(ShowCase, BatchInsert) {
    ...
    rows_to_insert.reserve(kRowsCount);
    for (int i = 0; i < kRowsCount; ++i) {
        rows_to_insert.push_back({i, "i am just some random string, don't mind me"});
    }
...
DbRow Convert(const UserRow& user_row, storages::mysql::convert::To<DbRow>) {
    return DbRow{user_row.id, fmt::format("{} {}", user_row.first_name, user_row.last_name)};
}
...
void PerformGetCursor(const Cluster& cluster, std::chrono::milliseconds timeout) {
