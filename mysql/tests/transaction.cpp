...
struct Row final {
    ...
    bool operator==(const Row& other) const { return id == other.id && value == other.value; }
};

struct TableMeta final {
    static TableMeta Create() { return TableMeta{}; };

    std::string GetInsertQuery() const { return table.FormatWithTableName("INSERT INTO {} VALUES (?, ?)"); }

    std::string GetSelectQuery() const { return table.FormatWithTableName(GetDefaultSelectQuery()); }

    static std::string GetDefaultSelectQuery() { return "SELECT Id, Value FROM {}"; };
...
UTEST(Transaction, Commit) {
    ...
    EXPECT_NO_THROW(meta.table.DefaultExecute(TableMeta::GetDefaultSelectQuery()).AsSingleRow<Row>());
}
...
UTEST(Transaction, Rollback) {
...

    {
        auto transaction = meta.table.Begin();
        transaction.Execute(meta.GetInsertQuery(), 1, "some text");
        EXPECT_NO_THROW(transaction.Execute(meta.GetSelectQuery()).AsSingleRow<Row>());
        transaction.Rollback();
    }

    auto row_opt = meta.table.DefaultExecute(TableMeta::GetDefaultSelectQuery()).AsOptionalSingleRow<Row>();
...
UTEST(Transaction, ActuallyTransactional) {
...

    {
        ...
        insert_transaction.Execute(meta.GetInsertQuery(), 5, "i am not seen by another transaction");

        const auto select = [&meta] {
            auto transaction = meta.table.Begin();
            return transaction.Execute(meta.GetSelectQuery()).AsOptionalSingleRow<Row>();
        };
...
UTEST(Transaction, InsertOne) {
...

    {
        ...
        EXPECT_EQ(transaction.Execute(meta.GetSelectQuery()).AsSingleRow<Row>(), row);

        transaction.Rollback();
        EXPECT_FALSE(meta.table.DefaultExecute(meta.GetDefaultSelectQuery()).AsOptionalSingleRow<Row>().has_value());
    }
}
