...
void PrepareSampleTable(const Cluster& cluster) {
    ...
    cluster.ExecuteCommand(ClusterHostType::kPrimary, "CREATE TABLE SampleTable(id INT NOT NULL, value TEXT NOT NULL)");
}
...
struct SampleRow final {
    ...
    bool operator==(const SampleRow& other) const { return id == other.id && value == other.value; }
};
...
void PerformAsVectorFieldTag(const Cluster& cluster) {
    const auto vector_of_field_values =
        cluster.Execute(ClusterHostType::kPrimary, "SELECT value FROM SampleTable").AsVector<std::string>(kFieldTag);

    static_assert(std::is_same_v<const std::vector<std::string>, decltype(vector_of_field_values)>);
}
/// [uMySQL usage sample - StatementResultSet AsVectorFieldTag]
...
void PerformAsSingleRow(const Cluster& cluster, const SampleRow& data) {
    cluster.ExecuteDecompose(ClusterHostType::kPrimary, "INSERT INTO SampleTable(id, value) VALUES (?, ?)", data);

    const auto db_row =
        cluster.Execute(ClusterHostType::kPrimary, "SELECT id, value FROM SampleTable").AsSingleRow<SampleRow>();
...
void PerformAsSingleField(const Cluster& cluster) {
    ...
    const auto value =
        cluster.Execute(ClusterHostType::kPrimary, "SELECT value FROM SampleTable").AsSingleField<std::string>();
...
struct SampleRow final {
    ...
    bool operator<(const SampleRow& other) const { return id < other.id || (id == other.id && value < other.value); }
    bool operator==(const SampleRow& other) const { return id == other.id && value == other.value; }
};
...
std::pair<const std::int32_t, std::string>
Convert(SampleRow&& db_row, storages::mysql::convert::To<std::pair<const std::int32_t, std::string>>) {
