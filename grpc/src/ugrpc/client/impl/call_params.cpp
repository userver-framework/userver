...
void ApplyQosConfigs(const ClientData& client_data,
                     grpc::ClientContext& client_context, const Qos& user_qos,
  ...
  if (user_qos.timeout) {
    ...
    SetTimeout(client_context, user_qos.timeout,
               client_data.GetTestsuiteControl());
    return;
  }

  if (client_context.deadline() !=
  ...
  if (const auto* const config_key = client_data.GetClientQos()) {
    ...
    if (const auto dynamic_qos = config[*config_key].GetOptional(call_name)) {
      SetTimeout(client_context, dynamic_qos->timeout,
                 client_data.GetTestsuiteControl());
    }
  }
}
