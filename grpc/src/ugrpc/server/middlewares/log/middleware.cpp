...
std::string ProtobufAsJsonString(const google::protobuf::Message& message, uint64_t max_msg_size) {
    ...
    const auto status = google::protobuf::util::MessageToJsonString(*trimmed, &as_json, options);
    if (!status.ok()) {
