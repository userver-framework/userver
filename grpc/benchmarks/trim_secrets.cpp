    ...
    for (auto _ : state) {
        {
            ...
            const auto status = google::protobuf::util::MessageToJsonString(request, &request_json, options);
        ...
        }
        {
            ...
            const auto status = google::protobuf::util::MessageToJsonString(response, &response_json, options);
            UINVARIANT(status.ok(), "Convert to JSON failed");
