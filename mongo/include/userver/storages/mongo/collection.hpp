...
template <typename... Options>
size_t Collection::Count(formats::bson::Document filter, Options&&... options) const {
...
using HasOptionHelper = std::disjunction<std::is_same<std::decay_t<Options>, Option>...>;
...
template <typename... Options>
WriteResult Collection::InsertOne(formats::bson::Document document, Options&&... options) {
...
template <typename... Options>
WriteResult Collection::InsertMany(std::vector<formats::bson::Document> documents, Options&&... options) {
...
template <typename... Options>
WriteResult Collection::DeleteOne(formats::bson::Document selector, Options&&... options) {
    operations::Delete delete_op(operations::Delete::Mode::kSingle, std::move(selector));
    (delete_op.SetOption(std::forward<Options>(options)), ...);
    return Execute(delete_op);
}

template <typename... Options>
WriteResult Collection::DeleteMany(formats::bson::Document selector, Options&&... options) {
    operations::Delete delete_op(operations::Delete::Mode::kMulti, std::move(selector));
    (delete_op.SetOption(std::forward<Options>(options)), ...);
    return Execute(delete_op);
}
...
template <typename... Options>
WriteResult Collection::FindAndRemove(formats::bson::Document query, Options&&... options) {
