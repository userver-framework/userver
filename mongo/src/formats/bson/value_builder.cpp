...
ValueBuilder ValueBuilder::operator[](const std::string& name) { return ValueBuilder(impl_->GetOrInsert(name)); }

ValueBuilder ValueBuilder::operator[](uint32_t index) { return ValueBuilder((*impl_)[index]); }
...
ValueBuilder::iterator ValueBuilder::begin() { return {*impl_, impl_->Begin()}; }
...
Value ValueBuilder::ExtractValue() {
    impl_->SyncBsonValue();
    return Value(std::exchange(impl_, std::make_shared<impl::ValueImpl>(nullptr)));
}
