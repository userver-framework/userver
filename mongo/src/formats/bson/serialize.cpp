...
json::Value Convert(const bson::Value& bson, parse::To<json::Value>) {
    formats::common::ConversionStack<bson::Value, json::ValueBuilder> conversion_stack(bson);
    while (!conversion_stack.IsParsed()) {
        ...
        if (from.IsBool()) {
            ...
            conversion_stack.CastCurrentPrimitive<std::chrono::system_clock::time_point>();
        } else if (from.IsOid()) {
...
bson::Value Convert(const json::Value& json, parse::To<bson::Value>) {
    if (json.IsMissing()) {
        return bson::ValueBuilder{common::Type::kNull}.ExtractValue();
    }
...
JsonString ApplyConversionToString(const impl::BsonHolder& bson, char* (*conversion)(const bson_t*, size_t*)) {
    size_t size = 0;
...
impl::BsonHolder DoParseJsonString(std::string_view json) {
    bson_error_t error;
    auto bson = impl::MutableBson::AdoptNative(
...
Value ArrayFromJsonString(std::string_view json) {
    ...
    auto value_impl = std::make_shared<impl::ValueImpl>(DoParseJsonString(json), impl::ValueImpl::DocumentKind::kArray);
...
JsonString ToCanonicalJsonString(const formats::bson::Document& doc) {
    return ApplyConversionToString(doc.GetBson(), &bson_as_canonical_extended_json);
}
...
std::string_view JsonString::GetView() const { return {impl_->Data(), impl_->Size()}; }

const char* JsonString::Data() const { return impl_->Data(); }
size_t JsonString::Size() const { return impl_->Size(); }

std::ostream& operator<<(std::ostream& os, const JsonString& json) { return os.write(json.Data(), json.Size()); }
