...
Oid::Oid(std::string_view hex_encoded) {
    if (!bson_oid_is_valid(hex_encoded.data(), hex_encoded.size())) {
        throw BsonException(fmt::format("Invalid hex-encoded ObjectId: '{}'", hex_encoded));
    }
...
Oid Oid::MakeMinimalFor(std::chrono::system_clock::time_point time) {
    ...
    if (epoch != time_as_time_t) {
        throw BsonException(fmt::format(
            "Can not represent time point {} as std::time_t because "
            "narrowing happens",
...
Decimal128::Decimal128(const std::string& value) {
    ...
    if (!bson_decimal128_from_string_w_len(value.data(), value.size(), &decimal_)) {
...
bool Decimal128::operator!=(const Decimal128& rhs) const { return !(*this == rhs); }
...
uint64_t Timestamp::Pack() const { return (static_cast<uint64_t>(GetTimestamp()) << 32) | GetIncrement(); }

Timestamp Timestamp::Unpack(uint64_t packed) {
    return {static_cast<uint32_t>(packed >> 32), static_cast<uint32_t>(packed & ((std::uint64_t{1} << 32) - 1))};
}
...
bool Timestamp::operator<(const Timestamp& rhs) const {
    return timestamp_ < rhs.timestamp_ || (timestamp_ == rhs.timestamp_ && increment_ < rhs.increment_);
}
...
size_t hash<USERVER_NAMESPACE::formats::bson::Oid>::operator()(const USERVER_NAMESPACE::formats::bson::Oid& oid) const {
    return bson_oid_hash(&oid.oid_);
}

size_t hash<USERVER_NAMESPACE::formats::bson::Timestamp>::operator()(
    const USERVER_NAMESPACE::formats::bson::Timestamp& timestamp
) const {
