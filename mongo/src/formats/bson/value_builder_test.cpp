...
static_assert(
    std::is_assignable_v<decltype(*std::declval<fb::ValueBuilder>().begin()), fb::ValueBuilder>,
...
TEST(BsonValueBuilder, Oid) {
    ...
    const auto tp_before = std::chrono::system_clock::now() - std::chrono::minutes{1};
    auto new_val1 = fb::ValueBuilder(fb::Oid{}).ExtractValue();
    auto new_val2 = fb::ValueBuilder(fb::Oid{}).ExtractValue();
    const auto tp_after = std::chrono::system_clock::now() + std::chrono::minutes{1};
...
TEST(BsonValueBuilder, MutateDocument) {
    static const auto kDoc = fb::MakeDoc(
        "arr",
        fb::MakeArray(1, "elem", fb::MinKey{}),  //
        "doc",
        fb::MakeDoc("b", true, "i", 0, "d", -1.25)
    );
...
TEST(BsonValueBuilder, PredefType) {
...

    {
        ...
        UEXPECT_THROW(fb::Document(arr_builder.ExtractValue()), fb::TypeMismatchException);
    }
}
...
template <>
struct InstantiationDeathTest<formats::bson::ValueBuilder> : public ::testing::Test {
    using ValueBuilder = formats::bson::ValueBuilder;

    using Exception = formats::bson::BsonException;
};

INSTANTIATE_TYPED_TEST_SUITE_P(FormatsBson, InstantiationDeathTest, formats::bson::ValueBuilder);
INSTANTIATE_TYPED_TEST_SUITE_P(FormatsBson, CommonValueBuilderTests, formats::bson::ValueBuilder);
...
formats::bson::Value Serialize(const MyKeyValue& data, formats::serialize::To<formats::bson::Value>) {
    formats::bson::ValueBuilder builder;
