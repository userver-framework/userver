...
const auto kDuplicateFieldsDoc = fb::MakeDoc("a", "first", "b", "other", "a", "second", "a", "third", "c", "end");

}  // namespace

static_assert(
    !std::is_assignable_v<decltype(*std::declval<fb::Value>().begin()), fb::Value>,
...
TEST(BsonValue, Default) {
    UEXPECT_THROW(kDoc["nonexistent"].As<std::string>(), fb::MemberMissingException);
...
TEST(BsonValue, DuplicateFieldsUseFirst) {
    ...
    doc_use_first.SetDuplicateFieldsPolicy(fb::Value::DuplicateFieldsPolicy::kUseFirst);
...
TEST(BsonValue, DuplicateFieldsUseLast) {
    ...
    doc_use_last.SetDuplicateFieldsPolicy(fb::Value::DuplicateFieldsPolicy::kUseLast);
