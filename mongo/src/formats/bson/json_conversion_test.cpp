...
const auto kTimePoint = std::chrono::system_clock::from_time_t(0) + std::chrono::milliseconds{1};
...
const auto kSimpleDoc = fb::MakeDoc(
    "double",
    1.1,
    "int",
    int64_t{1},
...
std::optional<std::string> ComparePrimitivesWithReason(const fb::Value& arg, const fb::Value& expected) {
    if (expected.IsArray()) {
    ...
    if (expected.IsDateTime()) {
        ...
        if (expected.As<std::chrono::system_clock::time_point>() != arg.As<std::chrono::system_clock::time_point>()) {
            return "Primitive argument value mismatch";
        }
    }
    if (expected.IsOid()) {
...
std::optional<std::string> CompareDocsWithReason(const fb::Value& arg, const fb::Value& expected) {
...
MATCHER(BsonPrimitiveMatcherFromJson, "Matcher for converting primitive from json to bson") {
...
MATCHER(JsonPrimitiveMatcherFromBson, "Matcher for converting primitive from bson to json") {
    auto b = fb::ValueBuilder(arg).ExtractValue();
    auto j = fj::ValueBuilder(arg).ExtractValue();
...
MATCHER(
    JsonPrimitiveToStringMatcherFromBson,
...
TEST(Convert, PrimitiveToJson) {
    auto tp = std::chrono::system_clock::time_point{std::chrono::milliseconds{1554138241}};
    ...
    EXPECT_THAT(fb::Oid::MakeMinimalFor(tp), JsonPrimitiveToStringMatcherFromBsonMatcher());
...
TEST(Convert, PrimitiveFromJson) {
    ...
    {
        auto value = std::chrono::system_clock::time_point{std::chrono::milliseconds{1554138241}};
