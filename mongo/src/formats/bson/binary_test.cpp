...
TEST(Binary, Empty) {
    EXPECT_EQ(kEmptyDocBinary, formats::bson::ToBinaryString(kEmptyDoc).ToString());
...
TEST(Binary, Invalid) {
    ...
    UEXPECT_THROW(formats::bson::FromBinaryString(std::string{}), formats::bson::ParseException);
    UEXPECT_THROW(formats::bson::FromBinaryString(std::string{'\x00'}), formats::bson::ParseException);
    UEXPECT_THROW(formats::bson::FromBinaryString(std::string{'\x01'}), formats::bson::ParseException);
    UEXPECT_THROW(formats::bson::FromBinaryString(std::string{4, 0, 0, 0}), formats::bson::ParseException);
    UEXPECT_THROW(formats::bson::FromBinaryString(std::string{5, 0, 0, 0}), formats::bson::ParseException);

    // Invalid size
    UEXPECT_THROW(
        formats::bson::FromBinaryString(std::string{5, 0, 0, 0, 0x0A, 't', 'e', 's', 't', '\0', 0x00}),
    ...
    UEXPECT_THROW(
        formats::bson::FromBinaryString(std::string{12, 0, 0, 0, 0x08, 't', 'e', 's', 't', '\0', 2, 0x00}),
        formats::bson::ParseException
    );

#ifndef USERVER_FEATURE_NO_PATCHED_BSON
    // Trailing data
    UEXPECT_THROW(formats::bson::FromBinaryString(std::string{6, 0, 0, 0, 0x00, 0x00}), formats::bson::ParseException);
