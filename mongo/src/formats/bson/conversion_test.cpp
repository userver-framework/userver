...
TEST(BsonConversion, Double) {
    ...
    const auto doc = fb::MakeDoc("a", fb::MakeArray(0.0, 0.123, -0.123), "ez", 0.0, "ep", 3.21, "en", -3.21);
...
TEST(BsonConversion, Utf8) {
    ...
    const auto doc = fb::MakeDoc("a", fb::MakeArray("", "test"), "ee", "", "e", "string");
    test_elem(doc["a"][0], {});
...
TEST(BsonConversion, DateTime) {
    auto test_elem = [](const fb::Value& elem, const std::chrono::system_clock::time_point& value, bool is_int32) {
        EXPECT_FALSE(elem.IsMissing());
        EXPECT_FALSE(elem.IsArray());
        EXPECT_FALSE(elem.IsDocument());
        EXPECT_FALSE(elem.IsNull());
        ...
        const auto int_value = std::chrono::duration_cast<std::chrono::milliseconds>(value.time_since_epoch()).count();
    ...
    const auto doc = fb::MakeDoc("a", fb::MakeArray(time1), "e", time2, "s", time3);
...
TEST(BsonConversion, Containers) {
    ...
    UEXPECT_THROW((doc["a"].ConvertTo<std::unordered_map<std::string, int>>()), fb::TypeMismatchException);
    UEXPECT_THROW(doc["d"].ConvertTo<std::vector<int>>(), fb::TypeMismatchException);
    ...
    EXPECT_TRUE((doc["n"].ConvertTo<std::unordered_map<std::string, int>>().empty()));
