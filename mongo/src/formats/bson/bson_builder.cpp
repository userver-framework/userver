...
BsonBuilder& BsonBuilder::Append(std::string_view key, unsigned long long value) {
    bson_append_int64(bson_->Get(), key.data(), key.size(), impl::ToInt64(value));
    return *this;
}
...
BsonBuilder& BsonBuilder::Append(std::string_view key, const Binary& value) {
    bson_append_binary(bson_->Get(), key.data(), key.size(), BSON_SUBTYPE_BINARY, value.Data(), value.Size());
    return *this;
}

BsonBuilder& BsonBuilder::Append(std::string_view key, const Decimal128& value) {
    bson_append_decimal128(bson_->Get(), key.data(), key.size(), value.GetNative());
    return *this;
}
...
BsonBuilder& BsonBuilder::Append(std::string_view key, const Timestamp& value) {
    bson_append_timestamp(bson_->Get(), key.data(), key.size(), value.GetTimestamp(), value.GetIncrement());
    return *this;
}

BsonBuilder& BsonBuilder::Append(std::string_view key, const Value& value) {
    value.impl_->CheckNotMissing();
    bson_append_value(bson_->Get(), key.data(), key.size(), value.impl_->GetNative());
    return *this;
}
