...
void AppendMaxServerTime(std::chrono::milliseconds& destination, const options::MaxServerTime& max_server_time) {
    UASSERT_MSG(destination == kNoMaxServerTime, "Duplicate options::MaxServerTime");
...
void VerifyMaxServerTime(std::chrono::milliseconds& max_server_time) {
    ...
    if (max_server_time < kNoMaxServerTime) {
        throw InvalidQueryArgumentException("Max server time of ") << max_server_time.count() << "ms is out of bounds";
    }
}
