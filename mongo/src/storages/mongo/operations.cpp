...
impl::cdriver::ReadPrefsPtr MakeCDriverReadPrefs(options::ReadPreference::Mode mode) {
    return impl::cdriver::ReadPrefsPtr(ToCDriverReadMode(mode));
}

ATTRIBUTE_NO_SANITIZE_UNDEFINED
impl::cdriver::ReadPrefsPtr MakeCDriverReadPrefs(const options::ReadPreference& option) {
    auto read_prefs = MakeCDriverReadPrefs(option.GetMode());

    const std::optional<std::chrono::seconds> max_staleness = option.GetMaxStaleness();
    if (max_staleness) {
        mongoc_read_prefs_set_max_staleness_seconds(read_prefs.Get(), max_staleness->count());
    }
    ...
    if (!mongoc_read_prefs_is_valid(read_prefs.Get())) {
        throw InvalidQueryArgumentException("Provided read preference is not valid");
    }
    return read_prefs;
}

void AppendReadConcern(formats::bson::impl::BsonBuilder& builder, options::ReadConcern level) {
...
void AppendWriteConcern(formats::bson::impl::BsonBuilder& builder, options::WriteConcern::Level level) {
    if (!mongoc_write_concern_append(impl::MakeCDriverWriteConcern(level).get(), builder.Get())) {
        throw MongoException("Cannot append write concern option");
    }
}

void AppendWriteConcern(formats::bson::impl::BsonBuilder& builder, const options::WriteConcern& wc_option) {
    if (!mongoc_write_concern_append(impl::MakeCDriverWriteConcern(wc_option).get(), builder.Get())) {
        throw MongoException("Cannot append write concern option");
    }
}
...
void Count::SetOption(options::ReadPreference::Mode mode) { impl_->read_prefs = MakeCDriverReadPrefs(mode); }

void Count::SetOption(options::ReadConcern level) { AppendReadConcern(impl::EnsureBuilder(impl_->options), level); }

void Count::SetOption(options::Skip skip) { AppendSkip(impl::EnsureBuilder(impl_->options), skip); }
...
void CountApprox::SetOption(options::ReadPreference::Mode mode) { impl_->read_prefs = MakeCDriverReadPrefs(mode); }

void CountApprox::SetOption(options::ReadConcern level) {
    AppendReadConcern(impl::EnsureBuilder(impl_->options), level);
}

void CountApprox::SetOption(options::Skip skip) { AppendSkip(impl::EnsureBuilder(impl_->options), skip); }

void CountApprox::SetOption(options::Limit limit) { AppendLimit(impl::EnsureBuilder(impl_->options), limit); }
...
void Find::SetOption(options::ReadPreference::Mode mode) { impl_->read_prefs = MakeCDriverReadPrefs(mode); }

void Find::SetOption(options::ReadConcern level) { AppendReadConcern(impl::EnsureBuilder(impl_->options), level); }
...
void Update::SetOption(options::RetryDuplicateKey) {
    if (impl_->mode != Mode::kSingle) {
        throw InvalidQueryArgumentException("Duplicate key errors cannot be retried for multi-document operations");
    }
...
void FindAndModify::SetOption(options::ReturnNew) { EnableFlag(impl_->options, MONGOC_FIND_AND_MODIFY_RETURN_NEW); }

void FindAndModify::SetOption(options::Upsert) { EnableFlag(impl_->options, MONGOC_FIND_AND_MODIFY_UPSERT); }
...
Aggregate::Aggregate(formats::bson::Value pipeline) : impl_(std::move(pipeline)) {
    ...
    if (impl_->pipeline.IsEmpty()) {
        throw InvalidQueryArgumentException("Provided aggregation pipeline is empty");
    }
}
...
void Aggregate::SetOption(options::ReadPreference::Mode mode) { impl_->read_prefs = MakeCDriverReadPrefs(mode); }

void Aggregate::SetOption(options::ReadConcern level) { AppendReadConcern(impl::EnsureBuilder(impl_->options), level); }
