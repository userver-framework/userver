...
DistLockComponentBase::DistLockComponentBase(
    ...
    auto mongo_timeout = component_config["mongo-timeout"].As<std::chrono::milliseconds>();
    ...
    auto strategy = std::make_shared<DistLockStrategy>(std::move(collection), lock_name);
