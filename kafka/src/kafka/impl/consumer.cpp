...
void CallErrorTestpoint(const std::string& testpoint_name,
...
std::function<void()> CreateDurationNotifier(
    std::chrono::milliseconds max_callback_duration) {
  return
      [max_callback_duration, start_time = std::chrono::system_clock::now()]() {
        ...
        if (callback_duration > max_callback_duration / 2) {
          LOG_WARNING() << fmt::format(
              ...
              callback_duration.count(), max_callback_duration.count());
        }
      };
}
...
void Consumer::RunConsuming(ConsumerScope::Callback callback) {
  ...
  while (!engine::current_task::ShouldCancel()) {
    auto polled_messages = consumer_->PollBatch(
        execution_params.max_batch_size,
        engine::Deadline::FromDuration(execution_params.poll_timeout));
    ...
    auto batch_processing_task =
        utils::Async(main_task_processor_, "messages_processing", callback,
                     utils::span{polled_messages});
...
void Consumer::StartMessageProcessing(ConsumerScope::Callback callback) {
  ...
  poll_task_ = utils::CriticalAsync(
        ...
        while (!engine::current_task::ShouldCancel()) {
          try {
            ...
            LOG_ERROR() << fmt::format(
                "Messages processing failed in consumer: {}", e.what());
          ...
          LOG_WARNING() << fmt::format(
              "Restarting consumer after {}ms...",
              execution_params.restart_after_failure_delay.count());
          engine::InterruptibleSleepFor(
              execution_params.restart_after_failure_delay);
        }
      });
}
...
void Consumer::ExtendCurrentSpan() const {
  tracing::Span::CurrentSpan().AddTag("kafka_consumer", name_);
}
