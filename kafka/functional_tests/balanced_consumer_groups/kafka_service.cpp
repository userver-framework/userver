...
class HandlerKafkaConsumerGroups final : public server::handlers::HttpHandlerJsonBase {
public:
    static constexpr std::string_view kName = "handler-kafka-consumer-groups";

    HandlerKafkaConsumerGroups(const components::ComponentConfig& config, const components::ComponentContext& context);
...
private:
    ...
    void Consume(const std::string& consumer_name, kafka::MessageBatchView messages) const;
    ...
    kafka::ConsumerScope& GetConsumerScope(const std::string& consumer_name) const;
...
formats::json::Value HandlerKafkaConsumerGroups::HandleRequestJsonThrow(
    ...
    switch (request.GetMethod()) {
        ...
        default:
            throw std::runtime_error{"Unexpected request method not stated in static config"};
    }
}

void HandlerKafkaConsumerGroups::StartConsuming(const std::string& consumer_name) const {
    auto& consumer = GetConsumerScope(consumer_name);

    consumer.Start([this, consumer_name = consumer_name, &consumer](kafka::MessageBatchView messages) {
        Consume(consumer_name, messages);
        consumer.AsyncCommit();
    });
}

void HandlerKafkaConsumerGroups::StopConsuming(const std::string& consumer_name) const {
    GetConsumerScope(consumer_name).Stop();
}

void HandlerKafkaConsumerGroups::Consume(const std::string& consumer_name, kafka::MessageBatchView messages) const {
    auto messages_by_consumer = messages_by_consumer_.Lock();

    auto messages_it = messages_by_consumer->find(consumer_name);
    UINVARIANT(
        messages_it != messages_by_consumer->end(),
        fmt::format("Expected to find consumer '{}' (according to HasConsumer check)", consumer_name)
    );
...
bool HandlerKafkaConsumerGroups::HasConsumer(const std::string& consumer_name) const {
    auto consumer_by_name = consumer_by_name_.Lock();
