...
class ToStringLogger : public logging::impl::LoggerBase {
 public:
  ...
  void Log(logging::Level level, std::string_view str) override {
    ...
    records_.push_back(
        LogRecord{utils::impl::InternalTag{}, level, std::string{str}});
  }
...
LogRecord::LogRecord(utils::impl::InternalTag, logging::Level level,
                     std::string&& log_raw)
  ...
  const auto on_invalid_record = [&] {
    throw std::runtime_error(
        fmt::format("Invalid log record: \"{}\"", log_raw_));
  };
...
logging::LoggerPtr LogCaptureLogger::GetLogger() const {
  return logger_.GetBase();
}

std::vector<LogRecord> LogCaptureLogger::GetAll() const {
