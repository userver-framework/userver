...
std::vector<std::string> CurrentProcessOpenFiles() {
    ...
    auto proc_fd_info = std::make_unique<struct proc_fdinfo[]>(buffer_size / sizeof(struct proc_fdinfo));

    buffer_size = proc_pidinfo(pid, PROC_PIDLISTFDS, 0, proc_fd_info.get(), buffer_size);
    if (buffer_size < 0) {
