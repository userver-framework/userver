...
struct hash<USERVER_NAMESPACE::logging::Level> {
    std::size_t operator()(USERVER_NAMESPACE::logging::Level level) const { return static_cast<std::size_t>(level); }
};
...
Level LevelFromString(std::string_view level_name) {
    ...
    if (!value) {
        throw std::runtime_error(
            fmt::format("Unknown log level '{}' (must be one of {})", level_name, kLowerCaseLevelMap.DescribeFirst())
        );
    }
    return *value;
}

std::string_view ToString(Level level) { return utils::impl::EnumToStringView(level, kLowerCaseLevelMap); }
