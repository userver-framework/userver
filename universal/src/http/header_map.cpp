...
std::string& DoAt(HeaderMap::Iterator it, HeaderMap::Iterator end, std::string_view key) {
    if (it == end) {
...
const std::string& DoAt(HeaderMap::ConstIterator it, HeaderMap::ConstIterator end, std::string_view key) {
    if (it == end) {
        // we can afford to construct a string here instead of
        // just throwing not-so-meaningful "at"
        throw std::out_of_range{std::string{key}};
...
std::size_t HeaderMap::count(std::string_view key) const noexcept { return contains(key) ? 1 : 0; }

std::size_t HeaderMap::count(const PredefinedHeader& key) const noexcept { return contains(key) ? 1 : 0; }

bool HeaderMap::contains(std::string_view key) const noexcept { return find(key) != end(); }

bool HeaderMap::contains(const PredefinedHeader& key) const noexcept { return find(key) != end(); }

std::string& HeaderMap::operator[](std::string&& key) {
    return impl_
        ->InsertOrModify(header_map::MaybeOwnedKey{key}, "", header_map::Map::InsertOrModifyOccupiedAction::kNoop)
        ->Get()
        .second;
}
...
std::string& HeaderMap::operator[](const PredefinedHeader& key) {
    return impl_->InsertOrModify(key, "", header_map::Map::InsertOrModifyOccupiedAction::kNoop)->Get().second;
}

HeaderMap::Iterator HeaderMap::find(std::string_view key) noexcept { return HeaderMap::Iterator{impl_->Find(key)}; }
...
HeaderMap::ConstIterator HeaderMap::find(const PredefinedHeader& key) const noexcept {
    return HeaderMap::ConstIterator{impl_->Find(key)};
}
...
void HeaderMap::insert_or_assign(const PredefinedHeader& key, std::string value) {
    impl_->InsertOrModify(key, std::move(value), header_map::Map::InsertOrModifyOccupiedAction::kReplace);
}
...
void HeaderMap::InsertOrAppend(const PredefinedHeader& key, std::string value) {
    impl_->InsertOrModify(key, std::move(value), header_map::Map::InsertOrModifyOccupiedAction::kAppend);
}

HeaderMap::Iterator HeaderMap::erase(HeaderMap::Iterator it) { return HeaderMap::Iterator{impl_->Erase(it->first)}; }

HeaderMap::Iterator HeaderMap::erase(HeaderMap::ConstIterator it) {
    return HeaderMap::Iterator{impl_->Erase(it->first)};
}

HeaderMap::Iterator HeaderMap::erase(std::string_view key) { return HeaderMap::Iterator{impl_->Erase(key)}; }

HeaderMap::Iterator HeaderMap::erase(const PredefinedHeader& key) { return HeaderMap::Iterator{impl_->Erase(key)}; }
...
HeaderMap::Iterator HeaderMap::begin() noexcept { return HeaderMap::Iterator{impl_->Begin()}; }

HeaderMap::ConstIterator HeaderMap::begin() const noexcept { return cbegin(); }

HeaderMap::ConstIterator HeaderMap::cbegin() const noexcept { return HeaderMap::ConstIterator{impl_->Begin()}; }

HeaderMap::Iterator HeaderMap::end() noexcept { return HeaderMap::Iterator{impl_->End()}; }

HeaderMap::ConstIterator HeaderMap::end() const noexcept { return cend(); }

HeaderMap::ConstIterator HeaderMap::cend() const noexcept { return HeaderMap::ConstIterator{impl_->End()}; }

bool HeaderMap::operator==(const HeaderMap& other) const noexcept { return *impl_ == *other.impl_; }

void HeaderMap::OutputInHttpFormat(HeadersString& buffer) const { impl_->OutputInHttpFormat(buffer); }
...
// NOLINTNEXTLINE(hicpp-use-equals-default,modernize-use-equals-default)
HeaderMap::Iterator::Iterator(HeaderMap::Iterator&& other) noexcept : it_{other.it_} {}

HeaderMap::Iterator& HeaderMap::Iterator::operator=(const HeaderMap::Iterator& other) = default;
// NOLINTNEXTLINE(hicpp-use-equals-default,modernize-use-equals-default)
HeaderMap::Iterator& HeaderMap::Iterator::operator=(HeaderMap::Iterator&& other) noexcept {
    it_ = other.it_;
...
bool HeaderMap::Iterator::operator!=(const Iterator& other) const { return it_ != other.it_; }

bool HeaderMap::Iterator::operator==(const ConstIterator& other) const { return it_ == other.it_; }
...
// NOLINTNEXTLINE(hicpp-use-equals-default,modernize-use-equals-default)
HeaderMap::ConstIterator::ConstIterator(ConstIterator&& other) noexcept : it_{other.it_} {}

HeaderMap::ConstIterator& HeaderMap::ConstIterator::operator=(const ConstIterator& other) = default;
// NOLINTNEXTLINE(hicpp-use-equals-default,modernize-use-equals-default)
HeaderMap::ConstIterator& HeaderMap::ConstIterator::operator=(ConstIterator&& other) noexcept {
    it_ = other.it_;
...
bool HeaderMap::ConstIterator::operator==(const ConstIterator& other) const { return it_ == other.it_; }
