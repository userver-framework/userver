...
TEST(HeaderMap, InsertFromIterators) {
    const std::vector<std::pair<std::string, std::string>> vec{{"a", "a"}, {"b", "b"}};
...
TEST(HeaderMap, ParseFromSerializeToJson) {
    ...
    const auto validate_key = [&map](const std::string& key, const std::string& value) {
        const auto it = map.find(key);
        ASSERT_NE(it, map.end());
        EXPECT_EQ(it->second, value);
    };
    ...
    const auto serialized_to_json = formats::json::ValueBuilder{map}.ExtractValue();

    EXPECT_EQ(serialized_to_json, json);
}

TEST(HeaderMap, TooManyHeaders) {
    constexpr std::size_t max_headers_count = http::headers::header_map::Traits::kMaxSize;
    HeaderMap map{};
...
TEST(HeaderMap, EraseWithCollisions) {
    ...
    ASSERT_EQ(TestsHelper::GetMapMaxDisplacement(original_map) + 1, collisions.size());

    const auto validate_erase = [](HeaderMap map, const std::vector<std::string>& shuffled_content) {
    ...
    }
    {
        ...
        ASSERT_EQ(map_copy.size(), random_shuffle.size() - (random_shuffle.size() / 2));
        for (std::size_t i = random_shuffle.size() / 2; i < random_shuffle.size(); ++i) {
...
bool CheckNoCollisions(const HeadersArray& headers, const impl::UnsafeConstexprHasher& hasher) {
...
TEST(HeaderMap, DefaultHeadersHashDistribution) {
    const auto check_uniqueness = [](const auto& headers) {
        const std::unordered_set<std::string_view> st(std::begin(headers), std::end(headers));
