...
class Pos final {
public:
    Pos(Traits::Size entries_index, Traits::HashValue hash, Traits::HeaderIndex header_index);
    Pos(std::size_t entries_index, Traits::HashValue hash, Traits::HeaderIndex header_index);
    static Pos None();
...
private:
    ...
    FindResult DoFind(std::string_view key, Traits::HashValue hash, int header_index) const noexcept;
    Iterator DoInsertOrModify(
        MaybeOwnedKey key,
    ...
    static ConstIterator ToReverseIterator(std::vector<MapEntry>::const_iterator it);

    static bool AreValuesICaseEqual(std::string_view lhs, std::string_view rhs) noexcept;

    std::size_t DebugMaxDisplacement() const;
    void DebugForceIntoRed();

    static constexpr bool IsPowerOf2(std::size_t n) noexcept { return (n & (n - 1)) == 0; }
