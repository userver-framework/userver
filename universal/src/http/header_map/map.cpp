...
inline std::size_t DesiredPos(Traits::Size mask, Traits::HashValue hash) noexcept {
...
inline std::size_t ProbeDistance(Traits::Size mask, Traits::HashValue hash, std::size_t current) noexcept {
...
inline bool Pos::IsNone() const noexcept { return entries_index_ == kNoneIndex; }

inline bool Pos::IsSome() const noexcept { return !IsNone(); }

inline Traits::Size Pos::GetEntriesIndex() const noexcept { return entries_index_; }

inline Traits::HashValue Pos::GetHash() const noexcept { return hash_; }

inline Traits::HeaderIndex Pos::GetHeaderIndex() const noexcept { return header_index_; }
...
MapEntry::MapEntry() { new (&slot_.mutable_value) std::pair<std::string, std::string>{}; }
MapEntry::~MapEntry() { std::destroy_at(&slot_.mutable_value); }

MapEntry::MapEntry(std::string&& key, std::string&& value) {
    new (&slot_.mutable_value) std::pair<std::string, std::string>{std::move(key), std::move(value)};
}

MapEntry::MapEntry(const MapEntry& other) {
    new (&slot_.mutable_value) std::pair<std::string, std::string>{other.slot_.mutable_value};
}
...
MapEntry::MapEntry(MapEntry&& other) noexcept {
    new (&slot_.mutable_value) std::pair<std::string, std::string>{std::move(other.slot_.mutable_value)};
}
...
std::pair<const std::string, std::string>& MapEntry::Get() { return slot_.value; }

const std::pair<const std::string, std::string>& MapEntry::Get() const { return slot_.value; }

std::pair<std::string, std::string>& MapEntry::GetMutable() { return slot_.mutable_value; }

bool MapEntry::operator==(const MapEntry& other) const { return Get() == other.Get(); }
...
inline bool Map::FindResult::IsSome() const noexcept { return entries_index != kNoneIndex; }

inline Map::FindResult Map::FindResult::None() noexcept { return FindResult{0, kNoneIndex}; }
...
void Map::Grow(std::size_t new_capacity) {
    ...
    if (new_capacity > Traits::kMaxSize) {
        throw HeaderMap::TooManyHeadersException{"HeaderMap reached its maximum capacity"};
    }
...
void Map::ReserveOne() {
    ...
    if (danger_.IsYellow()) {
        ...
        const auto load_factor = static_cast<float>(entries_.size()) / positions_.size();
...
void Map::Rebuild() {
    ...
    for (auto& entry : entries) {
        ...
        InsertOrModify(MaybeOwnedKey{value.first}, std::move(value.second), InsertOrModifyOccupiedAction::kAssert);
    }
}

std::size_t Map::Capacity() const noexcept { return UsableCapacity(positions_.size()); }
...
Traits::HashValue Map::HashKey(std::string_view key) const noexcept { return MaskHash(danger_.HashKey(key)); }
...
inline Map::FindResult Map::DoFind(std::string_view key, Traits::HashValue hash, int header_index) const noexcept {
    ...
    ProbeLoop(probe, [this, key, hash, header_index, &dist, &res](std::size_t idx) {
...
// dead code in debug otherwise
    ...
    const auto perform_robinhood =
        [this, hash](std::size_t dist, std::size_t positions_idx, std::string&& key, std::string&& value) {
            const auto entries_index = entries_.size();
            const auto header_index = InsertEntry(std::move(key), std::move(value));

            const auto num_displaced = DoRobinhoodAtPosition(positions_idx, Pos{entries_index, hash, header_index});

            if (dist >= kForwardShiftThreshold || num_displaced >= kDisplacementThreshold) {
                danger_.ToYellow();
            }
        };

    const auto perform_vacant =
        [this, hash](std::size_t dist, std::size_t positions_idx, std::string&& key, std::string&& value) {
...
Map::Iterator Map::DoErase(std::string_view key, Traits::HashValue hash) {
    ...
    if (entries_index < entries_.size()) {
        ...
        ProbeLoop(probe, [this, entries_index, entry_hash](std::size_t positions_idx) {
            if (positions_[positions_idx].IsSome() && positions_[positions_idx].GetEntriesIndex() >= entries_.size()) {
                // found it
                UASSERT(positions_[positions_idx].GetHash() == entry_hash);

                positions_[positions_idx] = Pos{entries_index, entry_hash, positions_[positions_idx].GetHeaderIndex()};
                return ProbingAction::kStop;
            } else {
                return ProbingAction::kContinue;
            }
    ...
    return entries_index < entries_.size() ? ToReverseIterator(entries_.begin() + entries_index) : Begin();
}
...
bool Map::operator==(const Map& other) const noexcept {
    ...
    for (const auto& entry : entries_) {
        ...
        if (other_it == other.End() || !AreValuesICaseEqual(other_it->Get().second, entry.Get().second)) {
            return false;
        }
    }
...
void Map::OutputInHttpFormat(HeadersString& buffer) const {
    ...
    const auto amount_to_add = [this] {
        ...
        for (const auto& entry : entries_) {
            total +=
                entry.Get().first.size() + kKeyValueHeaderSeparator.size() + entry.Get().second.size() + kCrlf.size();
        }
        return total;
    }();

    buffer.resize_and_overwrite(old_buffer_size + amount_to_add, [&](char* data, std::size_t size) {
...
inline Map::Iterator Map::ToReverseIterator(std::vector<MapEntry>::iterator it) {
    static_assert(!std::is_same_v<Iterator, decltype(it)>);

    return Iterator{++it /* ++ because reversed */};
}

Map::ConstIterator Map::ToReverseIterator(std::vector<MapEntry>::const_iterator it) {
    static_assert(!std::is_same_v<ConstIterator, decltype(it)>);

    return ConstIterator{++it /* ++ because reversed */};
}

bool Map::AreValuesICaseEqual(std::string_view lhs, std::string_view rhs) noexcept {
...
std::size_t Map::DebugMaxDisplacement() const {
    ...
    for (std::size_t idx = 0; idx < positions_.size(); ++idx) {
        ...
        if (pos.IsSome()) {
            max_displacement = std::max(max_displacement, ProbeDistance(mask_, pos.GetHash(), idx));
        }
    }
