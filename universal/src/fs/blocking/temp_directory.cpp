...
TempDirectory TempDirectory::Create() { return Create(boost::filesystem::temp_directory_path().string(), "userver-"); }
...
void TempDirectory::Remove() && {
    if (path_.empty()) {
        throw std::runtime_error("Remove called for an already removed TempDirectory");
    }
