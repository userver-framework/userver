...
TEST(CFile, Writing) {
    ...
    EXPECT_THROW(fs::blocking::CFile(path, fs::blocking::OpenFlag::kWrite), std::runtime_error);
...
TEST(CFile, Position) {
    ...
    {
        fs::blocking::CFile writer(path, {fs::blocking::OpenFlag::kWrite, fs::blocking::OpenFlag::kCreateIfNotExists});
