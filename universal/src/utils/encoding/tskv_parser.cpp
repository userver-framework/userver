...
std::optional<TskvParser::RecordStatus> TskvParser::ReadKey(std::string& result) {
    result.clear();
...
std::optional<TskvParser::RecordStatus> TskvParser::ReadValue(std::string& result) {
    result.clear();
    ...
    while (true) {
        ...
        if (separator != '\\') {
            ...
            return separator == '\n' ? std::make_optional(RecordStatus::kReachedEnd) : std::nullopt;
        }
...
const char* TskvParser::GetStreamPosition() const noexcept { return in_.data(); }
