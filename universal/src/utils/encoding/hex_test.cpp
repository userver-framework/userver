...
TEST(Hex, FromHex) {
    ...
    {
        const std::string data{"5f2b3d313536"};
        EXPECT_TRUE(IsHexData(data));
        const std::string reference{"_+=156"};
        std::string result;
    ...
    // Test missing symbol
    {
        ...
        // Remove one symbol
        const std::string data{"5f2b3d31353"};
        const std::string reference{"_+=15"};
        std::string result;
    ...
    {
        const std::string data{"0123456789abcdefABCDEF"};
        EXPECT_TRUE(IsHexData(data));
    }
}
...
TEST(Hex, GetHexPart) {
    ...
    // Test wrong symbol at even position
    {
        // True data is : std::string data{"5f2b3d313536"};
        // Place wrong symbol
        const std::string data{"5f2b!3d313536"};
        EXPECT_FALSE(IsHexData(data));  // no, because one symbol is not hex
        // Should have stopped at '!'
    ...
    // Test wrong symbol at odd position
    {
        // True data is : std::string data{"5f2b3d313536"};
        // Place wrong symbol
        const std::string data{"5f2b3!d313536"};
        EXPECT_EQ(4, GetHexPart(data).size());
        EXPECT_FALSE(IsHexData(data));  // no, because one symbol is not hex
    }
}
