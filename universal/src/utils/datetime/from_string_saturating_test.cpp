...
std::chrono::system_clock::time_point GetDateTimePlatformSpecificBigValue() {
    ...
    // saturate.
    return utils::datetime::FromRfc3339StringSaturating("9999-12-31T00:00:00+0000");
#else
...
TEST(FromStringSaturation, MicroSeconds) {
    ...
    utils::datetime::MockNowSet(utils::datetime::FromStringSaturating("1970-01-01 00:00:00.000000", format));

    auto tp = utils::datetime::Now() + std::chrono::microseconds(1395024427333304);
    EXPECT_EQ(utils::datetime::FromStringSaturating("2014-03-17 02:47:07.333304", format), tp);

    tp += std::chrono::microseconds(1);

    EXPECT_EQ(utils::datetime::FromStringSaturating("2014-03-17 02:47:07.333305", format), tp);

    tp -= std::chrono::microseconds(6);

    EXPECT_EQ(utils::datetime::FromStringSaturating("2014-03-17 02:47:07.333299", format), tp);
}
...
TEST(FromStringSaturation, MicroSecondsAdditionalFormat) {
    auto tp1 = utils::datetime::FromStringSaturating("2014-03-17 02:47:07.333305", "%Y-%m-%d %H:%M:%S.%E*f");
    auto tp2 = utils::datetime::FromStringSaturating("2014-03-17 02:47:07.333300", "%Y-%m-%d %H:%M:%S.%E*f");
...
TEST(FromStringSaturation, Formats) {
    EXPECT_EQ(
        utils::datetime::FromStringSaturating("9999-12-31T00:00:00Z", "%Y-%m-%dT%H:%M:%SZ"),
...
TEST(FromStringSaturation, Invalid) {
    ...
    EXPECT_THROW(
        utils::datetime::FromStringSaturating("2020-12-45T00:00:00Z", utils::datetime::kDefaultFormat),
        utils::datetime::DateParseError
    );
    EXPECT_THROW(
        utils::datetime::FromStringSaturating("2021-01-22T17:14:00Zzzz", utils::datetime::kDefaultFormat),
        utils::datetime::DateParseError
    );
}
...
TEST(FromStringSaturation, kRfc3339Format) {
    ...
    const auto tp = utils::datetime::FromStringSaturating("2014-03-17T02:47:07+00:00", utils::datetime::kRfc3339Format);
    EXPECT_EQ(tp, exp);
    /// [kRfc3339Format]
}
...
TEST(FromStringSaturation, kIsoFormat) {
    /// [kIsoFormat]
    const auto exp = utils::datetime::FromStringSaturating("2014-03-17 02:47:07.000000", "%Y-%m-%d %H:%M:%E*S");
    const auto tp = utils::datetime::FromStringSaturating("2014-03-17T02:47:07Z", utils::datetime::kIsoFormat);
    EXPECT_EQ(tp, exp);
    /// [kIsoFormat]
}
