...
TEST(Date, JSON) {
    const auto json_object = formats::json::FromString(R"json({"data" : "2048-01-11"})json");
...
TEST(Date, RequestedUsecase) {
    ...
    static_assert(!std::is_convertible_v<std::chrono::system_clock::time_point, utils::datetime::Date::SysDays>);
    static_assert(!std::is_constructible_v<utils::datetime::Date::SysDays, std::chrono::system_clock::time_point>);
