...
inline std::uint64_t RotateLeft(std::uint64_t x, std::uint64_t b) noexcept { return (x << b) | (x >> (64UL - b)); }
...
struct CaseFetcher final {
    ...
    static inline std::pair<std::uint64_t, std::uint64_t> Fetch16(const std::uint8_t* data) noexcept {
        return {Fetch8(data), Fetch8(data + 8)};
    }

    static inline std::uint64_t FetchN(const std::uint8_t* data, std::size_t n) noexcept {
        UASSERT(n < 8);
...
struct CaseInsensitiveSSEFetcher final {
    static inline std::uint64_t Fetch8(const std::uint8_t* data) noexcept { return LowercaseBytes(Load8(data)).first; }

    static inline std::pair<std::uint64_t, std::uint64_t> Fetch16(const uint8_t* data) noexcept {
        return LowercaseBytes(Load16(data));
    }

    static inline std::uint64_t FetchN(const std::uint8_t* data, std::size_t n) noexcept {
        const auto value = CaseFetcher::FetchN(data, n);
        return LowercaseBytes(_mm_loadl_epi64(reinterpret_cast<const __m128i*>(&value))).first;
    }

    static inline bool FailFastCompare8(const std::uint8_t* lhs, const std::uint8_t* rhs) noexcept {
        return FailFastCompare(Load8(lhs), Load8(rhs));
    }

    static inline bool FailFastCompare16(const std::uint8_t* lhs, const std::uint8_t* rhs) noexcept {
...
private:
    ...
    static inline bool FailFastCompare(__m128i lhs, __m128i rhs) noexcept {
        ...
        const auto lowercase_diff = _mm_xor_si128(DoLowercaseBytes(lhs), DoLowercaseBytes(rhs));

        return is_zero_vector(lowercase_diff);
    }

    static inline std::pair<std::uint64_t, std::uint64_t> LowercaseBytes(__m128i value) noexcept {
...
private:
    static inline void Lowercase(std::uint8_t* dst, const std::uint8_t* src, std::size_t len) noexcept {
...
std::uint64_t SipHash13(std::uint64_t k0, std::uint64_t k1, std::string_view data) noexcept {
    ...
    if (data.size() < 8) {
        b |= Fetcher::FetchN(reinterpret_cast<const std::uint8_t*>(data.data()), data.size());
        data_round(b);
    ...
    while (data.size() >= 16) {
        const auto [m1, m2] = Fetcher::Fetch16(reinterpret_cast<const std::uint8_t*>(data.data()));
        data_round(m1);
        data_round(m2);
    ...
    if (data.size() >= 8) {
        const std::uint64_t m = Fetcher::Fetch8(reinterpret_cast<const std::uint8_t*>(data.data()));
        data_round(m);
    ...
    if (!data.empty()) {
        ...
        b |= Fetcher::Fetch8(reinterpret_cast<const std::uint8_t*>(original_data.data() + original_data.size() - 8)) >>
             (overtake * 8);
    }
    data_round(b);
...
inline bool CompareAndAdvance(std::string_view& lhs, std::string_view& rhs) noexcept {
    static_assert(SixteenOrEight == 8 || SixteenOrEight == 16);

    UASSERT(lhs.size() == rhs.size() && lhs.size() >= SixteenOrEight);

    const auto to_uint8_ptr = [](std::string_view data) { return reinterpret_cast<const std::uint8_t*>(data.data()); };
...
SipHasher::SipHasher(std::uint64_t k0, std::uint64_t k1) noexcept : k0_{k0}, k1_{k1} {}

std::uint64_t SipHasher::operator()(std::string_view data) const noexcept {
    return SipHash13<CaseFetcher>(k0_, k1_, data);
}

CaseInsensitiveSipHasher::CaseInsensitiveSipHasher(std::uint64_t k0, std::uint64_t k1) noexcept : k0_{k0}, k1_{k1} {}

std::uint64_t CaseInsensitiveSipHasher::operator()(std::string_view data) const noexcept {
#ifdef __SSE2__
...
CaseInsensitiveSipHasherNoSse::CaseInsensitiveSipHasherNoSse(std::uint64_t k0, std::uint64_t k1) noexcept
    : k0_{k0}, k1_{k1} {}

std::uint64_t CaseInsensitiveSipHasherNoSse::operator()(std::string_view data) const noexcept {
    return SipHash13<CaseInsensitiveFetcher>(k0_, k1_, data);
}

bool CaseInsensitiveEqual::operator()(std::string_view lhs, std::string_view rhs) const noexcept {
#ifdef __SSE2__
...
bool CaseInsensitiveEqualNoSse::operator()(std::string_view lhs, std::string_view rhs) const noexcept {
