...
TYPED_TEST(TimeOfDayTest, DurationConstruct) {
    ...
    tod = TypeParam{std::chrono::hours{25} + std::chrono::minutes{10} + std::chrono::seconds{30}};
...
TEST(TimeOfDay, StringConstruct) {
    ...
    EXPECT_THROW(Milli{"1:"}, std::runtime_error) << "Expect 2 digits for minutes";
    EXPECT_THROW(Milli{"1:1"}, std::runtime_error) << "Expect 2 digits for minutes";
    EXPECT_THROW(Milli{"1:100"}, std::runtime_error) << "Expect 2 digits for minutes";
    ...
    EXPECT_THROW(Milli{"1:00:"}, std::runtime_error) << "Expect 2 digits for seconds";
    EXPECT_THROW(Milli{"1:00:1"}, std::runtime_error) << "Expect 2 digits for seconds";
    EXPECT_THROW(Milli{"1:00:111"}, std::runtime_error) << "Expect 2 digits for seconds";
    ...
    EXPECT_THROW(Milli{"1:00:00:0"}, std::runtime_error) << "Expect decimal point";
    EXPECT_THROW(Milli{"1:00:30."}, std::runtime_error) << "Expect some digits for subseconds";
...
TEST(TimeOfDay, Format) {
    Milli tod{std::chrono::hours{1} + std::chrono::minutes{2} + std::chrono::seconds{3}};
    EXPECT_THROW(static_cast<void>(fmt::format(fmt::runtime("{:%}"), tod)), fmt::format_error);
    EXPECT_THROW(static_cast<void>(fmt::format(fmt::runtime("{:%o}"), tod)), fmt::format_error);
    EXPECT_THROW(static_cast<void>(fmt::format(fmt::runtime("{:%H%}"), tod)), fmt::format_error);
    ...
    EXPECT_EQ("01h 02m 03s", fmt::format("{:%Hh %Mm %Ss}", tod)) << "HHh MMm SSs format";
}
