...
TEST(LazyPrvalue, MakeShared) {
    const auto ptr = std::make_shared<NonMovable>(utils::LazyPrvalue([] { return NonMovable::Make(); }));
    EXPECT_EQ(ptr->Get(), 42);
}

TEST(LazyPrvalue, Tuple) {
    const auto tup = std::tuple<int, std::atomic<int>>(5, utils::LazyPrvalue([] { return std::atomic<int>(10); }));
...
TEST(LazyPrvalue, Constexpr) {
    constexpr std::optional<std::atomic<int>> opt = utils::LazyPrvalue([] { return std::atomic<int>(42); });
    EXPECT_EQ(*opt, 42);
}
