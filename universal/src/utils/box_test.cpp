...
bool operator==(const RecursionStarterPack& lhs, const RecursionStarterPack& rhs) {
...
TEST(UtilsBox, ConstructionSFINAE) {
    ...
    static_assert(std::is_constructible_v<utils::Box<std::string>, std::string_view>);
    static_assert(std::is_constructible_v<utils::Box<std::string>, std::size_t, char>);

    static_assert(std::is_constructible_v<utils::Box<NonDefaulted>, int>);
    static_assert(!std::is_constructible_v<utils::Box<NonDefaulted>, std::string>);
}

TEST(UtilsBox, Explicit) {
    static_assert(std::is_convertible_v<const char*, utils::Box<std::string>>);
    static_assert(!std::is_convertible_v<std::string_view, utils::Box<std::string>>);
...
struct ThrowsOnAssignment final {
    ...
    ThrowsOnAssignment& operator=(const ThrowsOnAssignment&) { throw std::runtime_error("Test"); }
