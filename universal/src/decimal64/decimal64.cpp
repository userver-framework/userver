...
std::string ToString(int64_t before, int64_t after, int precision, const FormatOptions& format_options) {
    ...
    std::string result{(after + before < 0) ? format_options.negative_sign : format_options.positive_sign};
    ...
    fmt::format_to(std::back_inserter(result), FMT_COMPILE("{}"), groups_value[--groups_count]);
