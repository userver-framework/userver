...
TEST(Decimal64, ConstructFromString) {
    ...
    EXPECT_EQ(decimal64::Decimal<18>{"0.987654321123456789"}.AsUnbiased(), 987654321123456789LL);
    ...
    EXPECT_THROW(decimal64::Decimal<18>{"0.0000000000000000126"}, decimal64::ParseError);
    EXPECT_THROW(decimal64::Decimal<18>{"0.0000000000000000125"}, decimal64::ParseError);
    EXPECT_THROW(decimal64::Decimal<18>{"0.0000000000000000124"}, decimal64::ParseError);
...
TEST(Decimal64, FromStringPermissive) {
    ...
    EXPECT_EQ(decimal64::Decimal<18>::FromStringPermissive("0.987654321123456789").AsUnbiased(), 987654321123456789LL);
    EXPECT_EQ(decimal64::Decimal<18>::FromStringPermissive("0.0000000000000000126").AsUnbiased(), 13);
    ...
    EXPECT_THROW(Dec4::FromStringPermissive("1234567898765432.1012"), decimal64::ParseError);
    EXPECT_THROW(Dec4::FromStringPermissive("99999999999999999999999999"), decimal64::ParseError);
    EXPECT_THROW(Dec4::FromStringPermissive("+99999999999999999999999999"), decimal64::ParseError);
    EXPECT_THROW(Dec4::FromStringPermissive("-99999999999999999999999999"), decimal64::ParseError);
}
...
void TestFromStream(
    ...
    Dec4 expected_result,
    std::string_view expected_remaining
) {
    Dec4 result = kUnchangedOnError;
    std::istringstream is{input};
    EXPECT_EQ(static_cast<bool>(is >> result), expected_success) << "input: \"" << input << "\"";
    is.clear();
...
TEST(Decimal64, ParseValid) {
    const auto json_object = formats::json::FromString(R"json({"data" : "123"})json");
    const auto json_data = json_object["data"];
    EXPECT_EQ(json_data.As<Dec4>(), Dec4{123});
}

TEST(Decimal64, ParseInvalid) {
    const auto json_object = formats::json::FromString(R"json({"data" : "#"})json");
    const auto json_data = json_object["data"];
...
TEST(Decimal64, RoundPolicies) {
    ...
    EXPECT_EQ(HalfDown::FromStringPermissive("0.00005000000000001"), HalfDown{"0.0001"});
    EXPECT_EQ(HalfDown::FromStringPermissive("-0.00005"), HalfDown{0});
    EXPECT_EQ(HalfDown::FromStringPermissive("-0.00005000000000001"), HalfDown{"-0.0001"});
    ...
    EXPECT_EQ(Ceiling::FromStringPermissive("0.00000000000000001"), Ceiling{"0.0001"});
    ...
    EXPECT_EQ(Floor::FromStringPermissive("-0.00000000000000001"), Floor{"-0.0001"});
