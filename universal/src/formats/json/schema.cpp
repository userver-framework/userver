...
std::string_view SchemaValidationException::GetSchemaPath() const noexcept {
  return schema_path_;
}
...
Schema::ValidationResult::ValidationResult(ValidationResult&&) noexcept =
    default;

Schema::ValidationResult& Schema::ValidationResult::operator=(
    ValidationResult&&) noexcept = default;
...
void Schema::ValidationError::Throw() const {
  ...
  const auto cut_path = value_path_.size() <= 2
                            ? std::string_view{"/"}
                            : std::string_view{value_path_}.substr(2);

  throw SchemaValidationException(details_string_, cut_path, schema_path_);
}

std::string_view Schema::ValidationError::GetValuePath() const {
  return value_path_;
}

std::string_view Schema::ValidationError::GetSchemaPath() const {
  return schema_path_;
}

std::string_view Schema::ValidationError::GetDetailsString() const {
  return details_string_;
}
