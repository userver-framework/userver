...
%TAG !example! tag:example,2024:
  ...
  for (auto scalar_field :
       {"field_int", "field_string", "field_bool", "field_array", "field_obj",
        "field_array_json", "field_obj_json"}) {
  ...
  EXPECT_THAT(yaml["field_custom_tag_null"].GetTag(),
              testing::AnyOf(testing::Eq(""), testing::Eq("!my_tag")));
