...
TEST(Zstd, DecompressSmall) {
  ...
  const auto comp_size = ZSTD_compress(comp_buf.data(), comp_buf.capacity(),
                                       str.data(), str.size(), 1);
  ...
  auto decompressed = compression::zstd::Decompress(
      std::string_view(comp_buf.data(), comp_size), str.size());
...
TEST(Zstd, DecompressLarge) {
  ...
  const auto comp_size =
      ZSTD_compress(comp_buf.data(), comp_buf.capacity(), str.data(), kSize, 1);
...
TEST(Zstd, TestOverflow) {
  ...
  EXPECT_THROW(
      compression::zstd::Decompress(
          std::string_view(comp_buf.data(), comp_size), big_msg.size() / 2),
