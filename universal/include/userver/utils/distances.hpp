...
std::string SuggestNearestName(const StringViews& strings, std::string_view key) {
    static_assert(
        std::is_convertible_v<decltype(*std::begin(strings)), std::string_view>,
        "Parameter `strings` should be an iterable over strings."
    );
    constexpr std::size_t kMaxDistance = 3;
    return impl::SuggestNameErrorMsg(utils::GetNearestString(strings, key, kMaxDistance));
}
