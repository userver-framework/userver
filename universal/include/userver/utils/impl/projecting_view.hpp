...
public:
    ...
    using BaseIterator =
        std::conditional_t<std::is_const_v<Container>, BaseConstIterator, typename Container::iterator>;

    using const_iterator = ProjectingIterator<BaseConstIterator, Projection>;
    using value_type = typename const_iterator::value_type;

    explicit ProjectingView(Container& container) noexcept : container_(container) {}
    ...
    auto begin() {
        return ProjectingIterator<BaseIterator, Projection>{container_.begin(), static_cast<const Projection&>(*this)};
    }

    auto end() {
        return ProjectingIterator<BaseIterator, Projection>{container_.end(), static_cast<const Projection&>(*this)};
    }
