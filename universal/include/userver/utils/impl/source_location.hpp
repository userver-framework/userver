...
public:
    ...
    constexpr std::string_view GetLineString() const noexcept {
        return std::string_view(&line_string_[kMaxLineDigits - line_digits_], line_digits_);
    }

    constexpr std::string_view GetFileName() const noexcept { return file_name_; }

    constexpr std::string_view GetFunctionName() const noexcept { return function_name_; }
...
private:
    ...
    constexpr SourceLocation(
        ...
        : line_(line), line_digits_(DigitsBase10(line)), file_name_(file_name), function_name_(function_name) {
        FillLineString();
    }
    ...
    constexpr void FillLineString() noexcept {
        ...
        for (std::size_t index = kMaxLineDigits - 1; line > 0; line /= 10, --index) {
