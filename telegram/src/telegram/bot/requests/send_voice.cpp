...
SendVoiceMethod::Parameters Parse(
  ...
  SendVoiceMethod::Parameters parameters{
    data["chat_id"].template As<ChatId>(),
    data["voice"].template As<SendVoiceMethod::Parameters::Voice>()
  };
...
template <class Value>
Value Serialize(const SendVoiceMethod::Parameters& parameters, formats::serialize::To<Value>) {
  typename Value::Builder builder;
  builder["chat_id"] = parameters.chat_id;
  SetIfNotNull(builder, "message_thread_id", parameters.message_thread_id);
  builder["voice"] = parameters.voice;
...
SendVoiceMethod::Parameters::Parameters(
    ChatId _chat_id,
    SendVoiceMethod::Parameters::Voice _voice)
  : chat_id(std::move(_chat_id))
  , voice(std::move(_voice)) {}

void SendVoiceMethod::FillRequestData(clients::http::Request& request,
                                      const Parameters& parameters) {
  if (!std::get_if<InputFile>(&parameters.voice)) {
    FillRequestDataAsJson<SendVoiceMethod>(request, parameters);
    return;
  }
  ...
  FillFormSection(form, "voice", parameters.voice);
...
SendVoiceMethod::Reply SendVoiceMethod::ParseResponseData(
    clients::http::Response& response) {
  return ParseResponseDataFromJson<SendVoiceMethod>(response);
}

SendVoiceMethod::Parameters Parse(
    const formats::json::Value& json,
    formats::parse::To<SendVoiceMethod::Parameters> to) {
