...
AMQP::ExchangeType Convert(urabbitmq::ExchangeType type) {
  ...
  switch (type) {
    case From::kFanOut: return To::fanout;
    case From::kDirect: return To::direct;
    case From::kTopic: return To::topic;
    case From::kHeaders: return To::headers;
    case From::kConsistentHash: return To::consistent_hash;
    case From::kMessageDeduplication: return To::message_deduplication;
  }
}
...
AmqpChannel::AmqpChannel(AmqpConnection& conn) : thread_{conn.GetEvThread()} {
  ...
  thread_.RunInEvLoopSync([this, &conn, deferred] {
    ...
    channel_->onReady([deferred = deferred]{
      deferred->Ok();
    });
    channel_->onError([deferred = deferred] (const char* error) {
      deferred->Fail(error);
    });
  });
...
void AmqpChannel::DeclareExchange(const Exchange& exchange, ExchangeType exchangeType) {
  auto deferred = DeferredWrapper::Create();

  thread_.RunInEvLoopAsync([this, exchange = exchange.GetUnderlying(), exchangeType, deferred] {
...
void AmqpChannel::BindQueue(const Exchange& exchange, const Queue& queue, const std::string& routing_key) {
  auto deferred = DeferredWrapper::Create();

  thread_.RunInEvLoopAsync([this, exchange=exchange.GetUnderlying(), queue=queue.GetUnderlying(), routing_key, deferred] {
...
void AmqpChannel::Publish(const Exchange& exchange, const std::string& routing_key, const std::string& message) {
  auto deferred = DeferredWrapper::Create();

  thread_.RunInEvLoopAsync([this, exchange=exchange.GetUnderlying(), routing_key, message, deferred] {
...
void AmqpReliableChannel::Publish(const Exchange& exchange, const std::string& routing_key, const std::string& message) {
  auto deferred = DeferredWrapper::Create();

  channel_.thread_.RunInEvLoopAsync([this, exchange=exchange.GetUnderlying(), routing_key, message, deferred] {
