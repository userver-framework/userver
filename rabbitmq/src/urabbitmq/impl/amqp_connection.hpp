...
public:
    AmqpConnection(AmqpConnectionHandler& handler, size_t max_in_flight_requests, engine::Deadline deadline);
    ~AmqpConnection();
    ...
    LockedChannelProxy<ReliableChannel> GetReliableChannel(engine::Deadline deadline);
...
private:
    ...
    template <typename Channel>
    LockedChannelProxy<Channel> DoGetChannel(Channel& channel, engine::Deadline deadline) {
