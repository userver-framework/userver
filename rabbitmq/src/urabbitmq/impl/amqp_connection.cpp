...
AmqpConnection::AmqpConnection(AmqpConnectionHandler& handler, size_t max_in_flight_requests, engine::Deadline deadline)
    : handler_{handler},
...
AmqpConnection::~AmqpConnection() {
    handler_.OnConnectionDestruction();
    handler_.SetOperationDeadline(engine::Deadline::FromDuration(kGracefulCloseTimeout));
}

AMQP::Connection& AmqpConnection::GetNative() { return conn_; }

void AmqpConnection::SetOperationDeadline(engine::Deadline deadline) { handler_.SetOperationDeadline(deadline); }

statistics::ConnectionStatistics& AmqpConnection::GetStatistics() { return handler_.GetStatistics(); }

LockedChannelProxy<AMQP::Channel> AmqpConnection::GetChannel(engine::Deadline deadline) {
    return DoGetChannel(channel_, deadline);
}

LockedChannelProxy<AmqpConnection::ReliableChannel> AmqpConnection::GetReliableChannel(engine::Deadline deadline) {
...
ConnectionLock AmqpConnection::Lock(engine::Deadline deadline) { return {mutex_, deadline}; }
...
std::unique_ptr<AmqpConnection::ReliableChannel> AmqpConnection::CreateReliable(engine::Deadline deadline) {
    auto lock = Lock(deadline);
    SetOperationDeadline(deadline);
    return std::make_unique<AmqpConnection::ReliableChannel>(reliable_channel_);
}

void AmqpConnection::AwaitChannelCreated(AMQP::Channel& channel, engine::Deadline deadline) {
