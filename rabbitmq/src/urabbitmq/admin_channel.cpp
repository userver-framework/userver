...
void AdminChannel::DeclareExchange(
...
void AdminChannel::BindQueue(
    ...
    ConnectionHelper::BindQueue(*impl_, exchange, queue, routing_key, deadline).Wait(deadline);
}

void AdminChannel::RemoveExchange(const Exchange& exchange, engine::Deadline deadline) {
