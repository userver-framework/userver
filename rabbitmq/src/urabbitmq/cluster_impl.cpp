...
std::shared_ptr<ChannelPool> CreatePool(clients::dns::Resolver& resolver, bool reliable) {
  const ChannelPoolSettings settings {
      reliable ? ChannelPoolMode::kReliable : ChannelPoolMode::kUnreliable,
      10
  };
...
ChannelPtr ClusterImpl::GetUnreliable() {
  return unreliable_->Acquire();
}

ChannelPtr ClusterImpl::GetReliable() {
  return reliable_->Acquire();
}
