...
UTEST(Consumer, HeaderFieldStringConversionInvariants) {
    ...
    headers.set("uint32", std::numeric_limits<uint32_t>::max());
    headers.set("int32", std::numeric_limits<int32_t>::min());
    headers.set("uint64", std::numeric_limits<uint64_t>::max());
    headers.set("int64", std::numeric_limits<int64_t>::min());
