...
clients::dns::Resolver CreateResolver() { return clients::dns::Resolver{engine::current_task::GetTaskProcessor(), {}}; }

std::shared_ptr<urabbitmq::Client> CreateClient(clients::dns::Resolver& resolver) {
    return urabbitmq::Client::Create(resolver, urabbitmq::TestsHelper::CreateSettings());
}
...
uint16_t GetRabbitMqPort() {
    ...
    return rabbitmq_port_env ? utils::FromString<std::uint16_t>(rabbitmq_port_env) : kDefaultRabbitMqPort;
}
...
std::shared_ptr<urabbitmq::Client> ClientWrapper::Get() const { return client_; }

const urabbitmq::Exchange& ClientWrapper::GetExchange() const { return exchange_; }
...
void ClientWrapper::SetupRmqEntities() const {
    admin_.DeclareExchange(GetExchange(), urabbitmq::Exchange::Type::kFanOut, {}, GetDeadline());
