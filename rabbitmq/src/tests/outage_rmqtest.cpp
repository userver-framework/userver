...
urabbitmq::ClientSettings GetSettings(std::vector<urabbitmq::EndpointInfo> endpoints) {
...
UTEST(ClusterOutage, AllNodesDead) {
    ...
    const auto settings = GetSettings({{"unresolved", 5672}, {"another_unresolved", 5672}});
    ...
    EXPECT_ANY_THROW(client->GetAdminChannel(engine::Deadline::FromDuration(timeout)));
    EXPECT_ANY_THROW(client->GetChannel(engine::Deadline::FromDuration(timeout)));
    EXPECT_ANY_THROW(client->GetReliableChannel(engine::Deadline::FromDuration(timeout)));
}
