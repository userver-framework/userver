...
ScanRequest<ScanTag::kHscan> Client::Hscan(std::string key, const CommandControl& command_control) {
    return Hscan(std::move(key), {}, command_control);
}

ScanRequest<ScanTag::kSscan> Client::Sscan(std::string key, const CommandControl& command_control) {
    return Sscan(std::move(key), {}, command_control);
}

ScanRequest<ScanTag::kZscan> Client::Zscan(std::string key, const CommandControl& command_control) {
