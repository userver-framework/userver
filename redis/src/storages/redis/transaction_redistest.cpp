...
UTEST_P(RedisExpireOptionsTransactionTest, ExpireOptionsTest) {
    ...
    if (initial_expire.has_value()) {
        Get(client->Expire("mykey", std::chrono::seconds(initial_expire.value())));
    }
    EXPECT_EQ(Get(client->Expire("mykey", std::chrono::seconds(new_expire), options)), expected_reply);
...
INSTANTIATE_UTEST_SUITE_P(
    RedisClientTransactionTest,
    RedisExpireOptionsTransactionTest,
    testing::Values(
...
UTEST_F(RedisClientTransactionTest, Georadius) {
    auto& client = GetTransactionClient();
    Get(client->Geoadd("Sicily", {{13.361389, 38.115556, "Palermo"}, {15.087269, 37.502669, "Catania"}}));
...
UTEST_F(RedisClientTransactionTest, Geopos) {
    auto& client = GetTransactionClient();
...
UTEST_F(RedisClientTransactionTest, HdelMultiple) {
    auto& client = GetTransactionClient();
    EXPECT_EQ(Get(client->Hset("hash", "field1", "foo")), storages::redis::HsetReply::kCreated);
    EXPECT_EQ(Get(client->Hdel("hash", std::vector<std::string>{"field1", "field2"})), 1);
}
...
UTEST_F(RedisClientTransactionTest, Srem) {
    ...
    Get(client->Sadd("set", std::vector<std::string>{"one", "two", "three", "four"}));
    EXPECT_EQ(Get(client->Srem("set", "one")), 1);

    EXPECT_EQ(Get(client->Srem("set", std::vector<std::string>{"two", "three", "five"})), 2);
}
