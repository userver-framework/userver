...
RedisConnectionState::RedisConnectionState() {
  ...
  sentinel_ = Sentinel::CreateSentinel(
      thread_pools_, GetRedisSettings(), "none",
      dynamic_config::GetDefaultSource(), "pub", KeyShardFactory{""});
  sentinel_->WaitConnectedDebug();
  client_ = std::make_shared<ClientImpl>(sentinel_);

  subscribe_sentinel_ = SubscribeSentinel::Create(
      thread_pools_, GetRedisSettings(), "none",
      dynamic_config::GetDefaultSource(), "pub", "KeyShardZero", {}, {});
  subscribe_sentinel_->WaitConnectedDebug();
  subscribe_client_ =
      std::make_shared<SubscribeClientImpl>(subscribe_sentinel_);
}
...
RedisConnectionState::RedisConnectionState(InClusterMode) {
  ...
  sentinel_ = Sentinel::CreateSentinel(
      thread_pools_, GetRedisClusterSettings(), "none", configs_source, "pub",
      KeyShardFactory{USERVER_NAMESPACE::redis::kRedisCluster});
  sentinel_->WaitConnectedDebug();
  client_ = std::make_shared<ClientImpl>(sentinel_);

  subscribe_sentinel_ =
      SubscribeSentinel::Create(thread_pools_, GetRedisClusterSettings(),
                                "none", configs_source, "pub", "RedisCluster", {}, {});
  subscribe_sentinel_->WaitConnectedDebug();
  subscribe_client_ =
      std::make_shared<SubscribeClientImpl>(subscribe_sentinel_);
}
