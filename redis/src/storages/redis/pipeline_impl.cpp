...
RequestExec CreateExecRequest(impl::Request&& request, std::vector<PipelineImpl::ResultPromise>&& result_promises) {
    return RequestExec(std::make_unique<RequestExecDataImpl>(std::move(request), std::move(result_promises)));
}

}  // namespace

PipelineImpl::PipelineImpl(std::shared_ptr<ClientImpl> client, CheckShards check_shards)
    : client_(std::move(client)), check_shards_(check_shards) {}

RequestExec PipelineImpl::Exec(const CommandControl& command_control) {
    if (!shard_) {
...
RequestAppend PipelineImpl::Append(std::string key, std::string value) {
    UpdateShard(key);
    return AddCmd<RequestAppend>("append", true, std::move(key), std::move(value));
}

RequestBitop PipelineImpl::Bitop(BitOperation op, std::string dest, std::vector<std::string> srcs) {
    UpdateShard(dest);
    const auto operation = ToString(op);
    return AddCmd<RequestBitop>("bitop", true, std::move(operation), std::move(dest), std::move(srcs));
}

RequestDbsize PipelineImpl::Dbsize(size_t shard) {
    UpdateShard(shard);
    return AddCmd<RequestDbsize>("dbsize", false);
}

RequestDecr PipelineImpl::Decr(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestDecr>("decr", true, std::move(key));
}

RequestDel PipelineImpl::Del(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestDel>("del", true, std::move(key));
}

RequestDel PipelineImpl::Del(std::vector<std::string> keys) {
    UpdateShard(keys);
    return AddCmd<RequestDel>("del", true, std::move(keys));
}

RequestUnlink PipelineImpl::Unlink(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestUnlink>("unlink", true, std::move(key));
}

RequestUnlink PipelineImpl::Unlink(std::vector<std::string> keys) {
    UpdateShard(keys);
    return AddCmd<RequestUnlink>("unlink", true, std::move(keys));
}

RequestExists PipelineImpl::Exists(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestExists>("exists", false, std::move(key));
}

RequestExists PipelineImpl::Exists(std::vector<std::string> keys) {
    UpdateShard(keys);
    return AddCmd<RequestExists>("exists", false, std::move(keys));
}

RequestExpire PipelineImpl::Expire(std::string key, std::chrono::seconds ttl) {
    UpdateShard(key);
    return AddCmd<RequestExpire>("expire", true, std::move(key), ttl.count());
}

RequestExpire PipelineImpl::Expire(std::string key, std::chrono::seconds ttl, ExpireOptions options) {
    UpdateShard(key);
    return AddCmd<RequestExpire>("expire", true, std::move(key), ttl.count(), options);
}

RequestGeoadd PipelineImpl::Geoadd(std::string key, GeoaddArg point_member) {
    UpdateShard(key);
    return AddCmd<RequestGeoadd>("geoadd", true, std::move(key), std::move(point_member));
}

RequestGeoadd PipelineImpl::Geoadd(std::string key, std::vector<GeoaddArg> point_members) {
    UpdateShard(key);
    return AddCmd<RequestGeoadd>("geoadd", true, std::move(key), std::move(point_members));
}

RequestGeopos PipelineImpl::Geopos(std::string key, std::vector<std::string> members) {
    UpdateShard(key);
    return AddCmd<RequestGeopos>("geopos", false, std::move(key), std::move(members));
}

RequestGeoradius PipelineImpl::Georadius(
    std::string key,
    Longitude lon,
    Latitude lat,
    double radius,
...
RequestGeosearch PipelineImpl::Geosearch(
    std::string key,
    std::string member,
    double radius,
...
RequestGet PipelineImpl::Get(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestGet>("get", false, std::move(key));
}

RequestGetset PipelineImpl::Getset(std::string key, std::string value) {
    UpdateShard(key);
    return AddCmd<RequestGetset>("getset", true, std::move(key), std::move(value));
}

RequestHdel PipelineImpl::Hdel(std::string key, std::string field) {
    UpdateShard(key);
    return AddCmd<RequestHdel>("hdel", true, std::move(key), std::move(field));
}

RequestHdel PipelineImpl::Hdel(std::string key, std::vector<std::string> fields) {
    UpdateShard(key);
    return AddCmd<RequestHdel>("hdel", true, std::move(key), std::move(fields));
}

RequestHexists PipelineImpl::Hexists(std::string key, std::string field) {
    UpdateShard(key);
    return AddCmd<RequestHexists>("hexists", false, std::move(key), std::move(field));
}

RequestHget PipelineImpl::Hget(std::string key, std::string field) {
    UpdateShard(key);
    return AddCmd<RequestHget>("hget", false, std::move(key), std::move(field));
}

RequestHgetall PipelineImpl::Hgetall(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestHgetall>("hgetall", false, std::move(key));
}

RequestHincrby PipelineImpl::Hincrby(std::string key, std::string field, int64_t increment) {
    UpdateShard(key);
    return AddCmd<RequestHincrby>("hincrby", true, std::move(key), std::move(field), increment);
}

RequestHincrbyfloat PipelineImpl::Hincrbyfloat(std::string key, std::string field, double increment) {
    UpdateShard(key);
    return AddCmd<RequestHincrbyfloat>("hincrbyfloat", true, std::move(key), std::move(field), increment);
}

RequestHkeys PipelineImpl::Hkeys(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestHkeys>("hkeys", false, std::move(key));
}

RequestHlen PipelineImpl::Hlen(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestHlen>("hlen", false, std::move(key));
}

RequestHmget PipelineImpl::Hmget(std::string key, std::vector<std::string> fields) {
    UpdateShard(key);
    return AddCmd<RequestHmget>("hmget", false, std::move(key), std::move(fields));
}

RequestHmset PipelineImpl::Hmset(std::string key, std::vector<std::pair<std::string, std::string>> field_values) {
    UpdateShard(key);
    return AddCmd<RequestHmset>("hmset", true, std::move(key), std::move(field_values));
}

RequestHset PipelineImpl::Hset(std::string key, std::string field, std::string value) {
    UpdateShard(key);
    return AddCmd<RequestHset>("hset", true, std::move(key), std::move(field), std::move(value));
}

RequestHsetnx PipelineImpl::Hsetnx(std::string key, std::string field, std::string value) {
    UpdateShard(key);
    return AddCmd<RequestHsetnx>("hsetnx", true, std::move(key), std::move(field), std::move(value));
}

RequestHvals PipelineImpl::Hvals(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestHvals>("hvals", false, std::move(key));
}

RequestIncr PipelineImpl::Incr(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestIncr>("incr", true, std::move(key));
}

RequestKeys PipelineImpl::Keys(std::string keys_pattern, size_t shard) {
    UpdateShard(shard);
    return AddCmd<RequestKeys>("keys", false, std::move(keys_pattern));
}

RequestLindex PipelineImpl::Lindex(std::string key, int64_t index) {
    UpdateShard(key);
    return AddCmd<RequestLindex>("lindex", false, std::move(key), index);
}

RequestLlen PipelineImpl::Llen(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestLlen>("llen", false, std::move(key));
}

RequestLpop PipelineImpl::Lpop(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestLpop>("lpop", true, std::move(key));
}

RequestLpush PipelineImpl::Lpush(std::string key, std::string value) {
    UpdateShard(key);
    return AddCmd<RequestLpush>("lpush", true, std::move(key), std::move(value));
}

RequestLpush PipelineImpl::Lpush(std::string key, std::vector<std::string> values) {
    UpdateShard(key);
    return AddCmd<RequestLpush>("lpush", true, std::move(key), std::move(values));
}

RequestLpushx PipelineImpl::Lpushx(std::string key, std::string element) {
    UpdateShard(key);
    return AddCmd<RequestLpushx>("lpushx", true, std::move(key), std::move(element));
}

RequestLrange PipelineImpl::Lrange(std::string key, int64_t start, int64_t stop) {
    UpdateShard(key);
    return AddCmd<RequestLrange>("lrange", false, std::move(key), start, stop);
}

RequestLrem PipelineImpl::Lrem(std::string key, int64_t count, std::string element) {
    UpdateShard(key);
    return AddCmd<RequestLrem>("lrem", true, std::move(key), count, std::move(element));
}

RequestLtrim PipelineImpl::Ltrim(std::string key, int64_t start, int64_t stop) {
    UpdateShard(key);
    return AddCmd<RequestLtrim>("ltrim", true, std::move(key), start, stop);
}

RequestMget PipelineImpl::Mget(std::vector<std::string> keys) {
    UpdateShard(keys);
    return AddCmd<RequestMget>("mget", false, std::move(keys));
}

RequestMset PipelineImpl::Mset(std::vector<std::pair<std::string, std::string>> key_values) {
    UpdateShard(key_values);
    return AddCmd<RequestMset>("mset", true, std::move(key_values));
}

RequestPersist PipelineImpl::Persist(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestPersist>("persist", true, std::move(key));
}

RequestPexpire PipelineImpl::Pexpire(std::string key, std::chrono::milliseconds ttl) {
    UpdateShard(key);
    return AddCmd<RequestPexpire>("pexpire", true, std::move(key), ttl.count());
}

RequestPing PipelineImpl::Ping(size_t shard) {
    UpdateShard(shard);
    return AddCmd<RequestPing>("ping", false);
}

RequestPingMessage PipelineImpl::PingMessage(size_t shard, std::string message) {
    UpdateShard(shard);
    return AddCmd<RequestPingMessage>("ping", false, std::move(message));
}

RequestRename PipelineImpl::Rename(std::string key, std::string new_key) {
    UpdateShard(key);
    UpdateShard(new_key);
    return AddCmd<RequestRename>("rename", true, std::move(key), std::move(new_key));
}

RequestRpop PipelineImpl::Rpop(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestRpop>("rpop", true, std::move(key));
}

RequestRpush PipelineImpl::Rpush(std::string key, std::string value) {
    UpdateShard(key);
    return AddCmd<RequestRpush>("rpush", true, std::move(key), std::move(value));
}

RequestRpush PipelineImpl::Rpush(std::string key, std::vector<std::string> values) {
    UpdateShard(key);
    return AddCmd<RequestRpush>("rpush", true, std::move(key), std::move(values));
}

RequestRpushx PipelineImpl::Rpushx(std::string key, std::string element) {
    UpdateShard(key);
    return AddCmd<RequestRpushx>("rpushx", true, std::move(key), std::move(element));
}

RequestSadd PipelineImpl::Sadd(std::string key, std::string member) {
    UpdateShard(key);
    return AddCmd<RequestSadd>("sadd", true, std::move(key), std::move(member));
}

RequestSadd PipelineImpl::Sadd(std::string key, std::vector<std::string> members) {
    UpdateShard(key);
    return AddCmd<RequestSadd>("sadd", true, std::move(key), std::move(members));
}

RequestScard PipelineImpl::Scard(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestScard>("scard", false, std::move(key));
}

RequestSet PipelineImpl::Set(std::string key, std::string value) {
    UpdateShard(key);
    return AddCmd<RequestSet>("set", true, std::move(key), std::move(value));
}

RequestSet PipelineImpl::Set(std::string key, std::string value, std::chrono::milliseconds ttl) {
    UpdateShard(key);
    return AddCmd<RequestSet>("set", true, std::move(key), std::move(value), "PX", ttl.count());
}

RequestSetIfExist PipelineImpl::SetIfExist(std::string key, std::string value) {
    UpdateShard(key);
    return AddCmd<RequestSetIfExist>("set", true, std::move(key), std::move(value), "XX");
}

RequestSetIfExist PipelineImpl::SetIfExist(std::string key, std::string value, std::chrono::milliseconds ttl) {
    UpdateShard(key);
    return AddCmd<RequestSetIfExist>("set", true, std::move(key), std::move(value), "PX", ttl.count(), "XX");
}

RequestSetIfNotExist PipelineImpl::SetIfNotExist(std::string key, std::string value) {
    UpdateShard(key);
    return AddCmd<RequestSetIfNotExist>("set", true, std::move(key), std::move(value), "NX");
}

RequestSetIfNotExist PipelineImpl::SetIfNotExist(std::string key, std::string value, std::chrono::milliseconds ttl) {
    UpdateShard(key);
    return AddCmd<RequestSetIfNotExist>("set", true, std::move(key), std::move(value), "PX", ttl.count(), "NX");
}

RequestSetIfNotExistOrGet PipelineImpl::SetIfNotExistOrGet(std::string key, std::string value) {
    UpdateShard(key);
    return AddCmd<RequestSetIfNotExistOrGet>("set", true, std::move(key), std::move(value), "NX", "GET");
}

RequestSetIfNotExistOrGet PipelineImpl::SetIfNotExistOrGet(
    std::string key,
    std::string value,
...
RequestSetex PipelineImpl::Setex(std::string key, std::chrono::seconds seconds, std::string value) {
    UpdateShard(key);
    return AddCmd<RequestSetex>("setex", true, std::move(key), seconds.count(), std::move(value));
}

RequestSismember PipelineImpl::Sismember(std::string key, std::string member) {
    UpdateShard(key);
    return AddCmd<RequestSismember>("sismember", false, std::move(key), std::move(member));
}

RequestSmembers PipelineImpl::Smembers(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestSmembers>("smembers", false, std::move(key));
}

RequestSrandmember PipelineImpl::Srandmember(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestSrandmember>("srandmember", false, std::move(key));
}

RequestSrandmembers PipelineImpl::Srandmembers(std::string key, int64_t count) {
    UpdateShard(key);
    return AddCmd<RequestSrandmembers>("srandmember", false, std::move(key), count);
}

RequestSrem PipelineImpl::Srem(std::string key, std::string member) {
    UpdateShard(key);
    return AddCmd<RequestSrem>("srem", true, std::move(key), std::move(member));
}

RequestSrem PipelineImpl::Srem(std::string key, std::vector<std::string> members) {
    UpdateShard(key);
    return AddCmd<RequestSrem>("srem", true, std::move(key), std::move(members));
}

RequestStrlen PipelineImpl::Strlen(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestStrlen>("strlen", false, std::move(key));
}

RequestTime PipelineImpl::Time(size_t shard) {
    UpdateShard(shard);
    return AddCmd<RequestTime>("time", false);
}

RequestTtl PipelineImpl::Ttl(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestTtl>("ttl", false, std::move(key));
}

RequestType PipelineImpl::Type(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestType>("type", false, std::move(key));
}

RequestZadd PipelineImpl::Zadd(std::string key, double score, std::string member) {
    UpdateShard(key);
    return AddCmd<RequestZadd>("zadd", true, std::move(key), score, std::move(member));
}

RequestZadd PipelineImpl::Zadd(std::string key, double score, std::string member, const ZaddOptions& options) {
    UpdateShard(key);
    return AddCmd<RequestZadd>("zadd", true, std::move(key), options, score, std::move(member));
}

RequestZadd PipelineImpl::Zadd(std::string key, std::vector<std::pair<double, std::string>> scored_members) {
    UpdateShard(key);
    return AddCmd<RequestZadd>("zadd", true, std::move(key), std::move(scored_members));
}

RequestZadd PipelineImpl::Zadd(
    std::string key,
...
RequestZaddIncr PipelineImpl::ZaddIncr(std::string key, double score, std::string member) {
    UpdateShard(key);
    return AddCmd<RequestZaddIncr>("zadd", true, std::move(key), "INCR", score, std::move(member));
}

RequestZaddIncrExisting PipelineImpl::ZaddIncrExisting(std::string key, double score, std::string member) {
    UpdateShard(key);
    return AddCmd<RequestZaddIncrExisting>("zadd", true, std::move(key), "XX", "INCR", score, std::move(member));
}

RequestZcard PipelineImpl::Zcard(std::string key) {
    UpdateShard(key);
    return AddCmd<RequestZcard>("zcard", false, std::move(key));
}

RequestZcount PipelineImpl::Zcount(std::string key, double min, double max) {
    UpdateShard(key);
    return AddCmd<RequestZcount>("zcount", false, std::move(key), min, max);
}

RequestZrange PipelineImpl::Zrange(std::string key, int64_t start, int64_t stop) {
    UpdateShard(key);
    return AddCmd<RequestZrange>("zrange", false, std::move(key), start, stop);
}

RequestZrangeWithScores PipelineImpl::ZrangeWithScores(std::string key, int64_t start, int64_t stop) {
    UpdateShard(key);
    ScoreOptions with_scores{true};
    return AddCmd<RequestZrangeWithScores>("zrange", false, std::move(key), start, stop, with_scores);
}

RequestZrangebyscore PipelineImpl::Zrangebyscore(std::string key, double min, double max) {
    UpdateShard(key);
    return AddCmd<RequestZrangebyscore>("zrangebyscore", false, std::move(key), min, max);
}

RequestZrangebyscore PipelineImpl::Zrangebyscore(std::string key, std::string min, std::string max) {
    UpdateShard(key);
    return AddCmd<RequestZrangebyscore>("zrangebyscore", false, std::move(key), std::move(min), std::move(max));
}

RequestZrangebyscore PipelineImpl::Zrangebyscore(
    std::string key,
    double min,
    double max,
...
RequestZrangebyscoreWithScores PipelineImpl::ZrangebyscoreWithScores(std::string key, double min, double max) {
    UpdateShard(key);
...
RequestZrangebyscoreWithScores PipelineImpl::ZrangebyscoreWithScores(
    std::string key,
    std::string min,
    std::string max
) {
...
RequestZrem PipelineImpl::Zrem(std::string key, std::string member) {
    UpdateShard(key);
    return AddCmd<RequestZrem>("zrem", true, std::move(key), std::move(member));
}

RequestZrem PipelineImpl::Zrem(std::string key, std::vector<std::string> members) {
    UpdateShard(key);
    return AddCmd<RequestZrem>("zrem", true, std::move(key), std::move(members));
}

RequestZremrangebyrank PipelineImpl::Zremrangebyrank(std::string key, int64_t start, int64_t stop) {
    UpdateShard(key);
    return AddCmd<RequestZremrangebyrank>("zremrangebyrank", true, std::move(key), start, stop);
}

RequestZremrangebyscore PipelineImpl::Zremrangebyscore(std::string key, double min, double max) {
    UpdateShard(key);
    return AddCmd<RequestZremrangebyscore>("zremrangebyscore", true, std::move(key), min, max);
}

RequestZremrangebyscore PipelineImpl::Zremrangebyscore(std::string key, std::string min, std::string max) {
    UpdateShard(key);
    return AddCmd<RequestZremrangebyscore>("zremrangebyscore", true, std::move(key), std::move(min), std::move(max));
}

RequestZscore PipelineImpl::Zscore(std::string key, std::string member) {
    UpdateShard(key);
...
void PipelineImpl::UpdateShard(const std::string& key) {
    try {
...
void PipelineImpl::UpdateShard(const std::vector<std::string>& keys) {
    for (const auto& key : keys) {
        UpdateShard(key);
    }
}

void PipelineImpl::UpdateShard(const std::vector<std::pair<std::string, std::string>>& key_values) {
    for (const auto& key_value : key_values) {
        UpdateShard(key_value.first);
    }
}

void PipelineImpl::UpdateShard(size_t shard) {
    if (shard_) {
        if (check_shards_ == CheckShards::kSame && *shard_ != shard) {
            std::ostringstream os;
            os << "storages::redis::Pipeline must deal with the same shard across "
...
Request<Result, ReplyType> PipelineImpl::DoAddCmd(To<Request<Result, ReplyType>> to) {
...
Request PipelineImpl::AddCmd(std::string command, bool master, Args&&... args) {
    master_ |= master;
