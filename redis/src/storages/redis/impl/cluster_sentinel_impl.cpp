...
void ClusterTopologyHolder::CreateNodes() {
    ...
    for (auto&& host_port : host_ports_to_create) {
        ...
        instance->signal_state_change.connect([host_port,
...
public:
    ...
    bool WaitReadyOnce(engine::Deadline deadline, WaitConnectedMode mode) override {
        ...
        std::unique_lock<std::mutex> lock(mutex_);
        return cv_.WaitUntil(lock, deadline, [this, mode]() {
            if(auto node = node_.Read(); !node->has_value()) return false;
            auto ptr = topology_.Read();
            return ptr->IsReady(mode);
        });
    }
...
private:
    std::shared_ptr<RedisConnectionHolder> CreateRedisInstance(const ConnectionInfo& info) {
...
void ClusterSentinelImpl::Init() {
    topology_holder_->GetSignalNodeStateChanged().connect(
        [this, topology_holder = std::weak_ptr(topology_holder_)](std::string host_port, Redis::State /*state*/) {
