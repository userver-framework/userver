...
StandaloneImpl::StandaloneImpl(
    ...
    : ev_thread_(sentinel_thread_control),
      ...
      connection_holder_(new RedisConnectionHolder(
          ...
          makeRedisCreationSettings()
      )),
      master_shard_(kUnknownShard, connection_holder_, {}) {
    // https://github.com/boostorg/signals2/issues/59
    // NOLINTNEXTLINE(clang-analyzer-cplusplus.NewDelete)
    Init();
    LOG_DEBUG() << "Created StandaloneImpl, shard_group_name=" << shard_group_name_;
}

StandaloneImpl::~StandaloneImpl() { Stop(); }

std::unordered_map<ServerId, size_t, ServerIdHasher> StandaloneImpl::GetAvailableServersWeighted(
    size_t /*shard_idx*/,
    bool with_master,
    ...
    if (!redis_conn || !redis_conn->IsAvailable() ||
        (!cc.force_server_id.IsAny() && redis_conn->GetServerId() != cc.force_server_id)) {
        return {};
    }

    return {std::make_pair(redis_conn->GetServerId(), 1)};
}
...
void StandaloneImpl::WaitConnectedOnce(RedisWaitConnected wait_connected) {
    ...
    while (!master_shard_.IsReady(wait_connected.mode) && !deadline.IsReached()) {
...
void StandaloneImpl::ForceUpdateHosts() { throw std::runtime_error(std::string(__func__) + " Unimplemented yet"); }

void StandaloneImpl::Init() {}

void StandaloneImpl::AsyncCommand(const SentinelCommand& scommand, size_t prev_instance_idx) {
    ...
    CommandPtr const command_check_errors(PrepareCommand(
        ...
        [this, shard, master, start, counter, command](const CommandPtr& ccommand, ReplyPtr reply) {
            ...
            // TODO - скорее всего такой ошибки не в кластере не может просто быть, потому и обработчик здесь этот не
            // нужен
            const bool error_moved = reply->data.IsErrorMoved();
            if (error_moved) {
                const auto& args = ccommand->args.args;
                LOG_WARNING() << "MOVED" << reply->status_string << " c.instance_idx:" << ccommand->instance_idx
...
const std::string& StandaloneImpl::GetAnyKeyForShard(size_t /*shard_idx*/) const {
    throw std::runtime_error("GetAnyKeyForShard() is not supported in redis cluster mode");
}

void StandaloneImpl::Start() { process_waiting_commands_timer_->Start(); }
...
std::vector<std::shared_ptr<const Shard>> StandaloneImpl::GetMasterShards() const {
...
void StandaloneImpl::SetCommandsBufferingSettings(CommandsBufferingSettings commands_buffering_settings) {
    connection_holder_->SetCommandsBufferingSettings(std::move(commands_buffering_settings));
}

void StandaloneImpl::SetReplicationMonitoringSettings(const ReplicationMonitoringSettings& monitoring_settings) {
    connection_holder_->SetReplicationMonitoringSettings(std::move(monitoring_settings));
}

void StandaloneImpl::SetRetryBudgetSettings(const utils::RetryBudgetSettings& settings) {
    connection_holder_->SetRetryBudgetSettings(std::move(settings));
}

SentinelStatistics StandaloneImpl::GetStatistics(const MetricsSettings& settings) const {
...
size_t StandaloneImpl::ShardsCount() const { return 1; }

size_t StandaloneImpl::GetClusterSlotsCalledCounter() { return 0; }
