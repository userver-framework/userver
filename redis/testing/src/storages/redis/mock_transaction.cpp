...
RequestBitop MockTransaction::Bitop(BitOperation op, std::string dest, std::vector<std::string> srcs) {
    UpdateShard(dest);
...
RequestExpire MockTransaction::Expire(std::string key, std::chrono::seconds ttl) {
    UpdateShard(key);
...
RequestHdel MockTransaction::Hdel(std::string key, std::vector<std::string> fields) {
    UpdateShard(key);
...
RequestHincrby MockTransaction::Hincrby(std::string key, std::string field, int64_t increment) {
    UpdateShard(key);
    return AddSubrequest(impl_->Hincrby(std::move(key), std::move(field), increment));
}

RequestHincrbyfloat MockTransaction::Hincrbyfloat(std::string key, std::string field, double increment) {
    UpdateShard(key);
...
RequestHmget MockTransaction::Hmget(std::string key, std::vector<std::string> fields) {
    UpdateShard(key);
    return AddSubrequest(impl_->Hmget(std::move(key), std::move(fields)));
}

RequestHmset MockTransaction::Hmset(std::string key, std::vector<std::pair<std::string, std::string>> field_values) {
    UpdateShard(key);
    return AddSubrequest(impl_->Hmset(std::move(key), std::move(field_values)));
}

RequestHset MockTransaction::Hset(std::string key, std::string field, std::string value) {
    UpdateShard(key);
    return AddSubrequest(impl_->Hset(std::move(key), std::move(field), std::move(value)));
}

RequestHsetnx MockTransaction::Hsetnx(std::string key, std::string field, std::string value) {
    UpdateShard(key);
...
RequestLpush MockTransaction::Lpush(std::string key, std::vector<std::string> values) {
    UpdateShard(key);
...
RequestLrange MockTransaction::Lrange(std::string key, int64_t start, int64_t stop) {
    UpdateShard(key);
    return AddSubrequest(impl_->Lrange(std::move(key), start, stop));
}

RequestLrem MockTransaction::Lrem(std::string key, int64_t count, std::string element) {
    UpdateShard(key);
    return AddSubrequest(impl_->Lrem(std::move(key), count, std::move(element)));
}

RequestLtrim MockTransaction::Ltrim(std::string key, int64_t start, int64_t stop) {
    UpdateShard(key);
...
RequestMset MockTransaction::Mset(std::vector<std::pair<std::string, std::string>> key_values) {
...
RequestPexpire MockTransaction::Pexpire(std::string key, std::chrono::milliseconds ttl) {
    UpdateShard(key);
...
RequestPingMessage MockTransaction::PingMessage(size_t shard, std::string message) {
    UpdateShard(shard);
...
RequestRpush MockTransaction::Rpush(std::string key, std::vector<std::string> values) {
    UpdateShard(key);
...
RequestSadd MockTransaction::Sadd(std::string key, std::vector<std::string> members) {
    UpdateShard(key);
...
RequestSet MockTransaction::Set(std::string key, std::string value, std::chrono::milliseconds ttl) {
    UpdateShard(key);
    return AddSubrequest(impl_->Set(std::move(key), std::move(value), ttl));
}

RequestSetIfExist MockTransaction::SetIfExist(std::string key, std::string value) {
    UpdateShard(key);
    return AddSubrequest(impl_->SetIfExist(std::move(key), std::move(value)));
}

RequestSetIfExist MockTransaction::SetIfExist(std::string key, std::string value, std::chrono::milliseconds ttl) {
    UpdateShard(key);
    return AddSubrequest(impl_->SetIfExist(std::move(key), std::move(value), ttl));
}

RequestSetIfNotExist MockTransaction::SetIfNotExist(std::string key, std::string value) {
    UpdateShard(key);
    return AddSubrequest(impl_->SetIfNotExist(std::move(key), std::move(value)));
}

RequestSetIfNotExist MockTransaction::SetIfNotExist(std::string key, std::string value, std::chrono::milliseconds ttl) {
    UpdateShard(key);
...
RequestSrandmembers MockTransaction::Srandmembers(std::string key, int64_t count) {
    UpdateShard(key);
...
RequestSrem MockTransaction::Srem(std::string key, std::vector<std::string> members) {
    UpdateShard(key);
...
RequestZadd MockTransaction::Zadd(std::string key, double score, std::string member) {
    UpdateShard(key);
    return AddSubrequest(impl_->Zadd(std::move(key), score, std::move(member)));
}

RequestZadd MockTransaction::Zadd(std::string key, double score, std::string member, const ZaddOptions& options) {
    UpdateShard(key);
    return AddSubrequest(impl_->Zadd(std::move(key), score, std::move(member), options));
}

RequestZadd MockTransaction::Zadd(std::string key, std::vector<std::pair<double, std::string>> scored_members) {
    UpdateShard(key);
...
RequestZaddIncr MockTransaction::ZaddIncr(std::string key, double score, std::string member) {
    UpdateShard(key);
    return AddSubrequest(impl_->ZaddIncr(std::move(key), score, std::move(member)));
}

RequestZaddIncrExisting MockTransaction::ZaddIncrExisting(std::string key, double score, std::string member) {
    UpdateShard(key);
...
RequestZrange MockTransaction::Zrange(std::string key, int64_t start, int64_t stop) {
    UpdateShard(key);
    return AddSubrequest(impl_->Zrange(std::move(key), start, stop));
}

RequestZrangeWithScores MockTransaction::ZrangeWithScores(std::string key, int64_t start, int64_t stop) {
    UpdateShard(key);
    return AddSubrequest(impl_->ZrangeWithScores(std::move(key), start, stop));
}

RequestZrangebyscore MockTransaction::Zrangebyscore(std::string key, double min, double max) {
    UpdateShard(key);
    return AddSubrequest(impl_->Zrangebyscore(std::move(key), min, max));
}

RequestZrangebyscore MockTransaction::Zrangebyscore(std::string key, std::string min, std::string max) {
    UpdateShard(key);
...
RequestZrangebyscoreWithScores MockTransaction::ZrangebyscoreWithScores(std::string key, double min, double max) {
    UpdateShard(key);
...
RequestZrem MockTransaction::Zrem(std::string key, std::vector<std::string> members) {
    UpdateShard(key);
    return AddSubrequest(impl_->Zrem(std::move(key), std::move(members)));
}

RequestZremrangebyrank MockTransaction::Zremrangebyrank(std::string key, int64_t start, int64_t stop) {
    UpdateShard(key);
    return AddSubrequest(impl_->Zremrangebyrank(std::move(key), start, stop));
}

RequestZremrangebyscore MockTransaction::Zremrangebyscore(std::string key, double min, double max) {
    UpdateShard(key);
    return AddSubrequest(impl_->Zremrangebyscore(std::move(key), min, max));
}

RequestZremrangebyscore MockTransaction::Zremrangebyscore(std::string key, std::string min, std::string max) {
    UpdateShard(key);
...
void MockTransaction::UpdateShard(const std::vector<std::pair<std::string, std::string>>& key_values) {
