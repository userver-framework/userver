...
TEST(OdbcBulkBindings, StoresEmptyStringAndBytesWithoutCopyingNullPointers) {
    const auto rows = MakeRows({
        {impl::Parameter{std::string{}}, impl::Parameter{Bytes{}}},
        {impl::Parameter{std::string{"x"}}, impl::Parameter{Bytes{1}}},
    });
    const auto layout = detail::ValidateBulkRows(rows);

    auto bindings = detail::TryBuildBulkBindings(rows, layout, 0, 2);
    ASSERT_TRUE(bindings);
    ASSERT_EQ(bindings->columns.size(), 2);

    const auto& strings = std::get<detail::BulkInlineValues>(bindings->columns[0].values);
    EXPECT_EQ(strings.stride, 1);
    EXPECT_EQ(bindings->columns[0].indicators[0], 0);
    EXPECT_EQ(bindings->columns[0].indicators[1], 1);

    const auto& bytes = std::get<detail::BulkInlineValues>(bindings->columns[1].values);
    EXPECT_EQ(bytes.stride, 1);
    EXPECT_EQ(bindings->columns[1].indicators[0], 0);
    EXPECT_EQ(bindings->columns[1].indicators[1], 1);
}
