...
void ConfigureDriverManager(Connection::EnvironmentHandle& environment) {
    const auto result =
        SQLSetEnvAttr(environment.get(), SQL_ATTR_CONNECTION_POOLING, reinterpret_cast<SQLPOINTER>(SQL_CP_OFF), 0);
    if (!SQL_SUCCEEDED(result)) {
        throw MakeDriverError<ConnectionError>(
            "Failed to disable ODBC driver-manager connection pooling",
            result,
            environment.get(),
            SQL_HANDLE_ENV
        );
...
std::string DecodeNumericMagnitude(const SQL_NUMERIC_STRUCT& value) {
    ...
    for (const auto decimal_digit : std::ranges::reverse_view(decimal_digits)) {
        result.push_back(static_cast<char>('0' + decimal_digit));
    }
    return result;
}
...
detail::ResultWrapper::Cell ReadDecimalCell(
    ...
    if (scale != 0) {
        if (representation.size() <= scale) {
            representation.insert(std::size_t{0}, scale + 1 - representation.size(), '0');
        }
...
void ValidateBulkDmlStatement(SQLHSTMT statement, std::size_t expected_parameters, engine::Deadline deadline) {
...
void CleanupBulkStatement(SQLHSTMT statement) {
    ...
    const auto verify_pointer_attribute = [&](SQLINTEGER attribute, std::string_view operation) {
        auto actual = reinterpret_cast<SQLPOINTER>(static_cast<std::uintptr_t>(1));
        const auto result = SQLGetStmtAttr(statement, attribute, &actual, sizeof(actual), nullptr);
        auto diagnostics =
            result == SQL_SUCCESS
                ? std::vector<DiagnosticRecord>{}
      ...
      cursor_control_{std::make_shared<detail::CursorControl>(*this)}
{
    ...
    auto handles = RunBlocking(blocking_task_processor_, [dsn, deadline] {
        ...
        ConfigureDriverManager(environment);
        auto result =
        ...
        return guard.Execute([&] {
            return RunBlockingChecked(blocking_task_processor_, deadline, [&, query_text = std::string{statement}] {
                ...
                const auto prepare_current_statement = [&](bool must_prepare) {
                    ...
                    const auto expected_parameters = rows.empty() ? layout.columns.size() : rows.front().size();
                    ValidateBulkDmlStatement(statement_handle.get(), expected_parameters, deadline);
