...
std::size_t NormalizeMinPoolSize(std::size_t min_pool_size) {
    // unixODBC dlopens the driver on the first SQLDriverConnect. userver forbids
    // dlopen after the component system has started, so always open at least one
    // connection during construction.
    return min_pool_size == 0 ? 1 : min_pool_size;
}
...
Pool::Pool(
    ...
    : ConnectionPoolBase<Connection, Pool>(max_pool_size, kMaxSimultaneouslyConnectingClients),
      dsns_({dsn}),
      min_pool_size_(NormalizeMinPoolSize(min_pool_size)),
