...
ConnectionPtr Pool::Acquire(engine::Deadline deadline) {
    ...
    stats_.acquire_percentile.GetCurrentCounter().Account(elapsed.count());
...
Pool::ConnectionUniquePtr Pool::DoCreateConnection(engine::Deadline deadline) {
    ...
    try {
        ...
        stats_.connection_percentile.GetCurrentCounter().Account(elapsed.count());
...
void Pool::AccountQueryExecuted(std::chrono::microseconds duration) noexcept {
    ...
    stats_.transaction.busy_percentile.GetCurrentCounter().Account(duration.count());
}

void Pool::AccountQueryError() noexcept {
    ++stats_.transaction.error_execute_total;
}

void Pool::AccountQueryTimeout() noexcept {
    ++stats_.transaction.execute_timeout;
}

void Pool::AccountOutOfTransaction() noexcept {
    ++stats_.transaction.out_of_trx_total;
}

void Pool::AccountTransactionStarted() noexcept {
    ++stats_.transaction.total;
}

void Pool::AccountTransactionCommit(std::chrono::microseconds total_duration, std::chrono::microseconds busy_duration) noexcept {
    ++stats_.transaction.commit_total;
    stats_.transaction.total_percentile.GetCurrentCounter().Account(total_duration.count());
    stats_.transaction.busy_percentile.GetCurrentCounter().Account(busy_duration.count());
}

void Pool::AccountTransactionRollback() noexcept {
    ++stats_.transaction.rollback_total;
}
