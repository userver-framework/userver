...
async def _statement_metric_lines(monitor_client):
    ...
    return [
        line for line in metrics.splitlines() if line.startswith('odbc.statement_') and _STATEMENT_QUERY_LABEL in line
    ]
...
def _prepared_cache_current(lines):
    line = next(line for line in lines if line.startswith('odbc.connections.prepared-statements'))
