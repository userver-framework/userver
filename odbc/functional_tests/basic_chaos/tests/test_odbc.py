...
async def _check_that_restores(client, gate, url=CHAOS_URL):
...
async def test_odbc_happy_path(service_client, gate):
    await _check_crud(service_client)


async def test_odbc_transaction_happy_path(service_client, gate):
    await _check_crud(service_client, url=CHAOS_TRX_URL)


async def test_odbc_transaction_multi_statement(service_client, gate):
...
async def test_odbc_transaction_update_and_read(service_client, gate):
...
async def test_odbc_transaction_delete_in_trx(service_client, gate):
...
async def test_odbc_transaction_read_own_write(service_client, gate):
...
async def test_odbc_transaction_sequential(service_client, gate):
...
async def test_odbc_transaction_close_on_execute(service_client, gate):
    ...
    await gate.sockets_close()
    await _check_that_restores(service_client, gate, url=CHAOS_TRX_URL)
