...
_REQUEST_BODY_RE = re.compile(r'/paths/\[([^\]]*)\]/([a-zA-Z]*)/requestBody(/schema)?')
_REQUEST_BODY_CONTENT_RE = re.compile(r'/paths/\[([^\]]*)\]/([a-zA-Z]*)/requestBody/content/\[([^\]]*)\]/schema')
_RESPONSE_HEADER_RE = re.compile(r'/paths/\[([^\]]*)\]/([a-zA-Z]*)/responses/([0-9]*)/headers/([-a-zA-Z0-9_]*)/schema')
_COMPONENTS_RESPONSE_CONTENT_RE = re.compile(r'/components/responses/([-a-zA-Z_0-9]*)/content/(.*)/schema')
_COMPONENTS_RESPONSE_HEADER_RE = re.compile(r'/components/responses/([-a-zA-Z_0-9]*)/headers/([^/]*)/schema')
_PARAMETER_RE = re.compile(r'/paths/\[([^\]]*)\]/([a-zA-Z]*)/parameters/([0-9]*)(/schema)?')
_RESPONSE_SCHEMA_RE = re.compile(r'/paths/\[([^\]]*)\]/([a-zA-Z]*)/responses/([0-9]*)/schema')
_RESPONSE_CONTENT_RE = re.compile(r'/paths/\[([^\]]*)\]/([a-zA-Z]*)/responses/([0-9]*)/content/(.*)/schema')
_REQUEST_BODIES_RE = re.compile(r'/components/requestBodies/([-a-zA-Z0-9_]*)/content/\[([^\]]*)\]/schema')
_PARAMETERS_ROOT_RE = re.compile(r'/parameters/([-a-zA-Z0-9_]*)')
_COMPONENTS_PARAMETER_RE = re.compile(r'/components/parameters/([-a-zA-Z0-9_]*)/schema')
_COMPONENTS_HEADER_RE = re.compile(r'/components/headers/([-a-zA-Z0-9]*)/schema')
...
class BaseTranslator(abc.ABC):
    def __init__(
        ...
        *,
        ...
        try:
            self._translate(service)
        except chaotic_error.BaseError:
            raise
        except BaseException as exc:
            if isinstance(exc, extra_reraise):
                raise
    ...
    def spec(self) -> common_types.SpecBase:
        return self._spec

    def _translate_middlewares(self, x_middlewares: base_model.XMiddlewares) -> list:
        return []

    def _transform_path(self, path: str) -> str:
        return re.sub(DASH_IN_PATH_RE, '_', path)

    def _init_schemas(self, service: model.Service) -> None:
    ...
    def _translate(self, service: model.Service) -> None:
        ...
        for op in service.operations:
            if not self.should_generate_operation(op):
                continue

            if isinstance(op.requestBody, model.Ref):
                request_bodies = [
                    self._translate_request_body(body) for body in self._raw_request_bodies[op.requestBody.ref]
                ]
            else:
                request_bodies = [self._translate_request_body(body) for body in op.requestBody]

            translated_op = common_types.Operation(
                method=op.method.upper(),
                path=self._transform_path(op.path),
                operation_id=op.operationId,
                description=op.description,
                parameters=[self._translate_parameter(parameter) for parameter in op.parameters],
                request_bodies=request_bodies,
                responses=[self._translate_response(r, status) for status, r in op.responses.items()],
            ...
            self._spec.operations.append(translated_op)
    ...
    def map_infile_path_to_cpp_type(self, name: str, stem: str) -> str:
        ...
        match = _REQUEST_BODY_RE.fullmatch(name)
        if match:
            return '{}::{}::{}::Body'.format(
                self._spec.cpp_namespace,
                cpp_names.namespace(match.group(1)),
                common_types.map_method(match.group(2)),
            )

        match = _REQUEST_BODY_CONTENT_RE.fullmatch(name)
        if match:
            return '{}::{}::{}::Body{}'.format(
                self._spec.cpp_namespace,
                cpp_names.namespace(match.group(1)),
        ...
        match = _RESPONSE_HEADER_RE.fullmatch(name)
        if match:
        ...
        match = _COMPONENTS_RESPONSE_CONTENT_RE.fullmatch(name)
        if match:
        ...
        match = _COMPONENTS_RESPONSE_HEADER_RE.fullmatch(name)
        if match:
            return '{}::Response{}Header{}'.format(
                self._spec.cpp_namespace,
                match.group(1),
        ...
        match = _PARAMETER_RE.fullmatch(name)
        if match:
        ...
        match = _RESPONSE_SCHEMA_RE.fullmatch(name)
        if match:
            return '{}::{}::{}::Response{}Body'.format(
                self._spec.cpp_namespace,
                cpp_names.namespace(match.group(1)),
        ...
        match = _RESPONSE_CONTENT_RE.fullmatch(name)
        if match:
            return '{}::{}::{}::Response{}Body{}'.format(
                self._spec.cpp_namespace,
                cpp_names.namespace(match.group(1)),
        ...
        match = _REQUEST_BODIES_RE.fullmatch(name)
        if match:
            return '{}::{}{}'.format(
                self._spec.cpp_namespace,
        ...
        match = _PARAMETERS_ROOT_RE.fullmatch(name)
        if match:
            return '{}::Parameter{}'.format(
                self._spec.cpp_namespace,
                cpp_names.camel_case(
        ...
        match = _COMPONENTS_PARAMETER_RE.fullmatch(name)
        if match:
            return '{}::Parameter{}'.format(
                self._spec.cpp_namespace,
                cpp_names.camel_case(
        ...
        match = _COMPONENTS_HEADER_RE.fullmatch(name)
        if match:
    ...
    def _translate_response(
        ...
        return common_types.Response(
            status=status,
            headers=headers,
            body=body,
        )

    def _translate_request_body(self, request_body: model.RequestBody) -> common_types.Body:
    ...
    def _translate_parameter(self, parameter: model.Parameter) -> common_types.Parameter:
        in_ = parameter.in_
