...
Component::Component(const USERVER_NAMESPACE::components::ComponentConfig& config,
                     const USERVER_NAMESPACE::components::ComponentContext& context)
    : USERVER_NAMESPACE::components::ComponentBase(config, context),
      client_(USERVER_NAMESPACE::chaotic::openapi::client::ParseConfig(config, ClientImpl::kDefaultBaseUrl),
  ...
  if (config.HasMember("middlewares")) {
    ...
    for (const auto& [name, config] : USERVER_NAMESPACE::formats::common::Items(mw_config)) {
      auto& factory = context.FindComponent<USERVER_NAMESPACE::chaotic::openapi::client::MiddlewareFactory>(
          "chaotic-client-middleware-" + name);
    ...
    if (names) {
      ...
      for (const auto& name : *names) {
        auto& component = context.FindComponent<USERVER_NAMESPACE::clients::http::middlewares::ComponentBase>(name);
