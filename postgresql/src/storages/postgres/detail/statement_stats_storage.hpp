...
private:
    struct StatementEvent final {
        StatementEvent(const std::string& statement_name_, size_t duration_, ExecutionResult result_)
            : statement_name{statement_name_}, duration_ms{duration_}, result{result_} {}
    ...
    using StorageType = USERVER_NAMESPACE::cache::LruMap<std::string, std::unique_ptr<StoredStatementStats>>;
    using Storage = USERVER_NAMESPACE::concurrent::Variable<StorageType, engine::SharedMutex>;
    ...
    static StorageData CreateStorageData(size_t max_map_size, size_t max_queue_size);
