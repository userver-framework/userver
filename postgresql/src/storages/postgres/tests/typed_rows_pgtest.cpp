...
public:
    auto Introspect() { return std::tie(int_member_, string_member_, double_member_); }
...
static_assert(tt::kRowCategory<std::vector<std::string>> == tt::RowCategoryType::kNonRow);
static_assert(tt::kRowCategory<pg::MultiPrecision<50>> == tt::RowCategoryType::kNonRow);

static_assert(tt::kRowCategory<MyTupleType> == tt::RowCategoryType::kTuple);
static_assert(tt::kRowCategory<MyAggregateStruct> == tt::RowCategoryType::kAggregate);
static_assert(tt::kRowCategory<MyIntrusiveClass> == tt::RowCategoryType::kIntrusiveIntrospection);
static_assert(tt::kRowCategory<MyPolymorphicBase> == tt::RowCategoryType::kNonRow);
static_assert(tt::kRowCategory<MyPolymorphicDerived> == tt::RowCategoryType::kNonRow);
static_assert(tt::kRowCategory<MyPolymorphicInrospected> == tt::RowCategoryType::kIntrusiveIntrospection);
...
UTEST_P(PostgreConnection, TypedResult) {
    ...
    UEXPECT_NO_THROW(res = GetConn()->Execute("select $1, $2, $3", 42, "foobar", 3.14));
...
UTEST_P(PostgreConnection, EmptyTypedResult) {
    ...
    CheckConnection(GetConn());
    auto empty_res = GetConn()->Execute("select $1, $2, $3 limit 0", 42, "foobar", 3.14);
