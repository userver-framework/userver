...
PgMessage ReadPgMessage(engine::io::Socket &sock) {
  ...
  std::memcpy(&message.length, message.full_message.data() + 1,
              sizeof(message.length));
  message.length = ntohl(message.length);

  if (message.length < 4) {
    throw std::runtime_error{"Invalid PG message length: " +
  ...
  if (message_body_size > 0 &&
      !RecvExact(sock, message.full_message.data() + kHeaderSize,
                 message_body_size)) {
    throw std::runtime_error{"Failed to read PG message body"};
  }

  message.body =
      std::span{message.full_message.data() + kHeaderSize, message_body_size};

  return message;
}

void WritePgMessage(const PgMessage &message, engine::io::Socket &sock) {
  if (sock.SendAll(message.full_message.data(), message.full_message.size(),
                   {}) != message.full_message.size()) {
...
void BackwardProxy(engine::io::Socket &src, engine::io::Socket &dst) {
  ...
  LOG_INFO() << "GSSAPI response: `" << gssapi_response << "` was sent";
...
PostgresChaosProxy::PostgresChaosProxy(engine::TaskProcessor &task_processor,
  ...
  proxy_socket_ = engine::io::Socket{engine::io::AddrDomain::kInet,
  ...
  acceptor_task_ = engine::AsyncNoTracing(
      task_processor_, &PostgresChaosProxy::AcceptorLoop, this);
}
...
void PostgresChaosProxy::AcceptorLoop() {
  while (!engine::current_task::ShouldCancel()) {
    ...
    engine::io::Socket db_socket{postgres_addr_.Domain(),
                                 engine::io::SocketType::kTcp};
    db_socket.Connect(postgres_addr_, {});

    task_storage_.Detach(engine::AsyncNoTracing(
        task_processor_, &PostgresChaosProxy::ProcessSocket, this,
        std::move(proxied_socket), std::move(db_socket)));
  }
}

void PostgresChaosProxy::ProcessSocket(engine::io::Socket proxied_socket,
                                       engine::io::Socket db_socket) {
  auto forward_task =
      engine::AsyncNoTracing(task_processor_, &ForwardProxy,
                             std::ref(proxied_socket), std::ref(db_socket));
  auto backward_task =
      engine::AsyncNoTracing(task_processor_, &BackwardProxy,
                             std::ref(db_socket), std::ref(proxied_socket));
