...
const std::string kGetPostgresTypesSQL = kGetPostgresTypesSQLPrefix + "\norder by t.oid";
...
UTEST_P(PostgreConnection, PortalCreateFromTrxRecordArray) {
    ...
    pg::Transaction trx{std::move(GetConn()), pg::TransactionOptions{}};
    const std::string query = "select array_agg(t.typname) from pg_catalog.pg_type t";
...
UTEST_P(PostgreConnection, PortalInterleave) {
    ...
    CheckConnection(GetConn());
    EXPECT_NO_THROW(GetConn()->Execute(kQuery, 1));  // populating the statements cache
...
UTEST_P(PostgreConnection, PortalParallel) {
    ...
    auto second = trx.MakePortal("SELECT generate_series(10, $1, 10)", kIterations * 10);
