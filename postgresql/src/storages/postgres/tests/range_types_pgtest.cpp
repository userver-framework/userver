...
TEST(Postgre, RangeTypeEmpty) {
    ...
    EXPECT_FALSE(r.IsLowerBoundIncluded()) << "Empty range doesn't include the lower bound";
    EXPECT_FALSE(r.IsUpperBoundIncluded()) << "Empty range doesn't include the upper bound";
...
TEST(Postgre, RangeTypeIncludeLower) {
    ...
    EXPECT_TRUE(r.IsLowerBoundIncluded()) << "Range [0, 1) includes the lower bound";
    EXPECT_FALSE(r.IsUpperBoundIncluded()) << "Range [0, 1) doesn't include the upper bound";
}
...
TEST(Postgre, RangeTypeIncludeUpper) {
    ...
    EXPECT_TRUE(r.HasLowerBound()) << "Range (0, 1] has a lower bound";
    EXPECT_TRUE(r.HasUpperBound()) << "Range (0, 1] has an upper bound";
    EXPECT_FALSE(r.IsLowerBoundIncluded()) << "Range (0, 1] doesn't include the lower bound";
    EXPECT_TRUE(r.IsUpperBoundIncluded()) << "Range (0, 1] includes the upper bound";
}
...
TEST(Postgre, RangeTypeUnboundedUpperIncludeLower) {
    ...
    EXPECT_TRUE(r.IsLowerBoundIncluded()) << "Range [0, ∞) includes the lower bound";
    EXPECT_FALSE(r.IsUpperBoundIncluded()) << "Range [0, ∞) doesn't include the upper bound";
}
...
TEST(Postgre, RangeTypeUnboundedUpperExcludeLower) {
    ...
    EXPECT_TRUE(r.HasLowerBound()) << "Range (0, ∞) has a lower bound";
    EXPECT_FALSE(r.HasUpperBound()) << "Range (0, ∞) doesn't have an upper bound";
    EXPECT_FALSE(r.IsLowerBoundIncluded()) << "Range (0, ∞) doesn't include the lower bound";
    EXPECT_FALSE(r.IsUpperBoundIncluded()) << "Range (0, ∞) doesn't include the upper bound";
}
...
TEST(Postgre, RangeTypeUnboundedLowerIncludeUpper) {
    ...
    EXPECT_FALSE(r.IsLowerBoundIncluded()) << "Range (-∞, 0] doesn't include the lower bound";
    EXPECT_TRUE(r.IsUpperBoundIncluded()) << "Range (-∞, 0] includes the upper bound";
}
...
TEST(Postgre, RangeTypeUnboundedBoth) {
    ...
    EXPECT_FALSE(r.IsLowerBoundIncluded()) << "Range (-∞, ∞) doesn't include the lower bound";
    EXPECT_FALSE(r.IsUpperBoundIncluded()) << "Range (-∞, ∞) doesn't include the upper bound";
}

TEST(Postgre, RangeEquality) {
    EXPECT_EQ(pg::IntegerRange{}, pg::IntegerRange{});
    EXPECT_EQ((pg::IntegerRange{pg::kUnbounded, pg::kUnbounded}), (pg::IntegerRange{pg::kUnbounded, pg::kUnbounded}));
    EXPECT_NE(pg::IntegerRange{}, (pg::IntegerRange{pg::kUnbounded, pg::kUnbounded}));
    ...
    EXPECT_EQ(pg::MakeRange(1, pg::kUnbounded), pg::MakeRange(0, pg::kUnbounded, pg::RangeBound::kNone));

    EXPECT_EQ(pg::MakeRange(pg::kUnbounded, 0), pg::MakeRange(pg::kUnbounded, 0));
    EXPECT_EQ(pg::MakeRange(pg::kUnbounded, 1), pg::MakeRange(pg::kUnbounded, 0, pg::RangeBound::kUpper));
}
...
UTEST_P(PostgreConnection, Int4RangeRoundtripTest) {
    ...
    const TestData<pg::IntegerRange> test_data[]{
        ...
        {"(0, 2]", "upper-inclusive", pg::MakeRange(0, 2, pg::RangeBound::kUpper)},
    ...
    for (const auto& test : test_data) {
        UEXPECT_NO_THROW(
            res = GetConn()->Execute("select '" + test.select_expression + "'::int4range, '" + test.description + "'")
        ) << "Select "
          << test.description << " range from database";
        UEXPECT_NO_THROW(res.Front()[0].To(r)) << "Parse " << test.select_expression << " value";
        EXPECT_EQ(test.expected, r) << "Expect equality for " << test.description << " range";
...
UTEST_P(PostgreConnection, Int8RangeRoundtripTest) {
    ...
    for (const auto& test : test_data) {
        UEXPECT_NO_THROW(
            res = GetConn()->Execute("select '" + test.select_expression + "'::int8range, '" + test.description + "'")
        ) << "Select "
          << test.description << " range from database";
        UEXPECT_NO_THROW(res.Front()[0].To(r)) << "Parse " << test.select_expression << " value";
        EXPECT_EQ(test.expected, r) << "Expect equality for " << test.description << " range";
...
UTEST_P(PostgreConnection, BoundedInt8RangeRoundtripTest) {
    ...
    const TestData<pg::BoundedBigintRange> test_data[]{
        ...
        {"[0, 2]", "both-inclusive", pg::BoundedBigintRange(0L, 2L, pg::RangeBound::kBoth)},
    };
    for (const auto& test : test_data) {
...
UTEST_P(PostgreConnection, RangeStored) {
    ...
    UEXPECT_NO_THROW(res = GetConn()->Execute("select $1, $2", pg::ParameterStore{}.PushBack(exp1).PushBack(exp2)));
