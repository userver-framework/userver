...
TEST_P(Fail, InvalidDSN) { UEXPECT_THROW(pg::SplitByHost(GetParam()), pg::InvalidDSN); }
...
void PrintTo(const TestData& td, std::ostream* os) {
    *os << "TestData{original_dsn=`" << td.original_dsn.GetUnderlying() << "`; dsn_params_count=" << td.dsn_params_count
        << "; host=`" << td.host << "`; port=`" << td.port << "`; dbname=`" << td.dbname << "`}";
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        ...
        TestData{pg::Dsn{"host=localhost port=5432 dbname=mydb connect_timeout=10"}, 1, "localhost", "5432", "mydb"},
        TestData{
            pg::Dsn{"host=localhost,host1 port=5432,5433 dbname=mydb "
                    "connect_timeout=10"},
            2,
            "localhost",
            "5432",
        ...
        TestData{pg::Dsn{"postgresql://%2Fvar%2Flib%2Fpostgresql/dbname"}, 1, "/var/lib/postgresql", "", "dbname"}
    )
);
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        ...
        std::make_pair(pg::Dsn{"host=localhost password="}, "host=localhost password="),
        std::make_pair(pg::Dsn{"host=localhost password=pwd"}, "host=localhost password=***"),
        std::make_pair(pg::Dsn{"host=localhost password = pwd"}, "host=localhost password = ***"),
        std::make_pair(pg::Dsn{"host=localhost password=p%24wd"}, "host=localhost password=***"),
        std::make_pair(pg::Dsn{"host=localhost password='my secret'"}, "host=localhost password=***"),
        std::make_pair(pg::Dsn{"host=localhost password='my \\' secret'"}, "host=localhost password=***"),
        std::make_pair(pg::Dsn{"host=localhost password = 'my \\' secret'"}, "host=localhost password = ***"),
        std::make_pair(pg::Dsn{"host=localhost password=pwd user=user"}, "host=localhost password=*** user=user"),
        std::make_pair(pg::Dsn{"postgresql://localhost"}, "postgresql://localhost"),
        std::make_pair(pg::Dsn{"postgresql://user@localhost"}, "postgresql://user@localhost"),
        std::make_pair(
            pg::Dsn{"postgresql://@localhost"},
            "postgresql://@localhost"
        ),  // not actually a valid
            // connection string
        std::make_pair(
            pg::Dsn{"postgresql://:pwd@localhost"},
            "postgresql://:***@localhost"
        ),  // not actually a valid
            // connection string
        std::make_pair(
            ...
            // connection string
        std::make_pair(pg::Dsn{"postgresql://user:pwd@localhost"}, "postgresql://user:***@localhost"),
        std::make_pair(pg::Dsn{"postgresql://user:p%24wd@localhost"}, "postgresql://user:***@localhost"),
        std::make_pair(
