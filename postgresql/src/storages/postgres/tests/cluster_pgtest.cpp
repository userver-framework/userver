...
void CheckTransaction(pg::Transaction trx, CheckTxnType txn_type) {
    ...
    UEXPECT_NO_THROW(res = trx.Execute("SELECT current_setting('transaction_read_only')::bool"));
...
void CheckRoTransaction(pg::Transaction trx) { CheckTransaction(std::move(trx), CheckTxnType::kRo); }

void CheckRwTransaction(pg::Transaction trx) { CheckTransaction(std::move(trx), CheckTxnType::kRw); }
...
UTEST_F(PostgreCluster, ClusterEmptyPool) {
    ...
    auto cluster = CreateCluster(GetDsnListFromEnv(), GetTaskProcessor(), 0, testsuite_tasks);
...
UTEST_F(PostgreCluster, ClusterSyncSlaveRW) {
    ...
    UEXPECT_THROW(cluster.Begin(pg::ClusterHostType::kSyncSlave, {}), pg::ClusterUnavailable);
    UEXPECT_THROW(cluster.Begin(pg::ClusterHostType::kSyncSlave, pg::Transaction::RW), pg::ClusterUnavailable);
}
...
UTEST_F(PostgreCluster, ClusterMasterRW) {
    ...
    CheckRwTransaction(cluster.Begin(pg::ClusterHostType::kMaster, pg::Transaction::RW));
}
...
UTEST_F(PostgreCluster, ClusterAnyRW) {
    ...
    CheckRwTransaction(cluster.Begin({pg::ClusterHostType::kMaster, pg::ClusterHostType::kSlave}, {}));
    CheckRwTransaction(cluster.Begin({pg::ClusterHostType::kMaster, pg::ClusterHostType::kSlave}, pg::Transaction::RW));
    CheckRwTransaction(
        cluster.Begin({pg::ClusterHostType::kMaster, pg::ClusterHostType::kSlave, pg::ClusterHostType::kRoundRobin}, {})
    );
    CheckRwTransaction(cluster.Begin(
        {pg::ClusterHostType::kMaster, pg::ClusterHostType::kSlave, pg::ClusterHostType::kRoundRobin},
        pg::Transaction::RW
    ));
    CheckRwTransaction(
...
UTEST_F(PostgreCluster, ClusterSyncSlaveRO) {
    ...
    CheckRoTransaction(cluster.Begin(pg::ClusterHostType::kSyncSlave, pg::Transaction::RO));
}
...
UTEST_F(PostgreCluster, ClusterMasterRO) {
    ...
    CheckRoTransaction(cluster.Begin(pg::ClusterHostType::kMaster, pg::Transaction::RO));
}
...
UTEST_F(PostgreCluster, ClusterAnyRO) {
    ...
    CheckRoTransaction(cluster.Begin({pg::ClusterHostType::kSlave, pg::ClusterHostType::kMaster}, pg::Transaction::RO));
...
UTEST_F(PostgreCluster, SingleQuery) {
    ...
    pg::ResultSet res{nullptr};
    UEXPECT_NO_THROW(res = cluster.Execute(pg::ClusterHostType::kMaster, "select 1"));
    EXPECT_EQ(1, res.Size());
    UEXPECT_NO_THROW(res = cluster.Execute(pg::ClusterHostType::kSyncSlave, "select 1"));
    EXPECT_EQ(1, res.Size());
    UEXPECT_NO_THROW(res = cluster.Execute(pg::ClusterHostType::kSlave, "select 1"));
    EXPECT_EQ(1, res.Size());
    UEXPECT_NO_THROW(res = cluster.Execute({pg::ClusterHostType::kMaster, pg::ClusterHostType::kSlave}, "select 1"));
    EXPECT_EQ(1, res.Size());
}
...
UTEST_F(PostgreCluster, HostSelectionSingleQuery) {
    ...
    pg::ResultSet res{nullptr};
    UEXPECT_NO_THROW(
        res = cluster.Execute({pg::ClusterHostType::kSlave, pg::ClusterHostType::kRoundRobin}, "select 1")
    );
    EXPECT_EQ(1, res.Size());
    UEXPECT_NO_THROW(res = cluster.Execute({pg::ClusterHostType::kSlave, pg::ClusterHostType::kNearest}, "select 1"));
    EXPECT_EQ(1, res.Size());
    UEXPECT_NO_THROW(
        res = cluster.Execute(
...
UTEST_F(PostgreCluster, TransactionTimeouts) {
    ...
    }
    {
        ...
        auto trx = cluster.Begin(pg::Transaction::RW, kTestCmdCtl.WithStatementTimeout(std::chrono::milliseconds{250}));
        UEXPECT_NO_THROW(trx.Execute("select pg_sleep(0.1)"));
        trx.Commit();
    }
    {
        // Custom transaction timeout
        auto trx = cluster.Begin(pg::Transaction::RW, kTestCmdCtl.WithStatementTimeout(std::chrono::milliseconds{50}));
...
UTEST_F(PostgreCluster, NonTransactionExecuteWithParameterStore) {
    ...
    {
        auto res = cluster.Execute(pg::ClusterHostType::kMaster, "select $1", pg::ParameterStore{}.PushBack(1));
        EXPECT_EQ(1, res.Size());
    }
    {
        pg::CommandControl cc{std::chrono::milliseconds{50}, std::chrono::milliseconds{300}};
        auto res = cluster.Execute(pg::ClusterHostType::kMaster, cc, "select $1", pg::ParameterStore{}.PushBack(1));
        EXPECT_EQ(1, res.Size());
    }
}
...
UTEST_F(PostgreCluster, NonTransactionExecuteWithParameterStoreMove) {
...

    {
        ...
        auto res = cluster.Execute(pg::ClusterHostType::kMaster, "select $1", store_moved);
        EXPECT_EQ(1, res.Size());
    }
    {
        pg::CommandControl cc{std::chrono::milliseconds{50}, std::chrono::milliseconds{300}};
        ...
        store.PushBack(1);
        auto store_moved = std::move(store);
        auto res = cluster.Execute(pg::ClusterHostType::kMaster, cc, "select $1", store_moved);
        EXPECT_EQ(1, res.Size());
    }
}
...
UTEST_F(PostgreCluster, ListenNotify) {
    ...
    static const auto kNotifyDeadline = engine::Deadline::FromDuration(utest::kMaxTestWaitTime);

    testsuite::TestsuiteTasks testsuite_tasks{true};
    auto cluster = CreateCluster(GetDsnListFromEnv(), GetTaskProcessor(), 2, testsuite_tasks);

    auto scope = cluster.Listen(kListenChannel);

    UEXPECT_NO_THROW(cluster.Execute(pg::ClusterHostType::kMaster, "select pg_notify($1, NULL)", kListenChannel));
    UEXPECT_NO_THROW(
        cluster.Execute(pg::ClusterHostType::kMaster, "select pg_notify($1, $2)", kListenChannel, kNotifyPayload)
    );
