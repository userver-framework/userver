...
UTEST_P(PostgreConnection, InternalIntervalRoundtrip) {
    ...
    for (const auto& interval : intervals) {
        ...
        UEXPECT_NO_THROW(res = GetConn()->Execute("select interval '" + interval.str + "', '" + interval.str + "'"))
...
UTEST_P(PostgreConnection, IntervalRoundtrip) {
    ...
    pg::ResultSet res{nullptr};
    UEXPECT_NO_THROW(res = GetConn()->Execute("select $1", std::chrono::microseconds{1000}));
    ...
    UEXPECT_NO_THROW(res = GetConn()->Execute("select $1", std::chrono::seconds{-1}));
    UEXPECT_NO_THROW(res[0][0].To(us));
    UEXPECT_NO_THROW(res[0][0].To(ms));
...
UTEST_P(PostgreConnection, IntervalStored) {
    ...
    pg::ResultSet res{nullptr};
    UEXPECT_NO_THROW(
        res = GetConn()->Execute("select $1", pg::ParameterStore{}.PushBack(std::chrono::microseconds{1000}))
    );
    std::chrono::microseconds us;
    ...
    UEXPECT_NO_THROW(res = GetConn()->Execute("select $1", pg::ParameterStore{}.PushBack(std::chrono::seconds{-1})));
    UEXPECT_NO_THROW(res[0][0].To(us));
    EXPECT_EQ(std::chrono::seconds{-1}, us);
}
...
UTEST_P(PostgreConnection, ParamsStoreSample) {
    ...
    EXPECT_EQ(ParameterStoreSample(GetConn(), 1, 2, 3, 4, 5).AsSingleRow<int>(), 6);
    EXPECT_EQ(ParameterStoreSample(GetConn(), 1, 2, 3, {}, 5).AsSingleRow<int>(), 6);
    EXPECT_EQ(ParameterStoreSample(GetConn(), {}, 2, 3, {}, 5).AsSingleRow<int>(), 6);
    EXPECT_EQ(ParameterStoreSample(GetConn(), 1, {}, 3, {}, 5).AsSingleRow<int>(), 6);
    EXPECT_EQ(ParameterStoreSample(GetConn(), {}, {}, 3, {}).AsSingleRow<int>(), 6);
    EXPECT_EQ(ParameterStoreSample(GetConn(), {}, {}, {}, 4).AsSingleRow<int>(), 6);
}
}  // namespace
