...
template <>
struct CppToUserPg<static_test::MappedEnum> : EnumMappingBase<static_test::MappedEnum> {
    // dummy name, not actually used in test
    static constexpr DBTypeName postgres_name = "schema.name";
    static constexpr Enumerator enumerators[]{{EnumType::kOne, "one"}, {EnumType::kTwo, "two"}};
};
...
static_assert(tt::kIsMappedToPg<StringTypedef>, "Strong typedef must map to underlying type mapping");
static_assert(
...
static_assert(std::is_same<io::CppToPg<pg::TimePointTz>::Mapping, io::CppToSystemPg<pg::TimePointTz>>::value);
static_assert(std::is_same<tt::IO<pg::TimePointTz>::ParserType, io::BufferParser<pg::TimePointTz>>::value);
static_assert(std::is_same<tt::IO<pg::TimePointTz>::FormatterType, io::BufferFormatter<pg::TimePointTz>>::value);

static_assert(!tt::CanUseEnumAsStrongTypedef<std::string>::value, "not an enum");
...
static_assert(!tt::CanUseEnumAsStrongTypedef<MappedEnum>::value, "Mapped enum cannot be used as a strong typedef");

static_assert(
    tt::CanUseEnumAsStrongTypedef<sample::EnumStrongTypedef>::value,
...
UTEST_P(PostgreConnection, StringStrongTypedefArray) {
    ...
    StringTypedefVector str_vec{static_test::StringTypedef{"test"}, static_test::StringTypedef{}};
...
UTEST_P(PostgreConnection, IntStrongTypedefArray) {
    ...
    IntTypedefVector i_vec{static_test::IntTypedef{42}, static_test::IntTypedef{13}};
...
UTEST_P(PostgreConnection, IntEnumStrongTypedefArray) {
    ...
    EnumTypedefVector i_vec{sample::EnumStrongTypedef{42}, sample::EnumStrongTypedef{13}};
    UEXPECT_NO_THROW(res = GetConn()->Execute("select $1", i_vec));
