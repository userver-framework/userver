...
INSTANTIATE_UTEST_SUITE_P(
    StatsSettings,
    PostgreStats,
    ::testing::Combine(::testing::Values(kCachePreparedStatements), ::testing::Values(ConnectionMode::kDirect)),
    [](const testing::TestParamInfo<PostgreStats::ParamType>&) { return "Default"; }
);
...
UTEST_P(PostgreStats, TransactionExecuted) {
    ...
    UEXPECT_NO_THROW(GetConn()->Begin(pg::TransactionOptions{}, pg::detail::SteadyClock::now()));
    UEXPECT_NO_THROW(GetConn()->Execute("select 1")) << "select 1 successfully executed";
...
UTEST_P(PostgreStats, TransactionFailed) {
    ...
    const auto time_start = pg::detail::SteadyClock::now();
    UEXPECT_NO_THROW(GetConn()->Begin(pg::TransactionOptions{}, pg::detail::SteadyClock::now()));
    UEXPECT_THROW(GetConn()->Execute("select select select"), pg::Error) << "invalid query throws exception";
    ...
    EXPECT_GT(stats.trx_end_time, time_start);
    EXPECT_GT(stats.sum_query_duration.count(), 0);
    EXPECT_GE(stats.prepared_statements_current, old_stats.prepared_statements_current);
}
...
UTEST_P(PostgreStats, PortalInvalidQuery) {
    ...
    UEXPECT_NO_THROW(GetConn()->Begin(pg::TransactionOptions{}, pg::detail::SteadyClock::now()));
    UEXPECT_THROW(GetConn()->PortalBind("invalid query", "test", {}, {}), pg::SyntaxError);
    UEXPECT_NO_THROW(GetConn()->Rollback());
...
UTEST_P(PostgreStats, PortalDuplicateName) {
    ...
    UEXPECT_THROW(GetConn()->PortalBind("select 2", "test", {}, {}), pg::AccessRuleViolation);
    UEXPECT_NO_THROW(GetConn()->Rollback());
