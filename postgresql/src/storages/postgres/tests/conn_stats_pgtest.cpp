...
POSTGRE_CASE_TEST_P(PostgreStats, PortalExecuted) {
  const auto old_stats = conn->GetStatsAndReset();
...
POSTGRE_CASE_TEST_P(PostgreStats, PortalInvalidQuery) {
  const auto old_stats = conn->GetStatsAndReset();
...
POSTGRE_CASE_TEST_P(PostgreStats, PortalDuplicateName) {
  const auto old_stats = conn->GetStatsAndReset();
