...
void DumpMetric(USERVER_NAMESPACE::utils::statistics::Writer& writer, const InstanceStatisticsNonatomic& stats) {
    ...
    if (auto trx = writer["transactions"]) {
        ...
        // TODO remove legacy GAUGE values after dashboards are migrated.
        trx["committed"] = stats.transaction.commit_total.value;
        trx["committed"]["v2"] = stats.transaction.commit_total;
        trx["rolled-back"] = stats.transaction.rollback_total.value;
    ...
    if (auto query = writer["queries"]) {
        ...
        // TODO remove legacy GAUGE values after dashboards are migrated.
        query["executed"] = stats.transaction.execute_total.value;
    ...
    if (auto errors = writer["errors"]) {
        ...
        // TODO remove legacy GAUGE values after dashboards are migrated.
        errors.ValueWithLabels(stats.transaction.error_execute_total.value, {kPostgresqlError, "query-exec"});
        errors["v2"].ValueWithLabels(stats.transaction.error_execute_total, {kPostgresqlError, "query-exec"});

        errors.ValueWithLabels(stats.transaction.execute_timeout.value, {kPostgresqlError, "query-timeout"});
        errors["v2"].ValueWithLabels(stats.transaction.execute_timeout, {kPostgresqlError, "query-timeout"});

        errors.ValueWithLabels(
            stats.transaction.duplicate_prepared_statements.value,
        ...
        errors.ValueWithLabels(stats.connection.error_total.value, {kPostgresqlError, "connection"});
        errors["v2"].ValueWithLabels(stats.connection.error_total, {kPostgresqlError, "connection"});

        errors.ValueWithLabels(stats.pool_exhaust_errors.value, {kPostgresqlError, "pool"});
        errors["v2"].ValueWithLabels(stats.pool_exhaust_errors, {kPostgresqlError, "pool"});

        errors.ValueWithLabels(stats.queue_size_errors.value, {kPostgresqlError, "queue"});
        errors["v2"].ValueWithLabels(stats.queue_size_errors, {kPostgresqlError, "queue"});

        errors.ValueWithLabels(stats.connection.error_timeout.value, {kPostgresqlError, "connection-timeout"});
    ...
    if (!stats.per_statement_stats.empty()) {
        for (const auto& [stmt, stmt_stats] : stats.per_statement_stats) {
            writer["statement_timings"].ValueWithLabels(stmt_stats.timings, {"postgresql_query", stmt});
            writer["statement_executed"].ValueWithLabels(stmt_stats.executed, {"postgresql_query", stmt});
            writer["statement_errors"].ValueWithLabels(stmt_stats.errors, {"postgresql_query", stmt});
        }
    }
}
...
void DumpMetric(USERVER_NAMESPACE::utils::statistics::Writer& writer, const InstanceStatsDescriptor& value) {
    if (!value.host_port.empty()) {
        writer.ValueWithLabels(value.stats, {"postgresql_instance", value.host_port});
    }
}

void DumpMetric(USERVER_NAMESPACE::utils::statistics::Writer& writer, const ClusterStatistics& value) {
    constexpr std::string_view kPostgresqlClusterHostType = "postgresql_cluster_host_type";
    ...
    writer.ValueWithLabels(value.sync_slave, {kPostgresqlClusterHostType, "sync_slave"});
