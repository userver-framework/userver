...
void WriteDigit(std::string& res, std::uint16_t bin_dgt, bool truncate_leading_zeros) {
    ...
    p = buffer.data();
    auto* e = p + (truncate_leading_zeros ? (significant_digits ? significant_digits : 1) : kDigitWidth);
...
std::int16_t GetBinDigit(std::string_view str, int idx) {
    ...
    auto c = str[idx];
    if (!std::isdigit(c)) {
        throw InvalidInputFormat{"String representation for a numeric contains a non-digit"};
    }
    return c - '0';
}
...
void ConvertDecimalToBinary(std::string_view dec_digits, int left_padding, std::vector<std::int16_t>& target) {
    for (auto dec_pos = left_padding; dec_pos < static_cast<std::int32_t>(dec_digits.size()); dec_pos += kDigitWidth) {
...
void IntegralToBinary(std::int64_t integral_part, Smallint digits, std::vector<std::int16_t>& target) {
    // Left pad
...
void NumericData::ReadBuffer(FieldBuffer fb) {
    ...
    if (!(sign == kNumericPositive || sign == kNumericNegative || sign == kNumericNan)) {
...
std::string NumericData::ToString() const {
    ...
    if (dscale > 0) {
        res.push_back('.');
        for (auto dec_pos = 0; dec_pos < dscale; ++bin_digit_pos, dec_pos += kDigitWidth) {
            auto dig = (0 <= bin_digit_pos && bin_digit_pos < ndigits) ? digits[bin_digit_pos] : 0;
...
void NumericData::Parse(USERVER_NAMESPACE::utils::zstring_view str) {
    ...
    if (str.find_first_of("eE", start_pos) != std::string::npos) {
        throw InvalidInputFormat{"Scientific string representation for numerics is not supported"};
    }
    ...
    if (dec_point_pos != std::string::npos) {
        ...
        if (frac_part_begin == frac_part_end) {
            ...
            throw InvalidInputFormat{"Invalid numeric string format: no symbols after decimal point"};
        }
        ...
        fractional_part = {str.data() + frac_part_begin, frac_part_end - frac_part_begin};
        // Cut trailing zeros
    ...
    digits.reserve((integral_part.size() + fractional_part.size()) / kDigitWidth + 1);
    if (dec_weight >= 0) {
...
IntegralRepresentation NumericData::ToInt64() const {
    ...
    if (sign == kNumericNan) {
        throw ValueIsNaN{"PostgreSQL buffer for decimal/numeric contains a NaN value"};
    }
    ...
    if (dscale > 0) {
        for (auto decimal_pos = 0; decimal_pos < dscale; ++bin_dig_pos, decimal_pos += kDigitWidth) {
            auto dig = (0 <= bin_dig_pos && bin_dig_pos < ndigits) ? digits[bin_dig_pos] : 0;
...
void NumericData::FromInt64(IntegralRepresentation rep) {
    ...
    // digit width
    while (fractional_part && fractional_part % 10 == 0 && fractional_digits % kDigitWidth != 0) {
        fractional_part /= 10;
        --fractional_digits;
    }
