...
static_assert(
    pg_cache::detail::ClusterHostType<PostgresExamplePolicy>() == storages::postgres::ClusterHostType::kSlave
);
static_assert(
    pg_cache::detail::ClusterHostType<PostgresExamplePolicy2>() == storages::postgres::ClusterHostType::kSlave
);
...
struct MyStructureCompoundKey {
    ...
    bool operator==(const MyStructureCompoundKey& other) const { return id == other.id && bar == other.bar; }
};
...
struct PostgresExamplePolicy5 {
    ...
    [[maybe_unused]] static constexpr auto kKeyMember = [](const MyStructure& my_structure) {
    ...
    using CacheContainer = std::unordered_map<MyStructureCompoundKey, MyStructure, MyStructureCompoundKeyHash>;
};
/*! [Pg Cache Policy Compound Primary Key Example] */
