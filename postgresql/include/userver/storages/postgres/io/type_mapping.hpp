...
struct RegisterUserTypeParser {
    static RegisterUserTypeParser Register(const DBTypeName&, std::string cpp_name);
...
struct CppToSystemPgImpl {
    ...
    static_assert(array_oid != PredefinedOids::kInvalid, "Array type oid is invalid");
...
constexpr bool IsTypeMappedToSystem() {
    return traits::kIsMappedToPg<T> &&
           std::is_same<typename CppToPg<T>::Mapping, CppToSystemPg<typename CppToPg<T>::Type>>::value;
}
