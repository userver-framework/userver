...
auto JsonCompat(const T& v) {
    if constexpr (std::is_same_v<T, std::int32_t>) return std::int64_t{v};
    else if constexpr (std::is_same_v<T, float>)   return double{v};
    else                                           return v;
}
...
Builder JsonArrayFrom(const Range& range, Fn&& fn) {
    auto arr = JsonArray();
    for (const auto& item : range) arr.PushBack(fn(item));
    return arr;
}
...
std::vector<T> CollectTyped(const Container& items) {
    ...
    for (const auto& item : items)
        if (item.template Is<T>()) out.push_back(item.template Get<T>());
    return out;
}

template <typename T>
std::vector<T> CollectList(const Row& row, const std::string& name) {
...
template <typename T>
std::vector<T> CollectSet(const Row& row, const std::string& name) {
...
std::vector<std::pair<K, V>> CollectMap(const Row& row,
...
void BadRequest(std::string message) {
    throw server::handlers::ClientError(
        server::handlers::ExternalBody{std::move(message)});
}
...
Value JsonToBasicValue(std::string_view name,
    ...
    if (name == "bln") return Value{json.As<bool>()};
    if (name == "i32") return Value{json.As<std::int32_t>()};
    if (name == "i64") return Value{json.As<std::int64_t>()};
    if (name == "flt") return Value{json.As<float>()};
    if (name == "dbl") return Value{json.As<double>()};
...
formats::json::ValueBuilder ValueToJson(const Value& v) {
    if (v.IsNull()) return {};
    return JsonOneOf<std::string, bool,
...
formats::json::ValueBuilder RowToJson(const Row& row) {
    ...
    for (const auto& [name, value] : row)
        if (!value.IsNull()) obj[name] = ValueToJson(value);
    return obj;
}
...
void DecodeRow(const Row& row, Event& out) {
    ...
    if (auto ts = row.TryGet<Timestamp>("created_at")) out.created_at = *ts;
    if (auto ip = row.TryGet<Inet>("source_ip")) out.source_ip = ip->text;
}
...
formats::json::ValueBuilder EventToJson(const Event& event) {
    ...
    if (!event.source_ip.empty()) obj["source_ip"] = event.source_ip;
    return obj;
}
