...
constexpr std::string_view kOptionalFields[] = {"bln", "i32", "i64", "flt",
                                                "dbl"};
...
public:
    ...
    KeyValueHandler(const components::ComponentConfig& config,
                    const components::ComponentContext& context)
        : HttpHandlerBase(config, context),
          session_(context.FindComponent<components::Scylla>(kComponentName)
                       .GetSession()) {}
    ...
    std::string HandleRequest(server::http::HttpRequest& request,
        ...
        switch (request.GetMethod()) {
            ...
            default:
                BadRequest(fmt::format("Unsupported method {}",
                                       request.GetMethod()));
        }
    }
    ...
    std::string Patch(const server::http::HttpRequest& request) const {
        ...
        for (auto name : kOptionalFields)
            if (body.HasMember(name))
                op.Set(std::string{name},
                       JsonToBasicValue(name, body[name]));
        op.WhereString("key", RequireArg(request, "key"));
        if (body.HasMember("ttl"))
            op.UsingTtl(body["ttl"].As<std::int32_t>());
        session_->GetTable(std::string{kBasicTable}).Execute(op);
...
public:
    ...
    ListHandler(const components::ComponentConfig& config,
                const components::ComponentContext& context)
        : HttpHandlerBase(config, context),
          session_(context.FindComponent<components::Scylla>(kComponentName)
                       .GetSession()) {}
...
public:
    ...
    CountHandler(const components::ComponentConfig& config,
                 const components::ComponentContext& context)
        : HttpHandlerBase(config, context),
          session_(context.FindComponent<components::Scylla>(kComponentName)
                       .GetSession()) {}
    ...
    std::string HandleRequest(server::http::HttpRequest& request,
        ...
        return formats::json::ToString(
            formats::json::MakeObject("count", count));
    }
...
public:
    ...
    BulkInsertHandler(const components::ComponentConfig& config,
                      const components::ComponentContext& context)
        : HttpHandlerBase(config, context),
          session_(context.FindComponent<components::Scylla>(kComponentName)
                       .GetSession()) {}
    ...
    std::string HandleRequest(server::http::HttpRequest& request,
        ...
        return formats::json::ToString(formats::json::MakeObject(
            "inserted", static_cast<std::int64_t>(body.GetSize())));
    }
...
public:
    ...
    PaginatedHandler(const components::ComponentConfig& config,
                     const components::ComponentContext& context)
        : HttpHandlerBase(config, context),
          session_(context.FindComponent<components::Scylla>(kComponentName)
                       .GetSession()) {}
    ...
    std::string HandleRequest(server::http::HttpRequest& request,
        ...
        auto result = session_->GetTable(std::string{kBasicTable})
                          .ExecutePaged(op, std::move(cursor));
...
public:
    ...
    std::string HandleRequest(server::http::HttpRequest& request,
        ...
        op.IfNotExists();
        const auto result =
            session_->GetTable(std::string{kBasicTable}).ExecuteLwt(op);
        auto response = JsonObject();
...
class CompareAndSetHandler final
    : public server::handlers::HttpHandlerBase {
public:
    static constexpr std::string_view kName = "handler-kv-cas";

    CompareAndSetHandler(const components::ComponentConfig& config,
                         const components::ComponentContext& context)
        : HttpHandlerBase(config, context),
          session_(context.FindComponent<components::Scylla>(kComponentName)
                       .GetSession()) {}
...
class DeleteIfExistsHandler final
    : public server::handlers::HttpHandlerBase {
public:
    static constexpr std::string_view kName = "handler-kv-delete-if-exists";

    DeleteIfExistsHandler(const components::ComponentConfig& config,
                          const components::ComponentContext& context)
        : HttpHandlerBase(config, context),
          session_(context.FindComponent<components::Scylla>(kComponentName)
                       .GetSession()) {}
...
public:
    ...
    TruncateHandler(const components::ComponentConfig& config,
                    const components::ComponentContext& context)
        : HttpHandlerBase(config, context),
          session_(context.FindComponent<components::Scylla>(kComponentName)
                       .GetSession()) {}
    ...
    std::string HandleRequest(server::http::HttpRequest& request,
        ...
        request.GetHttpResponse().SetContentType(
            http::content_type::kApplicationJson);
        session_->GetTable(std::string{kBasicTable})
            .Execute(operations::Truncate{});
        return R"({"status":"ok"})";
    }
...
public:
    ...
    SchemaInitHandler(const components::ComponentConfig& config,
                      const components::ComponentContext& context)
        : HttpHandlerBase(config, context),
          session_(context.FindComponent<components::Scylla>(kComponentName)
                       .GetSession()) {}
...
public:
    ...
    RawQueryHandler(const components::ComponentConfig& config,
                    const components::ComponentContext& context)
        : HttpHandlerBase(config, context),
          session_(context.FindComponent<components::Scylla>(kComponentName)
                       .GetSession()) {}
...
public:
    ...
    EventHandler(const components::ComponentConfig& config,
                 const components::ComponentContext& context)
        : HttpHandlerBase(config, context),
          session_(context.FindComponent<components::Scylla>(kComponentName)
                       .GetSession()) {}
...
private:
    ...
    std::string Create(const server::http::HttpRequest& request) const {
        ...
        if (body.HasMember("source_ip"))
            op.BindInet("source_ip",
                        Inet{body["source_ip"].As<std::string>()});

        if (body.HasMember("tags") && body["tags"].IsArray()) {
            Set tags;
            for (std::size_t i = 0; i < body["tags"].GetSize(); ++i)
                tags.items.emplace_back(
                    body["tags"][i].As<std::string>());
        ...
        if (body.HasMember("scores") && body["scores"].IsArray()) {
            ...
            for (std::size_t i = 0; i < body["scores"].GetSize(); ++i)
                scores.items.emplace_back(
                    body["scores"][i].As<std::int32_t>());
        ...
        response["created_at"] = std::chrono::duration_cast<
...
public:
    ...
    EventListHandler(const components::ComponentConfig& config,
                     const components::ComponentContext& context)
        : HttpHandlerBase(config, context),
          session_(context.FindComponent<components::Scylla>(kComponentName)
                       .GetSession()) {}
    ...
    std::string HandleRequest(server::http::HttpRequest& request,
        ...
        auto cursor =
            session_->NewCursor("SELECT * FROM events", {}, page_size);
