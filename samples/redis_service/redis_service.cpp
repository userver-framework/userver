...
std::string EvalSha::HandleRequestThrow(const server::http::HttpRequest& request, server::request::RequestContext&)
    ...
    if (command.empty()) {
        throw server::handlers::ClientError(server::handlers::ExternalBody{"No 'command' query argument"});
    }
...
std::string EvalSha::EvalShaRequest(const server::http::HttpRequest& request) const {
    ...
    if (script_hash.empty()) {
        throw server::handlers::ClientError(server::handlers::ExternalBody{"No 'hash' query argument"});
    }
    const auto& key = request.GetArg("key");
    if (key.empty()) {
        throw server::handlers::ClientError(server::handlers::ExternalBody{"No 'key' query argument"});
    ...
    auto redis_request = redis_client_->EvalSha<std::string>(script_hash, {key}, {}, redis_cc_);
    ...
    throw server::handlers::InternalServerError(server::handlers::ExternalBody{"Some internal redis error"});
}

std::string EvalSha::ScriptLoad(const server::http::HttpRequest& request) const {
    const auto& script = request.GetArg("script");
    if (script.empty()) {
        throw server::handlers::ClientError(server::handlers::ExternalBody{"No 'script' query argument"});
    }
    const auto& key = request.GetArg("key");
    if (key.empty()) {
        throw server::handlers::ClientError(server::handlers::ExternalBody{"No 'key' query argument"});
