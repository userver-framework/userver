...
def genListOfRandStr(amount_of_keys, lenght=0, _randint=np.random.randint):
    ...
    if(lenght == 0):
        while len(keys) < amount_of_keys:
            keys |= {''.join([pickchar() for _ in range(_randint(12, 20))])
                     for _ in range(amount_of_keys - len(keys))}
    else:
        while len(keys) < amount_of_keys:
            keys |= {''.join([pickchar() for _ in range(_randint(lenght-2, lenght))])
                     for _ in range(amount_of_keys - len(keys))}
...
def fillUsers(con):
    ...
    usernames = USERS_DEFAULT + \
        genUsername(max(0, COUNT_OF_USERS - lenUsersDefault))
    ...
    for i in range(COUNT_OF_USERS):
        ...
        randPassHash = "".join(random.choices(
            list(string.ascii_letters), k=255))
