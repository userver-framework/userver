...
std::vector<int> GetProbation(SLRU& slru) {
  ...
  slru.VisitProbation([&actual] (int key, int) {
...
std::vector<int> GetProtected(SLRU& slru) {
  std::vector<int> actual;
  slru.VisitProtected([&actual] (int key, int) {
    actual.push_back(key);
  });
  std::sort(actual.begin(), actual.end());
  return actual;
...
TEST(SLRU, Put) {
  ...
  for (int i = 12; i < 20; i++)
    EXPECT_TRUE(slru.Put(i, 0));
  EXPECT_EQ(GetProbation(slru), (std::vector<int> {12, 13, 14, 15, 16, 17, 18, 19}));
  EXPECT_EQ(GetProtected(slru), (std::vector<int> {5, 8}));
}
