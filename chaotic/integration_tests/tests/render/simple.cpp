...
TEST(Simple, IntegerMinimum) {
  ...
  UEXPECT_THROW_MSG(
      json.As<ns::SimpleObject>(), chaotic::Error,
...
TEST(Simple, IntegerMaximum) {
  ...
  UEXPECT_THROW_MSG(json.As<ns::SimpleObject>(), chaotic::Error,
...
TEST(Simple, ObjectRequired) {
  auto json = formats::json::MakeObject();
  UEXPECT_THROW_MSG(json.As<ns::SimpleObject>(),
...
TEST(Simple, IntegerFormat) {
  static_assert(
      std::is_same_v<decltype(ns::SimpleObject::integer), std::optional<int>>);
...
TEST(Simple, ObjectWithRefType) {
  ...
  UEXPECT_THROW_MSG(
      json.As<ns::ObjectWithRef>(), chaotic::Error,
...
TEST(Simple, ObjectTypes) {
  ...
  EXPECT_EQ(obj, (ns::ObjectTypes{true,
                                  1,
                                  1.1,
                                  "foo",
                                  ns::ObjectTypes::Object{},
                                  {1},
                                  {},
                                  ns::ObjectTypes::String_Enum::kX1}));
}
...
TEST(Simple, ObjectWithAdditionalPropertiesInt) {
  ...
  EXPECT_EQ(obj.one, 1);
  EXPECT_EQ(obj.extra,
            (std::unordered_map<std::string, int>{{"two", 2}, {"three", 3}}));
...
TEST(Simple, ObjectWithAdditionalPropertiesTrue) {
  auto json = formats::json::MakeObject("one", 1, "two", 2, "three", 3,
                                        "object", formats::json::MakeObject());
  auto obj = json.As<ns::ObjectWithAdditionalPropertiesTrue>();

  EXPECT_EQ(obj.one, 1);
  EXPECT_EQ(obj.extra, formats::json::MakeObject("two", 2, "three", 3, "object",
...
TEST(Simple, ObjectExtraMemberFalse) {
  ...
  auto json_back = formats::json::ValueBuilder{obj}.ExtractValue();
  EXPECT_EQ(json_back, formats::json::MakeObject("one", 1))
      << ToString(json_back);
}

TEST(Simple, ObjectWithAdditionalPropertiesFalseStrict) {
  auto json = formats::json::MakeObject("foo", 1, "bar", 2);
  UEXPECT_THROW_MSG(json.As<ns::ObjectWithAdditionalPropertiesFalseStrict>(),
                    std::runtime_error, "Unknown property 'bar'");
}
...
TEST(Simple, StringEnum) {
  ...
  UEXPECT_THROW_MSG(
      json2["one"].As<ns::StringEnum>(), chaotic::Error,
  ...
  EXPECT_EQ(FromString("foo", formats::parse::To<ns::StringEnum>{}),
            ns::StringEnum::kFoo);
  UEXPECT_THROW_MSG(FromString("zoo", formats::parse::To<ns::StringEnum>{}),
                    std::runtime_error,
                    "Invalid enum value (zoo) for type ns::StringEnum");

  EXPECT_EQ(Parse("foo", formats::parse::To<ns::StringEnum>{}),
            ns::StringEnum::kFoo);
  UEXPECT_THROW_MSG(Parse("zoo", formats::parse::To<ns::StringEnum>{}),
                    std::runtime_error,
                    "Invalid enum value (zoo) for type ns::StringEnum");

  EXPECT_EQ(std::size(ns::kStringEnumValues), 3);

  ns::StringEnum values[] = {ns::StringEnum::kFoo, ns::StringEnum::kBar,
                             ns::StringEnum::kSomeThing};
  std::size_t index = 0;
...
TEST(Simple, OneOfWithDiscriminator) {
  auto json = formats::json::MakeObject(
      "oneof", formats::json::MakeObject("type", "ObjectFoo", "foo", 42));
...
TEST(Simple, Indirect) {
  auto json = formats::json::MakeObject(
      "data", "smth", "left", formats::json::MakeObject("data", "left"),
      "right",
      formats::json::MakeObject(
          "data", "right", "left",
          formats::json::MakeObject("data", "rightleft")));
  ...
  EXPECT_EQ(obj.right,
            (ns::TreeNode{"right",
                          ns::TreeNode{"rightleft", std::nullopt, std::nullopt},
                          std::nullopt}));
}
...
TEST(Simple, SubSubObjectSmoke) {
  [[maybe_unused]] ns::Objectx::Objectx_::Objectx__ x;
}

TEST(Simple, ExtraType) {
  static_assert(
      std::is_same_v<decltype(std::declval<ns::ObjectWithExtraType>().extra),
...
TEST(Simple, DateTime) {
  ...
  utils::datetime::TimePointTz tp{
      utils::datetime::Stringtime("2020-10-01T00:00:56Z"),
      std::chrono::seconds(12 * 60 * 60 + 34 * 60)};
  EXPECT_EQ(obj.updated_at, tp);

  auto str =
      Serialize(obj,
                formats::serialize::To<formats::json::Value>())["updated_at"]
          .As<std::string>();
  EXPECT_EQ(str, date);
}
...
TEST(Simple, DateTimeExtra) {
  ...
  auto str =
      Serialize(
          obj,
          formats::serialize::To<formats::json::Value>())["updated_at_extra"]
          .As<std::string>();
  EXPECT_EQ(str, date);
}
...
TEST(Simple, DateTimeIsoBasic) {
  ...
  auto str =
      Serialize(obj,
                formats::serialize::To<formats::json::Value>())["deleted_at"]
          .As<std::string>();
  EXPECT_EQ(str, date);
}
...
TEST(Simple, Uuid) {
  ...
  auto str =
      Serialize(obj, formats::serialize::To<formats::json::Value>())["uuid"]
          .As<std::string>();
  EXPECT_EQ(str, uuid);
}
