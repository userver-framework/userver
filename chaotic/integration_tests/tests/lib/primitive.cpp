...
TEST(Primitive, NoValidatorSerializer) {
  using Int = chaotic::Primitive<std::int32_t>;
  EXPECT_EQ(
      formats::json::ValueBuilder{Int{kJson["foo"].As<Int>()}}.ExtractValue(),
      kJson["foo"]);
}
...
MyInt Convert(const std::int32_t& i, chaotic::convert::To<MyInt>) {
  return {i};
}

std::int32_t Convert(const MyInt& i, chaotic::convert::To<std::int32_t>) {
  return i.value;
}
...
TEST(Primitive, IntMinMax) {
  using Int = chaotic::Primitive<std::int32_t, chaotic::Minimum<kOne>,
                                 chaotic::Maximum<kFive>>;

  int x = kJson["foo"].As<Int>();
  EXPECT_EQ(x, 1);

  UEXPECT_THROW_MSG(kJson["bar"].As<Int>(), chaotic::Error,
                    "Error at path 'bar': Invalid value, minimum=1, given=0");
  UEXPECT_THROW_MSG(kJson["zoo"].As<Int>(), chaotic::Error,
                    "Error at path 'zoo': Invalid value, maximum=5, given=6");
}

TEST(Primitive, UserTypeMinMax) {
  using Int =
      chaotic::WithType<chaotic::Primitive<std::int32_t, chaotic::Minimum<kOne>,
                                           chaotic::Maximum<kFive>>,
                        MyInt>;
...
TEST(Primitive, StringMinMaxLength) {
  auto kLocalJson =
      formats::json::MakeObject("1", "1", "2", "12", "6", "123456");

  using Str = chaotic::Primitive<std::string, chaotic::MinLength<2>,
                                 chaotic::MaxLength<5>>;
  ...
  UEXPECT_THROW_MSG(
      kLocalJson["1"].As<Str>(), chaotic::Error,
      "Error at path '1': Too short string, minimum length=2, given=1");
  UEXPECT_THROW_MSG(
      kLocalJson["6"].As<Str>(), chaotic::Error,
...
TEST(Primitive, StringPattern) {
  ...
  UEXPECT_THROW_MSG(kLocalJson["2"].As<Str>(), chaotic::Error,
                    "doesn't match regex");
}
