...
TEST(Array, OfIntSerializer) {
  ...
  auto result =
      formats::json::ValueBuilder{Arr{kJson.As<Arr>()}}.ExtractValue();
...
TEST(Array, OfIntWithValidatorsSerializer) {
  ...
  using Arr = chaotic::Array<std::string, std::vector<std::string>,
                             chaotic::MinItems<2>>;
  EXPECT_EQ(formats::json::ValueBuilder{Arr{kJson.As<Arr>()}}.ExtractValue(),
            kJson);
}
...
TEST(Array, OfArrayOfString) {
  ...
  using Arr =
      chaotic::Array<chaotic::Array<std::string, std::vector<std::string>>,
