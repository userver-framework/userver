...
std::vector<T> Convert(
    const CustomArray<T>& value,
    USERVER_NAMESPACE::chaotic::convert::To<std::vector<T>>) {
  return value.s;
}

template <typename T>
std::set<T> Convert(const CustomArray<T>& value,
                    USERVER_NAMESPACE::chaotic::convert::To<std::set<T>>) {
