...
template <typename BuilderFunc>
OneOfSettings(const char*, utils::TrivialSet<BuilderFunc>)
    -> OneOfSettings<BuilderFunc>;

template <const auto* Settings, typename... T>
struct OneOfWithDiscriminator {
  const std::variant<formats::common::ParseType<formats::json::Value, T>...>&
      value;
};

template <const auto* Settings, typename... T, typename Value>
std::variant<formats::common::ParseType<Value, T>...> Parse(
    Value value, formats::parse::To<OneOfWithDiscriminator<Settings, T...>>) {
  ...
  using Result =
      std::variant<formats::common::ParseType<formats::json::Value, T>...>;

  Result result;
  utils::WithConstexprIndex<sizeof...(T)>(
      index.value(), [&](auto index_constant) {
        constexpr auto kIndex = decltype(index_constant)::value;
        result.template emplace<kIndex>(
            value.template As<std::variant_alternative_t<kIndex, Result>>());
      });
  return result;
}

template <const auto* Settings, typename... T, typename Value>
Value Serialize(const OneOfWithDiscriminator<Settings, T...>& var,
