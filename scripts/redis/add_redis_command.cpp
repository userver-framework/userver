...
const string client_filename = add_path(userver_path, "redis/include/storages/redis/client.hpp");
...
string request_tmpl_param; // 'cpp_type_to_parse[, reply_cpp_type=cpp_type_to_parse]'
bool dry_run = false;
...
void write_file(const string &filename, const std::string &data)
{
	if (dry_run) {
		...
		diff = diff.substr(0, 4) + diff.substr(start_fn, ppp + 4 - start_fn) + dir + diff.substr(ppp + 8);
		cout << diff << endl;
		return;
	}
	ofstream f(filename);
...
void get_commands(
	...
	for (;;) {
		...
		command_names.emplace_back(
			file.substr(cmd_start, brpos - cmd_start)
		);
	...
	for (size_t i = 1; i < command_names.size(); i++) 
	{
		if (command_names[i - 1] > command_names[i]) {
			cerr << command_names[i - 1] << " > " << command_names[i] << " i=" << i << endl;
		}
...
//	cerr << client_file << endl;
	...
	size_t idx = upper_bound(command_names.begin(), command_names.end(), command_cc) - command_names.begin();
...
void process_client_file()
{
	...
	string decl_command = "  virtual Request" + command_cc + ' ' + command_cc + '(';
	for (const auto &param : params) {
...
{
	return type.find("const") == string::npos && (type.find("std::string") != string::npos || type.find("std::vector") != string::npos);
}
...
void add_dummy_request(const string &param_name, string &decl_command)
{
	...
	decl_command += "        std::make_shared<::redis::Reply>(\"" + command + "\", ";
	if (request_tmpl_param.find("std::vector") == 0) decl_command += "::redis::ReplyData::Array{}";
		...
		cerr << "not implemented for request_tmpl_param=" << request_tmpl_param << endl;
		abort();
	}
	decl_command += "));\n";
}
...
//	cerr << request_file << endl;
	...
	for (size_t pos = 0;;) {
		...
		reqtype_names.emplace_back(
			request_file.substr(
				...
				request_file.find(' ', reqtype_names_pos.back()) - reqtype_names_pos.back()
			)
		);
	}
	...
	size_t idx = lower_bound(reqtype_names.begin(), reqtype_names.end(), command_cc) - reqtype_names.begin();
//	if (idx < reqtype_names.size()) {
//		cerr << "reqtype_names[idx] = \"" << reqtype_names[idx] << '"' << endl;
//	}
	if (idx >= reqtype_names.size() || reqtype_names[idx] != command_cc) {
		string decl_reqtype = "using Request" + command_cc +
			" = Request<" + request_tmpl_param + ">;\n";
...
{
	for (const auto &param : params) {
		cerr << "type='" << param.type << '\'' << "  name='" << param.name << '\'' << endl;
	}
}
...
vector<param_t> parse_params(const char *begin, const char *end)
{
	...
	auto make_name = [&] {
		...
		while (sp_pos + 1 < type.size() && !isalpha(type[sp_pos + 1]) && type[sp_pos + 1] != '_') ++sp_pos;
...
[[noreturn]] void usage(const char *cmd)
{
	...
	fn = "./" + fn;
	cerr << "usage: " << fn << " command request_template_param command_arguments [--dry-run]" << endl;
	cerr << "example: " << fn << " zrange 'std::vector<std::string>' 'std::string key, int64_t start, int64_t stop' --dry-run" << endl;
	cerr << "example: " << fn << " hmset 'StatusOk, void' 'std::string key, std::vector<std::pair<std::string, std::string>> field_values'" << endl;
	exit(1);
}
...
int main(int argc, char **argv)
{
	...
	int call_res = system(("redis-cli -h taximeter-base-redis-myt-01.taxi.dev.yandex.net -p 6379 -a taximeter-dev command info " + command + " | grep 'write' >" + tmp_filename).c_str());
	cerr << "call_res=" << call_res << endl;
	string write_str;
