...
FieldsSectionBegin = "Field\tType\tDescription\n"
ObjectSectionBegin = "--------------------------------------------------------------------------------\n"
...
class Field:
  def __init__(self, name, type_, description):
    ...
    self.is_optional = self.description.find("Optional.") != -1
...
def parse_field(line):
  lines = line.split("\t")
  assert(len(lines) == 3)
  return Field(lines[0].strip(), lines[1].strip(), lines[2].strip())


def parse_fields(lines):
  assert(lines[0] == FieldsSectionBegin)
  lines = lines[1:]
