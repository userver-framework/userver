...
For basic information on querying data see @ref scripts/docs/en/userver/pg/run_queries.md and
...
In a PostgreSQL database the following kinds of user types are available:
...
@snippet postgresql/src/storages/postgres/tests/user_types_pgtest.cpp User domain type postgres

The following code allows retrieval of that type:

@snippet postgresql/src/storages/postgres/tests/user_types_pgtest.cpp User domain type cpp usage
...
composite types. The driver doesn't check the data type oids, it's the user's
responsibility to provide structures with compatible data members.

For a PostgreSQL database type defined as:

@snippet postgresql/src/storages/postgres/tests/user_types_pgtest.cpp User composite type postgres

The following C++ type should be written:

@snippet postgresql/src/storages/postgres/tests/user_types_pgtest.cpp User composite type cpp
...
@snippet postgresql/src/storages/postgres/tests/user_types_pgtest.cpp User composite type cpp usage
...
@snippet postgresql/src/storages/postgres/tests/enums_pgtest.cpp User enum type postgres

and a C++ enumeration declared as follows:

@snippet postgresql/src/storages/postgres/tests/enums_pgtest.cpp User enum type cpp
...
@snippet postgresql/src/storages/postgres/tests/enums_pgtest.cpp User enum type cpp usage
...
@snippet postgresql/src/storages/postgres/tests/enums_pgtest.cpp User enum type cpp2
...
A C++ enumeration can be mapped to PostgreSQL text type by providing a
`ToString()` and `Parse()` functions for the enumeration values and declaring specialization of
storages::postgres::io::CppToSystemPg template.

Example:

@snippet postgresql/src/storages/postgres/tests/enums_pgtest.cpp Autogenerated user enum type

@note Enums @ref scripts/docs/en/userver/chaotic.md "generated by codegen" can be easily mapped to postgres text type by only providing the CppToSystemPg specialization, as they already have ToString() and Parse() functions.
...
@snippet postgresql/src/storages/postgres/tests/user_types_pgtest.cpp User domainrange type postgres

and declare a mapping from C++ range to this type just as for any other user
type:

@snippet postgresql/src/storages/postgres/tests/user_types_pgtest.cpp User domainrange type cpp

Please note that `my_type` must be comparable both in Postgres and in C++.

The type could be used in code in the following way:

@snippet postgresql/src/storages/postgres/tests/user_types_pgtest.cpp User domainrange type usage
...
@snippet postgresql/src/storages/postgres/tests/user_types_pgtest.cpp User timerange type postgres

Mapping of the above type to a strong typedef with name TimeOfDay could be done
in the following way:

@snippet postgresql/src/storages/postgres/tests/user_types_pgtest.cpp User timerange type cpp

The actual usage:

@snippet postgresql/src/storages/postgres/tests/user_types_pgtest.cpp User timerange type usage
