...
* HTTP PUT by URL '/v1/key-value' with query parameters 'key' and 'value' stores the provided key and value or return an existing value for the key
...
@snippet samples/postgres_service.cpp  Postgres service sample - PutValue
...
```
bash
$ curl -X PUT 'localhost:8085/v1/key-value?key=hello&value=world' -i
...
world
$ curl -X PUT 'localhost:8085/v1/key-value?key=hello&value=nope' -i
