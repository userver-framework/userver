...
userver::formats::json::Value Handler::HandleRequestJsonThrow(
  ...
  const auto res_unfollowing = pg_cluster_->Execute(
      userver::storages::postgres::ClusterHostType::kSlave, sql::KUnFollowingUser.data(), username_id, user_id);
  
  const auto profile = res_unfollowing.AsSingleRow<real_medium::models::Profile>(userver::storages::postgres::kRowTag);
