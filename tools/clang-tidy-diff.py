...
def is_skip_filename(filename: str):
    if (not filename.endswith('.pb.cc')
          and not filename.endswith('.pb.cpp')
          and not 'benchmark.cpp' in filename
          and not '/boost_stacktrace/' in filename
          and not '/pq-extra/' in filename
          and not '/tools/' in filename
          and not '/uboost_coro/' in filename
          and not 'third_party/clickhouse-cpp/' in filename
          and not 'third_party/gbench/' in filename
          and not 'third_party/gtest/' in filename
...
def main():
  ...
  for line in sys.stdin:
    ...
    if match:
      ...
      skip_filename = is_skip_filename(filename)
    if filename is None:
      continue
    ...
    if match and not skip_filename:
