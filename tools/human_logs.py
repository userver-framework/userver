...
class HumanLogs:
    ...
    def __init__(self, highlights, ignores, filename, no_file_stores, verbosity):
        self.i = 0
    ...
    def __output_line(self, line):
        ...
        fmt = "{i:<3} {level_color}{level:<5} {text_color}{text} {module}{GRAY}{remains}{DEFAULT}\n"

        level_indx = HumanLogs.LOG_LEVELS.index(level)

        remains = ""
        if level_indx > self.verbosity_indx:
            remains = (x for x in values if (x not in self.ignores and
                       values[x] != ""))
...
def main():
    parser = argparse.ArgumentParser(description='Represents logs nicely',
                                     epilog=__EXAMPLES,
    ...
    parser.add_argument('-ign', '--ignores',
                        nargs='+',
    ...
    parser.add_argument('-x', '--no-file-stores',
                        action='store_true',
                        help="Do not store full logs into a separate file")
    parser.add_argument('-v', '--verbosity',
                        choices=HumanLogs.LOG_LEVELS,
                        default="WARNING",
                        help="Print unimportant fields if log level is greater than this value")
