...
#
# This target generates all add_custom_command files that are registered in it,
# without actually compiling any C++ code.
#
# This allows to quickly generate sources for IDE indexing and other tooling.
# If a generator is compiled from sources, e.g. Protobuf, then it will still have to be built.
#
# userver-codegen is similar to CMake's 3.31+ "codegen" target,
# except that userver-codegen actually works.
...
function(_userver_codegen_next_target_index RESULT_VAR)
  ...
  set("${RESULT_VAR}" "${userver_codegen_target_index}" PARENT_SCOPE)
endfunction()

function(_userver_codegen_make_directory_target)
  get_property(userver_codegen_files DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}" PROPERTY userver_codegen_files)
...
function(_userver_codegen_register_files FILES_LIST)
  ...
  get_property(has_userver_codegen_files DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}" PROPERTY userver_codegen_files SET)
  if(NOT has_userver_codegen_files)
    ...
    cmake_language(
        ...
        CALL _userver_codegen_make_directory_target
    )

    get_property(has_userver_codegen_targets GLOBAL PROPERTY userver_codegen_targets SET)
    if(NOT has_userver_codegen_targets)
      # On first codegen invocation, schedule userver-codegen target creation.
      cmake_language(
          DEFER
          DIRECTORY "${CMAKE_SOURCE_DIR}"
          ID userver_codegen_make_main_target
          CALL _userver_codegen_make_main_target
      )
    endif()
  endif()

  set_property(DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}" APPEND PROPERTY userver_codegen_files ${FILES_LIST})
endfunction()
