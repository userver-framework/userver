...
function(_userver_get_sanitize_options SANITIZER_LIST_VAR COMPILE_FLAGS_VAR LINK_FLAGS_VAR)
    ...
    if(USERVER_SANITIZE AND CMAKE_SYSTEM_NAME MATCHES "Darwin")
        # Boost.Context needs ucontext + BOOST_USE_ASAN for correct sanitizer stack
        # tracking, but ucontext is broken on modern macOS (getcontext crashes on
        # arm64; see https://github.com/userver-framework/userver/issues/564).
        message(WARNING "Sanitizers are not supported on macOS with userver "
                        "coroutines (Boost.Context/ucontext incompatibility). Disabling. "
                        "Build without -DUSERVER_SANITIZE=... on macOS."
        )
        ...
        foreach(sanitizer IN LISTS sanitizer_list)
            ...
            elseif(sanitizer STREQUAL "addr")
                ...
                if(NOT CMAKE_CXX_COMPILER_ID MATCHES "Clang")
                    # gcc links with ASAN dynamically by default, and that leads to all sorts of problems when we
                    # intercept dl_iterate_phdr, which ASAN uses in initialization before main.
            ...
            elseif(sanitizer STREQUAL "mem")
                ...
                message(FATAL_ERROR "-DUSERVER_SANITIZE has invalid value(s) (${SANITIZE_PENDING}), "
...
function(_userver_make_sanitize_target)
    ...
    target_compile_options(userver-internal-sanitize-options INTERFACE ${sanitize_cxx_flags})
    target_link_options(userver-internal-sanitize-options INTERFACE ${sanitize_link_flags})
endfunction()
...
function(_userver_make_sanitize_blacklist)
    ...
    _userver_get_sanitize_blacklist_file(compile_flags_bl link_flags_bl)
    target_compile_options(userver::internal-sanitize-options INTERFACE ${compile_flags_bl})
    target_link_options(userver::internal-sanitize-options INTERFACE ${link_flags_bl})
endfunction()
