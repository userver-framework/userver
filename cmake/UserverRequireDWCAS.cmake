...
function(userver_target_require_dwcas target visibility)
    ...
    # Prefer boost::atomic to std::atomic for two reasons. Reason 1. Clang's std::atomic already emits DWCAS
    # instructions for x86, x86_64 and armv8-a (a.k.a. ARM64) architectures (both libstdc++ and libc++).
    #
    # GCC's std::atomic falls back to libatomic: https://gcc.gnu.org/bugzilla/show_bug.cgi?id=80878
    # https://gcc.gnu.org/bugzilla/show_bug.cgi?id=84522
    #
    # Reason 2. Clang still does not support std::atomic_ref, while there is boost::atomic_ref and
    ...
    if("${Boost_VERSION_STRING}" VERSION_GREATER_EQUAL "${BOOST_DWCAS_MIN_VERSION}")
        ...
        get_target_property(BOOST_ATOMIC_INCLUDE_DIR Boost::atomic INCLUDE_DIRECTORIES)
    else()
    ...
    if("${Boost_VERSION_STRING}" VERSION_LESS "${BOOST_DWCAS_OPTIMAL_VERSION}")
        message(WARNING "DWCAS: Performance may be suboptimal. "
    ...
    if(NOT CMAKE_SYSTEM_NAME MATCHES "Darwin"
       AND NOT CMAKE_SYSTEM MATCHES "BSD"
       AND NOT WIN32
    )
    ...
    elseif("${RUN_RESULT}" STREQUAL "1")
        message(
            WARNING
                "DWCAS seems not to work on the host platform, userver may have suboptimal performance. "
