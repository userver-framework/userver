    ...
    if(TARGET "${name}")
        ...
        if(${name}_VERSION)
            if(${name}_FIND_VERSION VERSION_LESS_EQUAL ${name}_VERSION)
                ...
                message(FATAL_ERROR "Already using version ${${name}_VERSION}"
                                    "of ${name} when version ${${name}_FIND_VERSION} " "was requested."
                )
            endif()
        endif()
    endif()
...
macro(_userver_module_end)
    ...
    if(NOT USERVER_CMAKE_DIR)
        message(FATAL_ERROR "userver_setup_environment() should be run before trying to use any Find* from userver")
    endif()
    ...
    if(${name}_FIND_VERSION)
        if("${${current_package_name}_VERSION}" VERSION_LESS "${${name}_FIND_VERSION}")
            message(
                STATUS "Version of ${name} is ${${current_package_name}_VERSION}. "
    ...
    if((NOT "${${libraries_variable}}" STREQUAL "") OR (NOT "${${includes_variable}}" STREQUAL ""))
        ...
        if(${current_package_name}_VERSION)
            set("${name}_VERSION"
                "${${current_package_name}_VERSION}"
                CACHE STRING "Version of the ${name}"
            )
        endif()
    endif()
endmacro()
