...
public:
    ...
    bool emplace(Args&&... args) {
        if (buffer_size_ == 0) {
            ...
            auto& seq_index = fifo_buffer_.template get<0>();
            auto& buffer_seq_index = fifo_buffer_.template get<0>();

            auto result = seq_index.emplace_front(std::forward<Args>(args)...);
            auto buffer_result = buffer_seq_index.emplace_front(std::forward<Args>(args)...);

            if ((!buffer_result.second && result.second) || !result.second) {
