...
private:
    ...
    struct IteratorValuesAreNoexceptMovable<std::tuple<Ts...>>
        : std::conjunction<std::is_nothrow_move_constructible<typename Ts::iterator::value_type>...> {};

    static constexpr auto kCanMoveFromIterators = IteratorValuesAreNoexceptMovable<MappedType>::value;
...
template <typename Row>
typename RowsMapper<Row>::Iterator::reference RowsMapper<Row>::Iterator::operator*() const {
    return UpdateValue();
}

template <typename Row>
typename RowsMapper<Row>::Iterator::pointer RowsMapper<Row>::Iterator::operator->() const {
    return &UpdateValue();
}
