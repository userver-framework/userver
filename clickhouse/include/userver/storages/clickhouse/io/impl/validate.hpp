...
struct TupleColumnsValidate<T, std::index_sequence<S...>> {
    ~TupleColumnsValidate() { (..., (void)impl::EnsureInstantiationOfColumn<std::tuple_element_t<S, T>>{}); }
};
...
template <size_t I, typename Row>
using ClickhouseType = typename std::tuple_element_t<I, MappedType<Row>>::cpp_type;

template <typename T>
inline constexpr auto kCppTypeColumnsCount = boost::pfr::tuple_size_v<T>;

template <typename T>
inline constexpr auto kClickhouseTypeColumnsCount = std::tuple_size_v<MappedType<T>>;
...
constexpr void ValidateColumnsMapping(const T& t) {
    boost::pfr::for_each_field(t, [](const auto& field) { impl::EnsureInstantiationOfVector(field); });
...
constexpr void ValidateRowsMapping() {
    ...
    constexpr auto type_mismatch_index = FieldTypeFindMismatch<T>(std::make_index_sequence<columns_count>());
    if constexpr (type_mismatch_index != columns_count) {
        static_assert(
            std::is_same_v<CppType<type_mismatch_index, T>, ClickhouseType<type_mismatch_index, T>>,
            "Make sure your ClickHouse mapping is correct."
        );
        static_assert(FailIndexAssertion<type_mismatch_index>::value, "Recheck your mapping at this index.");
    }
}
...
void ValidateRowsCount(const T& t) {
    ...
    boost::pfr::for_each_field(t, [&rows_count](const auto& field) {
        ...
        UINVARIANT(*rows_count == field.size(), "All rows should have same number of elements");
    });
}
