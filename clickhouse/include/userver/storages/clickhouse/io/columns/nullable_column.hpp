...
template <typename T>
typename NullableColumn<T>::NullableDataHolder& NullableColumn<T>::NullableDataHolder::operator++() {
    ++nulls_;
    ++inner_;
...
template <typename T>
typename NullableColumn<T>::cpp_type& NullableColumn<T>::NullableDataHolder::UpdateValue() {
    if (!has_value_) {
        if (*nulls_) {
            current_value_.reset();
        } else {
...
template <typename T>
bool NullableColumn<T>::NullableDataHolder::operator==(const NullableDataHolder& other) const {
