...
public:
    InsertionRequest(const std::string& table_name, const std::vector<std::string_view>& column_names);
...
private:
    ...
    public:
        ...
        void operator()(const Field& field, std::integral_constant<size_t, Index> i) {
            ...
            io::columns::AppendWrappedColumn(block_, ColumnType::Serialize(field), column_names_[i], i);
        }
    ...
    public:
        ...
        void operator()(const Field&, std::integral_constant<size_t, Index> i) {
            ...
            io::columns::AppendWrappedColumn(block_, ColumnType::Serialize(column_data), column_names_[i], i);
        }
...
InsertionRequest InsertionRequest::Create(
    ...
    auto mapper = InsertionRequest::ColumnsMapper<MappedType>{*request.block_, request.column_names_};
...
InsertionRequest InsertionRequest::CreateFromRows(
    ...
    InsertionRequest request{table_name, column_names};
    using MappedType = typename io::CppToClickhouse<T>::mapped_type;
    auto mapper = InsertionRequest::RowsMapper<MappedType, Container>{*request.block_, request.column_names_, data};
