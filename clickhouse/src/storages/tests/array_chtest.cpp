...
struct CppToClickhouse<DataWithArrays> {
    using mapped_type = std::tuple<columns::ArrayColumn<columns::UInt64Column>, columns::Int32Column>;
};

template <>
struct CppToClickhouse<DataOfArrays> {
    using mapped_type = std::tuple<columns::UInt64Column, columns::ArrayColumn<columns::StringColumn>>;
};
