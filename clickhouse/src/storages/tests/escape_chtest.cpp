...
struct CppToClickhouse<DataWithOptValue> {
    using mapped_type = std::tuple<columns::StringColumn, columns::NullableColumn<columns::UInt64Column>>;
};
...
UTEST(ExecuteWithArgs, MaliciousString) {
    ...
    const storages::clickhouse::Query q{"SELECT login FROM users WHERE password = {}"};
