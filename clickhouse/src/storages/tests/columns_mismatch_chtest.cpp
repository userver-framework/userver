...
UTEST_DEATH(ColumnsMismatchDeathTest, Insert) {
    ...
    EXPECT_UINVARIANT_FAILURE(cluster->Insert("some_table", {"one", "two", "three"}, data));
}
...
UTEST_DEATH(ColumnsMismatchDeathTest, Select) {
    ...
    const storages::clickhouse::Query less_columns_query{"SELECT toUInt64(0) as one"};
    EXPECT_UINVARIANT_FAILURE(cluster->Execute(less_columns_query).As<DataWithTwoColumns>());
    EXPECT_UINVARIANT_FAILURE(cluster->Execute(less_columns_query).AsRows<DataWithTwoFields>());
    EXPECT_UINVARIANT_FAILURE(cluster->Execute(less_columns_query).AsContainer<std::vector<DataWithTwoFields>>());
}
