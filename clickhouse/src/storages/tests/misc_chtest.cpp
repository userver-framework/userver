...
struct SomeDataRow final {
    ...
    bool operator==(const SomeDataRow& other) const {
        return uint64 == other.uint64 && str == other.str && uint64_2 == other.uint64_2 && timepoint == other.timepoint;
    }
};
...
struct SomeDataRowWithSleep final {
    ...
    bool operator==(const SomeDataRowWithSleep& other) const {
        return uint64 == other.uint64 && str == other.str && sleep_result == other.sleep_result;
    }
};
...
struct CppToClickhouse<SomeData> {
    using mapped_type =
        std::tuple<columns::UInt64Column, columns::StringColumn, columns::UInt64Column, columns::DateTime64ColumnNano>;
};
...
struct CppToClickhouse<SomeDataRowWithSleep> {
    using mapped_type = std::tuple<columns::UInt64Column, columns::StringColumn, columns::UInt8Column>;
};
...
UTEST(Insert, Works) {
    ...
    auto result = cluster->Execute("SELECT id, value, count, tp FROM tmp_table").As<SomeData>();
    ...
    EXPECT_EQ(
        std::chrono::duration_cast<std::chrono::nanoseconds>(result.vec_timepoint.back().time_since_epoch()).count(),
...
UTEST(Query, AvoidUnexpectedCancellation) {
    ...
    cluster->InsertRows("tmp_table_with_sleep", {"id", "value", "sleep_result"}, data);
