...
public:
    static std::optional<std::string> GetCurrentValue(columns::ColumnIterator<columns::StringColumn>& iterator);

    template <typename T>
    static auto& GetCurrentIteratorsTuple(typename io::RowsMapper<T>::Iterator& iterator) {
        return iterator.iterators_;
    }

    template <typename T>
    static constexpr bool kCanMoveFromIterators = io::RowsMapper<T>::kCanMoveFromIterators;
};
