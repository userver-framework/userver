...
UTEST(Execute, MappingWorks) {
    ...
    const std::vector<RowData> as_rows{cluster->Execute(common_query).AsContainer<std::vector<RowData>>()};
...
public:
    ...
    static void CheckCurrentValue(typename io::RowsMapper<Row>::Iterator& iterator, U value) {
        const auto& iterators = io::IteratorsTester::GetCurrentIteratorsTuple<Row>(iterator);
