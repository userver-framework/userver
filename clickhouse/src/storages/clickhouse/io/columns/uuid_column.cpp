...
std::pair<std::uint64_t, std::uint64_t> SerializeToNative(const boost::uuids::uuid& user_uuid, Endianness endianness) {
    std::uint64_t first{};
    std::uint64_t second{};
    ...
    return {ApplyEndianness(first, endianness), ApplyEndianness(second, endianness)};
}

boost::uuids::uuid ParseFromNative(const ::clickhouse::UUID& native_uuid, Endianness endianness) {
    boost::uuids::uuid result{};
    const auto format = [&result, endianness](std::size_t offset, std::uint64_t data) {
...
ColumnRef SerializeToNative(const std::vector<boost::uuids::uuid>& what, Endianness endianness) {
    ...
    return std::make_shared<NativeType>(std::make_shared<clickhouse::impl::clickhouse_cpp::ColumnUInt64>(std::move(data)
    ));
}
...
static_assert(
    std::is_trivially_copyable_v<decltype(MismatchedEndiannessUuidColumn::cpp_type::data)>,
    "Can not use memcpy."
);
static_assert(
    sizeof(MismatchedEndiannessUuidColumn::cpp_type::data) == sizeof(std::uint64_t) * 2,
...
ColumnRef MismatchedEndiannessUuidColumn::Serialize(const container_type& from) {
...
ColumnRef UuidRfc4122Column::Serialize(const container_type& from) { return SerializeToNative(from, Endianness::kBE); }
