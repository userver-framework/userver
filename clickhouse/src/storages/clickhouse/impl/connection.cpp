...
void AppendToBlock(NativeBlock& result, const NativeBlock& new_data) {
    ...
    UINVARIANT(result.GetColumnCount() == new_data.GetColumnCount(), "Shouldn't happen");

    for (size_t ind = 0; ind < result.GetColumnCount(); ++ind) {
        const auto& result_column = result[ind];
        const auto& new_column = new_data[ind];
        UINVARIANT(result_column->Type()->IsEqual(new_column->Type()), "Shouldn't happen");
...
engine::Deadline GetDeadline(OptionalCommandControl optional_cc) {
    const auto duration = optional_cc.has_value() ? optional_cc->execute : kDefaultExecuteTimeout;
...
void Connection::Insert(OptionalCommandControl optional_cc, const InsertionRequest& request) {
    const auto& block = request.GetBlock();

    auto guard = GetBrokenGuard();
    client_.Insert(request.GetTableName(), block.GetNative(), GetDeadline(optional_cc));
}
...
Connection::ConnectionBrokenGuard Connection::GetBrokenGuard() { return ConnectionBrokenGuard{broken_}; }

void Connection::DoExecute(OptionalCommandControl optional_cc, const clickhouse_cpp::Query& query) {
    auto guard = GetBrokenGuard();
