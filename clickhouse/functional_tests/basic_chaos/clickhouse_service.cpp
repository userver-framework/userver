...
struct CppToClickhouse<chaos::PairOfUuids> final {
    using mapped_type = std::tuple<columns::MismatchedEndiannessUuidColumn, columns::UuidRfc4122Column>;
};
...
PairOfUuids Parse(const formats::json::Value& value, formats::parse::To<PairOfUuids>) {
    PairOfUuids result{};
    result.uuid_mismatched = utils::BoostUuidFromString(value["uuid_mismatched"].As<std::string>());
    result.uuid_correct = utils::BoostUuidFromString(value["uuid_correct"].As<std::string>());
...
private:
    formats::json::Value PostValue(const formats::json::Value& request_json) const {
        const auto pair_of_uuids = request_json.As<PairOfUuids>();
        const std::vector<PairOfUuids> rows{{pair_of_uuids.uuid_mismatched, pair_of_uuids.uuid_correct}};
        clickhouse_client_->InsertRows(cc_, "uuids", {"uuid_mismatched", "uuid_correct"}, rows);
    ...
    formats::json::Value GetValues() const {
        const auto rows =
            clickhouse_client_->Execute(cc_, "SELECT uuid_mismatched, uuid_correct FROM uuids").AsRows<PairOfUuids>();
