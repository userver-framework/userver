...
template <typename T>
void DoTestPreparedRequestType(ydb::TableClient& table_client, std::string type_str, std::string value_str, T value) {
    ...
    auto result = table_client.ExecuteDataQuery(ydb::OperationSettings{}, query, std::move(builder));
...
UTEST_F(TTypesYdbTestCase, PreparedRequestType) {
    ...
    DoTestPreparedRequestType(GetTableClient(), "Int32", "value_int32", std::int32_t{-32});
    DoTestPreparedRequestType(GetTableClient(), "Uint32", "value_uint32", std::uint32_t{32});
    DoTestPreparedRequestType(GetTableClient(), "Int64", "value_int64", std::int64_t{-64});
    DoTestPreparedRequestType(GetTableClient(), "Uint64", "value_uint64", std::uint64_t{64});
    DoTestPreparedRequestType<double>(GetTableClient(), "Double", "value_double", 1.23);
...
UTEST_F(TTypesYdbTestCase, PreparedStructUpsertTypes) {
    ...
    auto row = ydb::InsertRow{
        ...
        ydb::InsertColumn{"value_ts", std::chrono::system_clock::time_point(std::chrono::microseconds{12345})},
    };
