...
class RetryOperationFixture : public ydb::ClientFixtureBase {
 public:
  ...
  auto RetryOperationSync(std::size_t retries, Func func) {
    ...
    return ydb::impl::GetFutureValueUnchecked(
        ydb::impl::RetryOperation(context, std::move(func)));
  }
...
inline NThreading::TFuture<NYdb::TStatus> MakeStatusFuture(
    NYdb::EStatus status) {
...
UTEST_F(RetryOperationFixture, HandleOfInheritorsOfTStatus) {
  const TestOperationResults data{NYdb::TStatus{kSuccess, NYql::TIssues()},
                                  "qwerty"};
  auto res = RetryOperationSync(
      /*retries=*/0, [&data](NYdb::NTable::TSession) {
        return NThreading::MakeFuture<TestOperationResults>(
            TestOperationResults{data});
      });
...
UTEST_F(RetryOperationFixture, Success) {
  ...
  auto res =
      RetryOperationSync(/*retries=*/3, [&attempts](NYdb::NTable::TSession) {
        attempts++;
...
UTEST_F(RetryOperationFixture, SuccessOnTheLastAttempt) {
  ...
  std::size_t attempts = 0;
  auto res = RetryOperationSync(/*retries=*/kRetries,
...
UTEST_F(RetryOperationFixture, AttemptsIsRetriesPlusOne) {
  ...
  std::size_t attempts = 0;
  auto res = RetryOperationSync(
      /*retries=*/kRetries, [&attempts](NYdb::NTable::TSession) {
        attempts++;
        return MakeStatusFuture(kRetryableStatus);
      });
...
UTEST_F(RetryOperationFixture, RetriesLimit) {
  ...
  std::size_t attempts = 0;
  auto res = RetryOperationSync(
      /*retries=*/1000, [&attempts](NYdb::NTable::TSession) {
        attempts++;
        if (attempts < kRetries) {
          return MakeStatusFuture(kRetryableStatus);
        }
...
UTEST_F(RetryOperationFixture, Exception) {
  UASSERT_THROW_MSG(RetryOperationSync(
                        /*retries=*/0,
