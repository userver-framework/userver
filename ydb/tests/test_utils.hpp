...
public:
    ...
    utils::statistics::Snapshot GetMetrics() const { return utils::statistics::Snapshot{statistics_storage_}; }
    ...
    std::shared_ptr<ydb::CoordinationClient> GetCoordinationClientPtr() { return coordination_client_; }
    ...
    ~ClientFixtureBase() override {
        for (const auto& table : tables_created_) {
            try {
                ...
                ADD_FAILURE() << "Failed to drop table '" << table << "' after test: " << ex.what();
            }
        }
    }

    void DoCreateTable(std::string_view path, NYdb::NTable::TTableDescription&& table_desc) {
