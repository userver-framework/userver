...
UTEST_F(YdbCoordinationFixture, SessionState) {
    ...
    ASSERT_EQ(NYdb::NCoordination::ESessionState::ATTACHED, session.GetSessionState());
    ASSERT_EQ(NYdb::NCoordination::EConnectionState::CONNECTED, session.GetConnectionState());
}
...
UTEST_F(YdbCoordinationFixture, CreateSemaphore) {
    ...
    UASSERT_NO_THROW(
        desc =
            session.DescribeSemaphore(kSemaphoreName, NYdb::NCoordination::TDescribeSemaphoreSettings{}.IncludeOwners())
    );
