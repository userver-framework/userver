...
utils::statistics::Rate Load(const utils::statistics::Rate& metric) { return metric; }

utils::statistics::Rate Load(const utils::statistics::RateCounter& metric) { return metric.Load(); }
...
void DumpMetric(utils::statistics::Writer& writer, const StatsAggregator& stats) { DoDumpMetric(writer, stats); }

Stats::Stats(
    utils::span<const double> by_database_histogram_bounds,
...
void DumpMetric(utils::statistics::Writer& writer, const Stats& stats) {
    {
...

        {
            ...
            for (const auto& [query_name, query_stats] : stats.by_query) {
                ...
                by_query_writer.ValueWithLabels(current_approx, {"ydb_query", query_name});
            }
        }
...

    {
...

        {
            ...
            for (const auto& [tx_name, tx_stats] : stats.by_transaction) {
                ...
                by_transaction_writer.ValueWithLabels(current_approx, {"ydb_transaction", tx_name});
            }
        }
