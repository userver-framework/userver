...
ydb::OperationSettings Parse(const yaml_config::YamlConfig& config, To<ydb::OperationSettings>) {
    ...
    result.retries = config["retries"].As<uint32_t>(ydb::kDefaultOperationRetries);
    result
        .operation_timeout_ms = config["operation-timeout"].As<std::chrono::milliseconds>(ydb::kDefaultOperationTimeout
    );
    result.cancel_after_ms = config["cancel-after"].As<std::chrono::milliseconds>(ydb::kDefaultCancellationTimeout);
    result.client_timeout_ms = config["client-timeout"].As<std::chrono::milliseconds>(ydb::kDefaultClientTimeout);
    result.get_session_timeout_ms =
        config["get-session-timeout"].As<std::chrono::milliseconds>(ydb::kDefaultGetSessionTimeout);
