...
bool IsStopped(CoordinationSession& session) {
    return NYdb::NCoordination::EConnectionState::STOPPED == session.GetConnectionState();
}
    ...
    [[nodiscard]] bool Acquire() {
        try {
            ...
            const auto result = session_.AcquireSemaphore(
                semaphore_name_,
                NYdb::NCoordination::TAcquireSemaphoreSettings{}.Count(impl::dist_lock::kSemaphoreLimit)
            );
    ...
    void Run(bool run_once) {
        while (!engine::current_task::ShouldCancel() && !IsStopped(session_)) {
            ...
            if (0 == *idx) {
                ...
                CancelTask(std::move(task));
                engine::InterruptibleSleepUntil(from_time + settings_.restart_session_delay);
                return;
            }
...
void DistLockedWorker::Start() {
    ...
    worker_task_ = utils::CriticalAsync(task_processor_, "dist-locked-worker", [this] { Run(/*run_once=*/false); });
...
void DistLockedWorker::Run(bool run_once) {
    while (!engine::current_task::ShouldCancel()) {
        try {
            ...
            session_settings.OnStopped([session_stopped_promise = std::move(session_stopped_promise)]() mutable {
                session_stopped_promise->set_value();
            });
            auto session = coordination_client_->StartSession(coordination_node_, session_settings);
            LOG_INFO() << "Session started";
            ++stats_->start_session_successes;

            SessionTask session_task{
                std::move(session),
                std::move(session_stopped_future),
                semaphore_name_,
                callback_,
                settings_,
                *stats_,
...
void DumpMetric(utils::statistics::Writer& writer, const DistLockedWorker& worker) {
    ...
    writer["cancel-task-time-limit-exceeded"] = stats.cancel_task_time_limit_exceeded;
}
